...
>
> **선배**: "Compose는 한 서버 안에서 묶어 돌리는 거잖아. 자동 복구에 무중단 배포, 트래픽 따라 늘렸다 줄였다까지 하려면 다른 게 필요해. 니 이제 Kubernetes 만날 때 됐다."
...
쿠버네티스가 무엇이고 어떻게 동작하는지 알아보았습니다. 이제 직접 실행해 볼 차례입니다. 로컬 PC 한 대로도 쿠버네티스를 체험할 수 있는 **미니큐브(Minikube)** 를 먼저 설치해보겠습니다.
...
![Pod --- 최소 실행 단위](images/k8s-step1.png)
*그림 4-8: Pod --- 컨테이너를 감싸는 최소 실행 단위*
...
![kubectl run hello-pod1 실행 결과](images/01_kubectl-run-hello-pod1.png)
*그림 4-9: kubectl run hello-pod1 실행 결과*

> **오픈이**: "이게 컨테이너랑 뭐가 다른 거지...?"

이 한 줄이 실제로 무엇을 했는지 궁금하다면, YAML 파일로 풀어 볼 수 있습니다. **같은 결과를 파일로 저장해서 재사용할 수 있게 만든 것** 이 YAML입니다.
...
위에서 `kubectl run`으로 만든 Pod을, YAML 파일로 작성하면 이렇게 됩니다.
...
명령어 한 줄과 비교해 보겠습니다. `kubectl run hello-pod1 --image=nginx`에서 Pod 이름은 `metadata.name`에, 이미지는 `spec.containers[].image`에 대응됩니다. YAML은 같은 내용을 **파일로 저장해서 반복 사용할 수 있는 영구 저장판** 입니다.
...
![Pod 목록 조회](images/02_kubectl-get-pod.png)
*그림 4-11: Pod 목록 조회*
...
>
> Docker에서는 컨테이너마다 독립된 Network Namespace(독립 IP)를 가졌습니다. Kubernetes의 Pod은 다릅니다. Pod이 생성되면 **pause** 라는 인프라 컨테이너가 먼저 만들어지고, 앱 컨테이너들이 pause의 Network Namespace에 합류합니다. 그래서 같은 Pod 안의 컨테이너들은 **같은 IP, 같은 포트 공간** 을 공유하며 **localhost** 로 서로 통신합니다. Docker에서 컨테이너가 네트워크의 단위였다면, Kubernetes에서는 **Pod이 네트워크의 단위** 입니다.
>
> ![Pod 공유 네트워크](images/net-07-pod-namespace.png)
> *그림 4-A1: Pod의 공유 Network Namespace --- pause 컨테이너가 네트워크를 소유*
>
...
![Deployment -> ReplicaSet -> Pod](images/k8s-step2.png)
*그림 4-13: Deployment가 ReplicaSet을 생성하고, ReplicaSet이 Pod 수를 유지한다*
...
*그림 4-14: Selector가 app: web인 Pod만 매칭하고, app: db인 Pod는 매칭하지 않음*
...
![Deployment와 Pod 생성 확인](images/03_kubectl-get-pod-deploy.png)
*그림 4-15: Deployment와 Pod 생성 확인*

이제 진짜 실험입니다. 오픈이는 모든 Pod를 삭제해보았습니다. 4.2에서 만든 hello-pod1, hello-pod2도 포함됩니다.
...
![Pod 제거 후 자동 재생성 확인](images/04_kubectl-delete-pod-all.png)
*그림 4-16: Pod 제거 후 자동 재생성 확인*

> **오픈이**: "어?! hello-pod1, hello-pod2는 사라졌는데 Deployment로 만든 건 알아서 살아났어요! 이거 누가 살려준 거예요?"
>
> **선배**: "Deployment가 살린 거지. 'Pod 1개 유지해라' 선언해놨으니까, 죽으면 알아서 새로 만드는 거야. 이게 선언적 관리거든."
...
*그림 4-17: Pod 3이 종료되면 ReplicaSet이 설정 개수를 맞추기 위해 Pod 4를 자동 생성*
...
![replicas 설정으로 Pod 4개 생성](images/07_kubectl-get-pods-replicas.png)
*그림 4-18: replicas 설정으로 Pod 4개 생성*
...
![롤링 업데이트 진행 상황](images/06_kubectl-get-pods-w-rolling.png)
*그림 4-20: 롤링 업데이트 진행 상황*
...
> **오픈이**: "버전 바꿨는데 서비스가 안 끊기는데요?!"

> deploy-ex02.yml에서 `maxSurge: 4`, `maxUnavailable: 0`으로 설정했기 때문에, 새 Pod 4개가 먼저 생성되고 모두 Running 상태가 된 뒤에야 기존 Pod가 순차적으로 종료됩니다. 기존 Pod가 먼저 죽지 않으므로 서비스 중단이 발생하지 않습니다.
...
![Service -> Pod](images/k8s-step3.png)
*그림 4-22: Service는 고정 주소를 제공한다. Pod IP가 바뀌어도 Service 주소는 그대로다*
...
![Pod IP 변경 확인](images/08_pod-ip-change.png)
*그림 4-23: Pod 재시작 시 IP 변경 확인*
...
![Service 생성](images/10_kubectl-apply-service.png)
*그림 4-24: Service 생성*
...
| **NodePort** | ClusterIP + 노드의 특정 포트(30000~32767)를 통해 외부에서 접근 가능 | 외부 접근 가능 |
| **LoadBalancer** | NodePort + 클라우드 환경에서 외부 로드밸런서를 자동 생성 | 외부 접근 가능 (클라우드) |

> **네트워크 돋보기: kube-proxy와 iptables DNAT**
>
> Service의 ClusterIP(예: **10.96.0.10**)는 실제 네트워크 인터페이스에 할당된 IP가 아닙니다. 그러면 이 가상 IP로 오는 패킷은 누가 처리할까요? 모든 Node에서 실행되는 **kube-proxy** 가 그 역할을 합니다. kube-proxy는 API Server를 감시하다가 Service나 Pod에 변경이 생기면, 자기 Node의 **iptables 규칙을 자동 업데이트** 합니다. Service 뒤에 Pod이 3개 있다면, "Service IP로 오는 패킷을 1/3 확률로 각 Pod에 DNAT하라"는 규칙이 만들어집니다.
>
> Docker에서 **-p 8080:80** 포트매핑이 iptables DNAT였던 것처럼, Service의 가상 IP를 Pod의 실제 IP로 바꾸는 것도 동일한 DNAT입니다. 범위가 한 대의 호스트에서 클러스터 전체로 확장되었을 뿐입니다.
>
> ![kube-proxy DNAT](images/net-08a-kube-proxy.png)
> *그림 4-A2: kube-proxy가 설치한 iptables DNAT 규칙*
>
> ![Service 타입 계층](images/net-08b-service-layers.png)
> *그림 4-A3: Service 타입 계층 구조 --- ClusterIP / NodePort / LoadBalancer*
>
> **한 줄 정리**: kube-proxy가 iptables 규칙을 자동 관리하여, Service의 가상 IP를 실제 Pod IP로 DNAT합니다.
...
![Pod 삭제 후 Service 접속](images/11_delete-pod-minikube-service.png)
*그림 4-27: Pod 삭제 후 Service 접속*
...
> **오픈이**: "도메인으로 접속하고 싶은데요!"
>
> **선배**: "그러면 Ingress를 써야 돼. 건물로 치면, Service가 부서별 내선 번호고 Ingress는 1층 **안내 데스크** 같은 거야."

![Ingress -> Service -> Pod](images/k8s-step4c.png)
*그림 4-28: 외부 요청은 Ingress를 통해 Service로 전달된다*
...
- **Docker에서 배운 네트워크가 Kubernetes에서도 그대로 쓰입니다.** 포트포워딩의 iptables DNAT가 kube-proxy의 Service 라우팅으로, Docker DNS가 CoreDNS로 확장되었습니다. 마법이 아니라 같은 도구의 클러스터 버전입니다.

쿠버네티스의 핵심인 Pod, Deployment, Service까지 익혔습니다. 하지만 실제 서비스를 운영하려면 DB 비밀번호는 어디에 넣고, 외부 사용자는 어떻게 접속하게 할까요? 챕터 5에서는 ConfigMap과 Secret으로 설정을 외부에서 주입하고, Ingress로 외부 요청을 라우팅하며, 3장에서 만든 풀스택 웹사이트를 Kubernetes 위에 배포합니다.
