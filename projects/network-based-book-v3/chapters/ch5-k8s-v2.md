...
실제 서비스를 운영하려면 데이터베이스 주소, API 키, 비밀번호 같은 설정 값이 필요합니다. 이런 값을 코드에 직접 넣으면 값이 바뀔 때마다 이미지를 다시 빌드해야 합니다. ConfigMap과 Secret이 이 문제를 해결합니다.

![ConfigMap / Secret -> Pod](images/k8s-step4a.png)
...
![Pod 환경 변수에 Secret 값 확인](images/13_secret-env.png)
...
`ConfigMap`이나 `Secret`에서 환경 변수 값을 변경한 후 `kubectl apply -f 파일명`을 실행하면 변경 내용이 `Kube API Server`에 저장됩니다. 저장된 환경 변수가 실행 중인 `Pod`에 즉시 반영될까요? 그렇지 않습니다.
...
>
> 5.2절의 풀스택 예제에서 ConfigMap에 **db-service:3306**, **redis-service:6379** 처럼 IP 대신 서비스 이름을 적게 됩니다. 이것이 가능한 이유는 쿠버네티스 안에 **CoreDNS** 라는 전용 DNS 서버가 돌고 있기 때문입니다. Service가 생성되면 CoreDNS에 **서비스명.네임스페이스.svc.cluster.local** 형태의 DNS 레코드가 자동 등록됩니다. 같은 네임스페이스라면 서비스 이름만으로 충분합니다.
>
> 인터넷의 DNS가 도메인을 IP로 변환하듯, CoreDNS는 서비스 이름을 ClusterIP로 변환합니다. DB Pod이 죽고 새로 태어나 IP가 바뀌더라도, 서비스 이름과 ClusterIP는 그대로이므로 설정을 바꿀 필요가 없습니다.
>
> ![CoreDNS 이름 해석](images/net-09-coredns.png)
> *그림 5-A1: CoreDNS --- 서비스 이름을 ClusterIP로 변환*
>
> **한 줄 정리**: CoreDNS 덕분에 Pod끼리 IP 대신 서비스 이름으로 통신할 수 있습니다.

---

## 5.2 풀스택 웹사이트 : Kubernetes 배포
...
![ex08 풀스택 Kubernetes 아키텍처](images/fig-3-7-v2.png)
*그림 5-8: ex08 풀스택 Kubernetes 아키텍처*
...
*그림 5-9: 같은 Cluster 안에서 Namespace로 리소스를 분리한다*
...
**PV(PersistentVolume)** 와 **PVC(PersistentVolumeClaim)** 는 챕터 2에서 배운 마운트의 Kubernetes 버전입니다. 컨테이너가 삭제되어도 데이터를 보존하는 원리는 같지만, Kubernetes에서는 저장소(PV)와 저장소 요청서(PVC)를 분리합니다. Pod가 직접 저장소를 지정하는 대신 "이만큼의 저장 공간이 필요하다"는 요청서(PVC)를 제출하면 Kubernetes가 조건에 맞는 저장소(PV)를 연결해주는 구조입니다.
...
*그림 5-10: Ingress Controller 활성화*
...
*그림 5-11: Ingress Controller 실행 확인*
...
*그림 5-15: Deployment, Pod, Service 조회*
...
>
> ![전체 경로 1](images/net-10a-full-path.png)
> *그림 5-A2: 전체 경로 (1) --- 브라우저 -> Ingress -> frontend Service -> frontend Pod*
>
> ![전체 경로 2](images/net-10b-full-path.png)
> *그림 5-A3: 전체 경로 (2) --- frontend Pod -> backend Service -> backend Pod -> DB/Redis*
>
> **한 줄 정리**: Ingress -> Service -> Pod의 경로에서 CoreDNS가 이름을 IP로, kube-proxy가 가상 IP를 Pod IP로 변환합니다.

---

## 5.3 문제가 생겼을 때 : 디버깅

풀스택 배포까지 성공한 오픈이. 그런데 다음 날, Pod STATUS에 **CrashLoopBackOff** 가 떠 있었습니다.
...
| **Service 접속 불가** | selector와 Pod labels 불일치 | `kubectl get endpoints <Service명>`으로 연결된 Pod이 있는지 확인. labels가 일치하는지 점검 |
...
```bash
kubectl get endpoints <Service명>     # Pod이 연결되어 있는지 확인
```
...
- **Docker에서 배운 네트워크가 Kubernetes 전체를 관통합니다.** 포트포워딩의 iptables DNAT가 kube-proxy로, Docker DNS가 CoreDNS로, docker0 브리지가 cni0로 확장되었습니다. 마법이 아니라 같은 리눅스 도구의 클러스터 버전입니다.
...
Docker와 Kubernetes를 향한 여정이 마무리되었습니다. 2장에서 컨테이너 하나를 띄우는 것으로 시작해, 3장에서 Docker Compose로 여러 컨테이너를 한 번에 관리했습니다. 4장에서 쿠버네티스의 핵심(Pod, Deployment, Service)을 익혔고, 5장에서 설정 관리와 Ingress, 풀스택 배포까지 완성했습니다. Ingress로 외부 요청을 도메인 기반으로 연결하는 것까지 마쳤으니, 실무 환경과의 거리가 한층 좁혀졌습니다. 오픈이는 조금 더 자신감을 갖게 되었습니다. 새벽 알람이 오더라도, 이제는 쿠버네티스가 먼저 움직일 것입니다.
