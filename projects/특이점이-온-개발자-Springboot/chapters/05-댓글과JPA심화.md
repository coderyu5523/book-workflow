...
게시글 하나에 댓글 여러 개가 달리므로 `@OneToMany`이고, `mappedBy = "board"`가 "이 관계의 주인은 내가 아니라 `Reply`의 `board` 필드"라고 밝히는 부분입니다. 게시글은 외래 키를 갖지 않고, 댓글이 들고 있는 `board_id`를 거꾸로 따라가 자기에게 달린 댓글을 `replies`에 채웁니다. 그래서 이 `replies`는 데이터베이스 테이블에 컬럼으로 생기지 않습니다. 객체에서 관계를 다루기 위한 참조일 뿐이라 `board_tb`에도 `reply_tb`에도 `replies` 칸은 없고, 실제 연결은 여전히 `reply_tb`의 `board_id` 하나로만 저장됩니다. 뒤에 붙인 `cascade = CascadeType.REMOVE`가 앞의 영속성 전이여서, 게시글을 지우면 이 목록에 담긴 댓글까지 함께 지워집니다. `replies`를 `new ArrayList<>()`로 초기화해 두면 아직 댓글이 없는 글도 빈 목록을 가집니다.

## 5.2 댓글을 쓰고 지운다

엔티티로 관계를 이었으니, 이제 댓글을 실제로 쓰고 지우는 부분을 만듭니다. 구조는 게시글을 만들 때와 같아서, 리포지토리로 저장하고 서비스가 흐름을 맡고 컨트롤러가 요청을 받습니다.
...
2장에서 만든 `BoardRepository`와 같은 모양입니다. `save`는 `em.persist`로 댓글을 작업대에 올리고, `findById`는 없으면 빈 `Optional`을 돌려주며, `delete`는 삭제 대상으로 표시합니다. 댓글은 목록을 따로 조회하지 않는데, 댓글 목록은 게시글 상세를 부를 때 글에 딸려 함께 나가기 때문입니다.
...
`댓글쓰기`는 먼저 `boardId`로 댓글을 달 글을 찾고, 없는 글이면 `Exception404`를 던져 막습니다. 대상 글을 찾으면 `toEntity`로 로그인 유저와 그 글을 붙인 댓글을 만들어 저장하는데, 이때 엔티티가 들고 있는 `board`의 기본 키가 `board_id` 칸에 담기면서 글과 이어집니다. `댓글삭제`는 4장에서 남의 글을 막던 방식을 그대로 가져와, 댓글의 작성자 아이디와 요청을 보낸 사람의 아이디를 견주고 다르면 `Exception403`을 던집니다. 대상이 글에서 댓글로 바뀌었을 뿐 소유자를 따지는 구조는 4장과 똑같습니다. 두 메서드 모두 데이터를 바꾸므로 `@Transactional`을 붙였습니다.
...
댓글 쓰기와 삭제도 로그인한 사람만 하도록 막아야 합니다. 4장에서 만든 인터셉터가 게시글 주소만 지키고 있으므로, `core/config/WebMvcConfig.java`의 인터셉터 보호 경로에 `/api/replies`와 그 하위 경로를 더합니다. 그러면 게시글과 똑같이 댓글 쓰기와 삭제도 로그인하지 않으면 인터셉터에서 401로 막힙니다.
...
`DetailDTO`에 두 가지가 붙었습니다. 하나는 댓글 목록입니다. `board.getReplies()`로 글에 달린 댓글을 꺼내 각각을 안쪽의 `ReplyDTO`로 바꿔 담으므로, 상세를 부르면 글과 함께 그 글의 댓글이 한 덩어리로 나갑니다. 다른 하나는 `isOwner`인데, 로그인한 사람이 이 글(또는 이 댓글)의 작성자인지를 참·거짓으로 알려 주는 값입니다. `checkOwner`가 요청을 보낸 사람의 아이디(`sessionUserId`)와 작성자 아이디를 견줘 같으면 `true`를 돌려주고, 비로그인이라 `sessionUserId`가 `null`이면 `false`입니다. 이 값은 화면에서 수정·삭제 버튼을 본인에게만 보여 줄 때 쓰입니다. 서버가 "이건 당신 글입니다"를 미리 계산해 내려 주는 셈입니다.
...
핵심은 `join fetch b.user`입니다. 지연 로딩에서는 글을 먼저 가져오고 작성자를 프록시로 미뤄 두지만, `join fetch`는 글을 조회하는 그 select에 작성자 조회를 끼워 넣어 한 번에 가져옵니다. 그래서 이 메서드로 가져온 글은 작성자가 이미 채워져 있어, `getUser().getUsername()`을 불러도 추가 쿼리가 나가지 않습니다. 앞에서 본 두 번째 select가 사라지는 것입니다. 앞 장에서 별생각 없이 이름만 보고 쓰던 그 메서드가, 사실은 이 N+1을 막는 장치였습니다.
...
작성자와 댓글을 모두 꺼냈는데도 SQL 로그에는 조회할 때 나간 select 하나뿐입니다. 5.3의 `findByIdLazyLoading_test`에서는 작성자를 꺼내는 순간 select가 한 번 더 나갔지만, 여기서는 조회 한 번에 작성자와 댓글이 담겨 오기 때문입니다. 작성자만 함께 가져오는 `findByIdJoinUser_test`도 마찬가지로 `getUsername()`에서 추가 쿼리가 나가지 않습니다.
