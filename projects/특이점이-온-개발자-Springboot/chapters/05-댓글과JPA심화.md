...
앞 챕터에서 오픈이는 게시판에 문을 두 개 달았습니다. 로그인한 사람만 글을 쓰고, 본인만 자기 글을 고치고 지웁니다. 며칠 뒤, 화면을 붙이던 동료가 노트북을 들고 자리로 왔습니다.

**동료**: "로그인도 되고 남의 글도 못 고치게 됐는데, 정작 댓글을 못 달아요. 게시판인데 댓글이 없으니까 화면이 허전하네요."

오픈이는 동료가 띄워 둔 목업을 들여다봤습니다. 글 아래에 댓글 입력칸이 비어 있었습니다.

*댓글이라. 글은 혼자 서는데, 댓글은 어디에 붙지?*

글은 그 자체로 하나의 자원입니다. 하지만 댓글은 다릅니다. "누구의 어느 글에 달린 댓글인가"가 빠지면 댓글은 공중에 뜬 문장일 뿐입니다. 댓글을 만들려면 먼저 댓글이 어느 글에 매달리는지부터 정해야 합니다. 감이 잡히지 않아, 오픈이는 옆자리 선배에게 물었습니다.

**오픈이**: "댓글을 만들려는데, 댓글이랑 글을 어떻게 이어야 할지 모르겠어요."<br>
**선배**: "댓글은 글에 붙이는 포스트잇 같은 거예요. 포스트잇마다 '몇 번 글에 붙는다'가 적혀 있고, 글이 사라지면 거기 붙은 포스트잇도 같이 사라져야 하잖아요. 그렇게 글과 댓글이 서로를 아는 관계를 만들어 두면 돼요."

포스트잇. 오픈이는 게시물 아래에 쪽지가 줄줄이 붙은 장면을 떠올렸습니다. 글과 댓글을 이어 두면, 게시글 상세를 부를 때 글에 딸린 댓글이 함께 나와야 합니다. 그런데 그 함께 나오는 조회가 뒤에서 예상치 못한 문제를 부릅니다. 이번 챕터가 나아갈 큰 흐름을 먼저 펼쳐 보겠습니다.
...
선배가 말한 포스트잇을 오픈이는 다시 떠올렸습니다. 포스트잇에는 저마다 "어느 글의 것"이라는 표시가 적혀 있습니다. 글은 자기에게 붙은 포스트잇이 몇 장인지 일일이 들고 있지 않아도 됩니다. 각 포스트잇이 자기가 어느 글 소속인지 알고 있으니, 그 표시만 따라가면 글에 붙은 포스트잇을 전부 모을 수 있습니다.
...
이번 챕터에서 새로 만드는 댓글 도메인과, 손을 대는 게시글 계열 클래스를 먼저 정리하면 다음과 같습니다.
...
댓글 하나는 누가 썼는지(`user`)와 어느 글에 달렸는지(`board`)를 함께 담습니다. 두 필드 모두 `@ManyToOne`입니다. 댓글 여러 개가 한 명의 회원에 속하고, 댓글 여러 개가 한 게시글에 속하기 때문입니다. `@JoinColumn(name = "board_id")`이 앞서 이야기한 그 표시입니다. `reply_tb`에 `board_id` 칸을 두고, 그 칸에 소속 글의 기본 키를 담습니다. 외래 키를 이 엔티티가 들고 있으니 댓글이 연관관계 주인입니다.

두 `@ManyToOne`에 붙은 `fetch = FetchType.LAZY`는 지연 로딩 설정입니다. 이것이 이번 챕터 후반의 핵심이 되므로, 여기서는 붙여만 두고 5.3에서 제대로 다룹니다.
...
엔티티로 관계를 이었으니, 이제 댓글을 실제로 쓰고 지우는 부분을 만듭니다. 구조는 게시글을 만들 때와 같습니다. 리포지토리로 저장하고, 서비스가 흐름을 맡고, 컨트롤러가 요청을 받습니다.
...
2장에서 만든 `BoardRepository`와 같은 모양입니다. `save`는 `em.persist`로 댓글을 작업대에 올리고, `findById`는 없으면 빈 `Optional`을 돌려주며, `delete`는 삭제 대상으로 표시합니다. 댓글은 목록을 따로 조회하지 않습니다. 댓글 목록은 게시글 상세를 부를 때 글에 딸려 함께 나가기 때문입니다.
...
`SaveDTO`는 댓글 내용과 어느 글에 달지(`boardId`)를 받습니다. `toEntity`는 4장에서 게시글을 만들 때처럼 로그인 유저와 대상 글을 넘겨받아 댓글 엔티티로 옮겨 담습니다. `DTO`는 응답으로 나갈 댓글 하나를 담는 접시입니다.
...
`댓글쓰기`는 먼저 `boardId`로 댓글을 달 글을 찾습니다. 없는 글에 댓글을 달려 하면 `Exception404`를 던져 막습니다. 대상 글을 찾으면 `toEntity`로 로그인 유저와 그 글을 붙인 댓글을 만들어 저장합니다. 댓글이 저장될 때, 엔티티가 들고 있는 `board`의 기본 키가 `board_id` 칸에 담기면서 글과 이어집니다.

`댓글삭제`는 4장에서 남의 글을 막던 방식을 그대로 가져옵니다. 댓글을 조회해, 그 작성자 아이디와 요청을 보낸 사람의 아이디를 견줍니다. 두 값이 다르면 남의 댓글을 지우려는 것이므로 `Exception403`을 던집니다. 소유자를 따지는 이 검증은 대상이 글에서 댓글로 바뀌었을 뿐, 4장에서 만든 구조와 똑같습니다. 두 메서드 모두 데이터를 바꾸므로 `@Transactional`을 붙였습니다.
...
댓글 쓰기와 삭제도 로그인한 사람만 하도록 막아야 합니다. 4장에서 만든 인터셉터가 게시글 주소만 지키고 있으므로, `core/config/WebMvcConfig.java`의 인터셉터 보호 경로에 `/api/replies`와 그 하위 경로를 더합니다. 이렇게 하면 게시글과 똑같이, 댓글 쓰기와 삭제도 로그인하지 않으면 인터셉터에서 401로 막힙니다.

댓글은 저장했지만, 아직 화면에서 보이지 않습니다. 댓글은 게시글 상세에 딸려 나가야 합니다. 5.1에서 게시글에 `replies` 목록을 달아 두었으니, 상세 응답 그릇에 그 목록을 담습니다. `board/BoardResponse.java`의 `DetailDTO`를 아래처럼 고칩니다.
...
`DetailDTO`에 두 가지가 붙었습니다. 하나는 댓글 목록입니다. `board.getReplies()`로 글에 달린 댓글을 꺼내, 각각을 안쪽의 `ReplyDTO`로 바꿔 담습니다. 상세를 부르면 글과 함께 그 글의 댓글이 한 덩어리로 나갑니다.

다른 하나는 `isOwner`입니다. 로그인한 사람이 이 글(또는 이 댓글)의 작성자인지를 참·거짓으로 알려 주는 값입니다. `checkOwner`가 요청을 보낸 사람의 아이디(`sessionUserId`)와 작성자 아이디를 견줘, 같으면 `true`를 돌려줍니다. 비로그인이라 `sessionUserId`가 `null`이면 `false`입니다. 이 값은 화면에서 수정·삭제 버튼을 본인에게만 보여 줄 때 쓰입니다. 서버가 "이건 당신 글입니다"를 미리 계산해 내려 주는 것입니다.

`isOwner`를 채우려면 상세 조회가 로그인한 사람이 누구인지 알아야 합니다. 그래서 서비스가 `sessionUserId`를 받습니다. `board/BoardService.java`의 상세 메서드를 아래처럼 고칩니다.
...
조회는 `findByIdJoinUserAndReply`로 합니다. 작성자와 댓글을 함께 가져오는 조회인데, 왜 이렇게 묶어 가져와야 하는지는 5.4에서 밝힙니다. 4장에서 `findByIdJoinUser`를 이름만 쓰고 넘어갔던 것과 같습니다. 여기서는 "작성자와 댓글을 한 번에 가져오는 조회"로만 씁니다. 가져온 글과 `sessionUserId`를 `DetailDTO`에 넘기면, 앞의 `isOwner`와 댓글 목록이 채워집니다.

마지막으로 컨트롤러가 로그인 유저를 꺼내 서비스로 넘깁니다. 상세는 공개라 로그인하지 않아도 볼 수 있으므로, `board/BoardController.java`의 상세 메서드는 `request.getAttribute("sessionUser")`로 유저를 꺼내되 비로그인이면 `null`을 넘깁니다. 로그인한 요청이면 그 아이디를, 비로그인이면 `null`을 `게시글상세`에 전달합니다. `null`이 넘어가도 `checkOwner`가 `false`를 돌려주므로, 로그인하지 않은 사람에게는 모든 `isOwner`가 `false`로 나갑니다.
...
댓글까지 나가는 것을 확인하고, 오픈이는 서버 로그를 무심코 내려다봤습니다. 4장에서 켜 둔 `show-sql` 설정 덕에, JPA가 실제로 내보내는 SQL이 콘솔에 그대로 찍힙니다.

*상세 한 번 불렀는데. select가 왜 이렇게 여러 개 나가지?*

글 하나를 조회했을 뿐인데 콘솔에는 select 문이 여러 줄 지나갔습니다. 이 낌새를 제대로 파헤치려면 조회를 하나씩 뜯어보는 테스트가 필요합니다. 마침 선배가 지나가다 화면을 봤습니다.

**선배**: "글 목록 불러올 때 작성자 이름도 같이 보여 주죠? 그거 쿼리 몇 개 나가는지 세어 봤어요?"

세어 본 적이 없었습니다. 오픈이는 리포지토리 테스트를 열어, 조회할 때 SQL이 실제로 몇 번 나가는지 확인해 보기로 했습니다.

그 전에 짚어야 할 것이 지연 로딩입니다. 5.1에서 댓글에 붙인 `FetchType.LAZY`가 그것입니다. 4장에서는 `Board.user`를 `EAGER`로 두었는데, 이번 챕터에서 이 값을 `LAZY`로 바꿉니다. `board/Board.java`의 작성자 필드를 아래처럼 고칩니다.
...
즉시 로딩(EAGER)은 글을 조회할 때 작성자를 무조건 함께 가져옵니다. 지연 로딩(LAZY)은 글만 먼저 가져오고, 작성자는 실제로 꺼내 쓰는 순간까지 미룹니다. 미뤄 둔 그 자리에는 진짜 작성자 대신 가짜 대리인이 들어앉는데, 이것을 프록시(Proxy)라고 합니다. 프록시는 겉으로는 `User`처럼 보이지만 속은 비어 있습니다. `getUsername()`처럼 실제 값이 필요한 순간, 프록시가 그제서야 데이터베이스에 작성자를 조회하는 쿼리를 내보냅니다.

말로는 잘 와닿지 않으니 테스트로 확인합니다. 먼저 조심할 것이 하나 있습니다. 지금 볼 테스트는 콘솔에 `board.getId()`만 찍습니다. 즉시 로딩이든 지연 로딩이든 이 출력은 똑같습니다. 둘의 차이는 콘솔이 아니라 `show-sql`이 찍어 주는 SQL 로그에서 드러납니다.

`test/board/BoardRepositoryTest.java`에 `findByIdEager_test`와 `findByIdLazy_test`를 준비합니다. 두 테스트의 코드는 글자 그대로 같습니다. 글 하나를 `findById`로 조회해 `board.getId()`만 출력하고, 작성자는 건드리지 않습니다. 다른 것은 `Board.user`의 어노테이션 상태뿐입니다. `findByIdEager_test`를 실행할 때는 방금 `LAZY`로 바꾼 작성자 필드를 잠시 `EAGER`로 되돌립니다. 이때 SQL 로그를 보면 `findById` 한 줄에 board와 user를 함께 조회하는 select가 나갑니다. 확인했으면 어노테이션을 다시 `LAZY`로 되돌리고 `findByIdLazy_test`를 실행합니다. 이번에는 같은 `findById`인데 board만 조회하는 select 한 줄만 나가고, 작성자는 프록시로 남아 로그에 나타나지 않습니다.
...
```java [실습 5] test/board/BoardRepositoryTest.java. 프록시가 추가 쿼리를 부른다
    @Test
...
`findById`로 글을 가져올 때는 board select 하나만 나갑니다. 그런데 마지막 줄에서 `board.getUser().getUsername()`으로 작성자 이름을 꺼내는 순간, SQL 로그에 user를 조회하는 select가 한 줄 더 찍힙니다. 미뤄 두었던 프록시가 그제서야 진짜 작성자를 가져오는 것입니다. 글 하나를 조회하는 데 select가 두 번 나갑니다.

글 하나면 두 번이라 별것 아닌 것 같습니다. 선배가 던진 질문이 여기서 살아납니다. 글 목록을 불러 글마다 작성자 이름을 화면에 보여 준다고 하겠습니다. 먼저 목록을 조회하는 select가 한 번 나갑니다. 그다음 목록의 각 글에서 작성자 이름을 꺼낼 때마다, 방금 본 그 추가 select가 글 수만큼 반복됩니다. 글이 N개면 작성자 조회가 N번 더 붙어, 전부 1 + N개가 됩니다. 목록 하나를 불렀을 뿐인데 쿼리가 글 수에 비례해 불어나는 이 현상을 N+1 문제라고 합니다.
...
오른쪽 해법이 눈에 들어옵니다. 작성자를 나중에 따로 가져오지 말고, 글을 조회할 때 아예 함께 묶어 가져오면 쿼리는 한 번으로 끝납니다. 이 방법이 다음 절의 fetch join입니다.

## 5.4 fetch join으로 해결

작성자를 글과 함께 묶어 가져오는 조회는 이미 있습니다. 4장에서 `findByIdJoinUser`를 만들어 게시글 상세와 수정, 삭제에 썼습니다. 그때는 "작성자를 함께 가져오는 조회"라는 이름으로만 쓰고, 왜 그렇게 묶어야 하는지는 미뤄 두었습니다. 그 메서드의 JPQL은 `select b from Board b join fetch b.user where b.id = :id`였습니다.

핵심은 `join fetch b.user`입니다. 지연 로딩에서는 글을 먼저 가져오고 작성자를 프록시로 미뤄 두지만, `join fetch`는 글을 조회하는 그 select에 작성자 조회를 끼워 넣어 한 번에 가져옵니다. 그래서 이 메서드로 가져온 글은 작성자가 이미 채워져 있어, `getUser().getUsername()`을 불러도 추가 쿼리가 나가지 않습니다. 5.3에서 본 두 번째 select가 사라지는 것입니다. 4장에서 별생각 없이 이름만 보고 쓰던 그 메서드가, 사실은 이 N+1을 막는 장치였습니다.
...
이번에는 `join fetch`가 두 개입니다. `join fetch b.user`로 작성자를, `left join fetch b.replies`로 댓글 목록을 함께 가져옵니다. 댓글 쪽에 `left`를 붙인 것은, 댓글이 하나도 없는 글도 조회에서 빠지지 않게 하기 위해서입니다. 이 메서드 하나로 글과 작성자와 댓글이 전부 채워진 채 돌아오므로, 5.2의 상세 응답에서 `board.getUser()`나 `board.getReplies()`를 꺼내도 추가 쿼리가 나가지 않습니다.

이 조회가 실제로 쿼리를 한 번에 끝내는지 테스트로 확인합니다. `test/board/BoardRepositoryTest.java`에 `findByIdJoinUserAndReply_test`를 작성합니다.

```java [실습 7] test/board/BoardRepositoryTest.java. fetch join 조회
    @Test
    public void findByIdJoinUserAndReply_test() {
        ...
        System.out.println("Reply : " + board.getReplies().get(1).getComment()); // 댓글도 추가 쿼리 없음
    }
```

작성자와 댓글을 모두 꺼냈는데, SQL 로그에는 조회할 때 나간 select 하나뿐입니다. 5.3의 `findByIdLazyLoading_test`에서는 작성자를 꺼내는 순간 select가 한 번 더 나갔지만, 여기서는 조회 한 번에 작성자와 댓글이 담겨 오기 때문입니다. 작성자만 함께 가져오는 `findByIdJoinUser_test`도 마찬가지로, `getUsername()`에서 추가 쿼리가 나가지 않습니다.
...
| 비유 | 용어 | 정식 정의 |
|------|------|-----------|
| 글에 매달린 포스트잇 | 양방향 연관관계 | 두 엔티티가 서로를 참조하는 관계. 게시글은 댓글 목록을, 댓글은 게시글을 가짐 |
| 포스트잇에 적힌 소속 표시 | 연관관계 주인 | 외래 키를 실제로 들고 있는 쪽. 반대쪽은 `mappedBy`로 주인을 가리킴 |
| 글을 떼면 함께 떨어지는 포스트잇 | 영속성 전이(cascade) | 부모 엔티티의 작업을 자식에 전이. `CascadeType.REMOVE`는 삭제를 함께 적용 |
| 자리를 대신 지키는 대리인 | 프록시 | 지연 로딩에서 실제 값을 꺼낼 때까지 자리를 지키는 가짜 객체. 꺼내는 순간 조회 쿼리를 냄 |
| 목록 하나에 쿼리가 불어남 | N+1 문제 | 목록 조회 1개에 각 항목의 연관 조회 N개가 붙어 1+N개가 되는 문제 |
| 묶어서 한 번에 가져오기 | fetch join | 조회할 때 연관 엔티티를 함께 가져와 추가 쿼리를 없애는 것 |
