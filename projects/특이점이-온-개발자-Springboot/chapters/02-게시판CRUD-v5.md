...
스프링이 내 코드를 어떻게 찾아 실행하는지 이해한 오픈이는, 이제 본격적으로 스프링을 다뤄 보기로 했습니다. 첫 목표는 가장 기본이 되는 게시판입니다. 새 글을 쓰고, 목록을 조회하고, 글을 수정하거나 삭제하는 핵심 기능을 직접 구현해 볼 계획입니다.
...
그래서 서버는 화면 대신 데이터만, 그것도 어떤 기기든 해석할 수 있는 형식으로 넘기는 방향으로 바뀌었습니다. 여기에 쓰이는 형식이 JSON입니다.
...
주소는 어떤 자원인지를 가리키고, 자원에 무엇을 할지는 HTTP 메서드로 나타냅니다. 게시판에서 쓰는 메서드는 네 가지입니다.
