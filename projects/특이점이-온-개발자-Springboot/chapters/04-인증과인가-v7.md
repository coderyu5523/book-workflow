...
서버가 토큰을 검증하는 방법은 토큰의 종류에 따라 다릅니다. 아무 정보도 담지 않은 토큰은 그 값만으로는 진짜인지 알 수 없어, 서버가 발급한 토큰을 저장해 두고 대조해야 합니다. 반면 사용자 정보를 JSON으로 담는 **JWT(JSON Web Token)** 는 저장해 둔 것과 대조할 필요 없이, 토큰 안의 정보만 확인하면 됩니다.
...
앞으로 만들 서버는 무상태로 동작하도록 JWT를 사용합니다. JWT는 점(.)으로 구분된 헤더, 페이로드, 서명 세 부분으로 이루어집니다.
