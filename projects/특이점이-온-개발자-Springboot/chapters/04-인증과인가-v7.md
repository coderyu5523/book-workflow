...
지금까지 우리가 만든 게시판의 게시글에는 작성자 정보가 없습니다. 회원 기능을 추가하면 서버는 요청을 보낸 사람이 누구인지 확인해야 합니다.

### 4.1.1 토큰 인증

로그인에 성공하면 서버는 사용자를 식별할 문자열인 **토큰(Token)** 을 발급합니다.
...
서버가 토큰을 검증하는 방법은 토큰의 종류에 따라 다릅니다. 아무 정보도 담지 않은 토큰은 그 값만으로는 진짜인지 알 수 없어, 서버가 발급한 토큰을 저장해 두고 대조해야 합니다. 반면 사용자 정보를 JSON으로 담는 **JWT(JSON Web Token)** 는 저장해 둔 것과 맞춰 볼 필요 없이, 토큰의 서명만 확인하면 됩니다.

그래서 서버는 누가 로그인해 있는지 보관할 필요가 없습니다. 요청마다 담겨 온 토큰만 확인하면 되는 이 방식을 **무상태(Stateless)** 라고 합니다.

### 4.1.2 무상태와 서버 확장

상태를 갖지 않으면 서버의 부하가 줄고, 서버 간 독립성이 생겨 서버를 쉽게 확장할 수 있습니다.

로드밸런서를 두고 클라이언트의 요청을 여러 대의 서버로 나누어 처리하는 경우를 보겠습니다. 서버가 로그인 정보를 보관하는 **상태 유지(Stateful)** 에서는 인증 정보가 특정 서버에 종속되기 때문에, 그 서버만 요청을 정상적으로 처리할 수 있습니다. 로드밸런서가 부하 분산을 위해 다른 서버로 요청을 전달하면, 그 서버에는 인증 정보가 없으므로 요청이 실패합니다.
...
서버가 검증하는 JWT는 점(.)을 기준으로 세 부분으로 나뉘며, 각각 **헤더(Header)**, **페이로드(Payload)**, **서명(Signature)** 을 의미합니다.
...
**해시(Hash)** 는 원본 데이터를 고정된 길이의 값으로 변환하는 단방향 계산입니다. 입력값이 같으면 항상 동일한 결과가 나오지만, 단 한 글자만 달라져도 전혀 다른 결과가 출력됩니다. 또한 결과값만으로는 원본 데이터를 복구할 수 없습니다.
...
토큰을 확인하면 서버는 요청의 주인을 알 수 있습니다. 서버의 검증은 회사 건물의 출입 절차와 같습니다. 회사에 출근하면 가장 먼저 게이트에서 사원증을 찍어 이 회사 직원임을 증명합니다. 서버에서도 요청을 보낸 사람이 누구인지 확인하는 과정을 **인증(Authentication)** 이라고 합니다.

사원증을 찍고 로비를 통과해도 회사의 모든 곳에 갈 수는 없습니다. 자료실처럼 허가받은 사람만 들어갈 수 있는 공간이 따로 있습니다. 서버에서도 인증된 사용자가 요청한 동작을 할 자격이 있는지 따지는 과정을 **인가(Authorization)** 라고 합니다.
