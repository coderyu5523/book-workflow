...
<svg viewBox="0 0 1000 350" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="챕터 4 한눈에 보기. 1단계에서 클라이언트가 로그인하면 서버가 JWT를 발급한다. 2단계에서 클라이언트는 요청마다 JWT를 헤더에 담아 보내고, 필터가 JWT를 확인해 회원 정보를 담고, 컨트롤러는 담긴 회원 정보를 서비스로 전달하고, 서비스가 로그인 여부와 소유자를 확인한다.">
  ...
  <text x="390" y="300" text-anchor="middle" font-size="11" fill="#3730a3">JWT 확인 · 회원 정보 담기</text>
  ...
  <text x="630" y="300" text-anchor="middle" font-size="11" fill="#6b7280">회원 정보를 서비스로 전달</text>
...
- **무상태**로 서버를 늘려도 로그인이 유지되는 이유를 이해할 수 있습니다
- **JWT**를 발급하고 검증할 수 있습니다
:::
...
이번 챕터는 JWT를 다루므로 토큰을 생성하고 검증하는 라이브러리가 필요합니다.

`build.gradle`에 `com.auth0:java-jwt` 의존성을 추가해 두었습니다.
