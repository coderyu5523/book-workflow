...
지금까지 우리가 만든 게시판의 게시글에는 작성자 정보가 없습니다. 회원 기능을 추가하면 서버는 사용자를 검증해야 합니다.
...
인가를 하려면 서버는 요청을 보낸 사람이 누구인지부터 알아야 합니다. 그래서 로그인에 성공하면 서버가 신원을 증명하는 문자열을 발급합니다. 이 문자열을 **토큰(Token)** 이라고 합니다. 토큰에는 여러 종류가 있는데, 앞으로 만들 서버는 사용자 정보를 JSON으로 담는 **JWT(JSON Web Token)** 를 사용합니다.
...
서버가 토큰을 검증하는 방법은 토큰의 종류에 따라 다릅니다. 단순한 문자열로 된 토큰은 그 값만으로는 진짜인지 알 수 없어, 서버가 발급한 토큰을 저장해 두고 대조해야 합니다. 반면 **JWT**는 사용자 정보를 토큰 안에 담고 있어, 서버는 아무것도 저장하지 않고 토큰 안의 정보만 확인하면 됩니다.

HTTP는 본래 앞 요청을 기억하지 않는 프로토콜입니다. 서버 역시 누가 로그인해 있는지 기억하지 않고, 요청마다 담겨 온 토큰만 확인합니다. 서버가 클라이언트의 상태를 갖지 않는 이 방식을 **무상태(Stateless)** 라고 합니다.
