...
회원 정보를 응답하는 공통 DTO로, 비밀번호를 제외한 컬럼을 담습니다. 이를 위해 `user/UserResponse.java`를 열어 아래와 같이 작성합니다.
...
글쓰기와 마찬가지로 먼저 로그인 여부를 확인합니다. 이어서 작성자 일치 여부를 검증하는데, 작성자 정보는 게시글을 조회해야 알 수 있으므로 이 과정 역시 서비스에서 처리합니다. 이를 반영하여 `board/BoardService.java`의 `게시글수정()`을 아래와 같이 변경합니다.
