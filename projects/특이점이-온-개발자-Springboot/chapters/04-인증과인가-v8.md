...
응답 결과로 서버에서 발행한 JWT를 확인할 수 있습니다.
...
이제 **API 요청 시 JWT를 함께 보내야 합니다**. 인증 탭에서 인증 유형을 Bearer로 선택 후 응답받은 JWT를 추가합니다.

![](../assets/CH4/setup/01_hoppscotch-bearer.png)
...
![](../assets/CH4/terminal/08_board-detail-other-user.png)
*그림 4-17. 다른 회원 게시글의 상세 조회 응답*

`isOwner`와 같은 값을 활용해 수정, 삭제와 같은 버튼의 활성 유무를 제어할 수 있습니다.
...
로그인에서 받은 JWT로 게시글 추가 API를 호출합니다.
...
2번 게시물은 작성자가 다르기 때문에 인가에서 실패하여 403 에러가 발생합니다.
...
![](../assets/CH4/terminal/09_board-update-403.png)
*그림 4-20. 다른 회원 게시글의 수정 응답*
...
ssar이 작성한 1번 게시글의 삭제 API를 호출합니다.
