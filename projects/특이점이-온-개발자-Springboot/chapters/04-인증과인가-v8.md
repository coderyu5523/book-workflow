...
회원 테이블을 만들었으니 이제 게시글 테이블에 두 테이블을 연결할 외래 키가 필요합니다.
...
회원 테이블이 추가되었으므로 시작할 때 넣어 둘 데이터도 함께 변경합니다.
...
`core/util/JwtProvider.java`를 열어 아래와 같이 작성합니다.
...
`core/filter/JwtAuthenticationFilter.java`를 열어 아래와 같이 작성합니다.
