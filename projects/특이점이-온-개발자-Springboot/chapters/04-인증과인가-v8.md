...
<svg viewBox="0 0 1000 350" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="챕터 4 한눈에 보기. 1단계에서 클라이언트가 로그인하면 서버가 JWT를 발급한다. 2단계에서 클라이언트는 요청마다 JWT를 헤더에 담아 보내고, 필터가 JWT를 확인해 회원 정보를 담고, 컨트롤러는 담긴 회원 정보를 서비스로 전달하고, 서비스가 로그인 여부와 소유자를 확인한다.">
  ...
  <text x="830" y="278" text-anchor="middle" font-size="13" font-weight="800" fill="#c2410c">서비스 (인가)</text>
...
| **core/filter/JwtAuthenticationFilter.java** | 요청마다 회원 정보를 담아 둡니다 |

### 4.5.1 JwtUtil

토큰 생성과 검증에는 `build.gradle`에 추가해 둔 `com.auth0:java-jwt` 라이브러리를 사용합니다.
...
토큰이 없거나 서명이 일치하지 않으면 null을 반환합니다. 이어서 만들 필터는 이 반환값만으로 회원 정보를 담을지 판단합니다.
...
회원 정보를 응답하는 공통 DTO로, 비밀번호를 제외한 컬럼을 담습니다.
...
컨트롤러는 서비스가 반환한 토큰 문자열을 응답 바디에 담아 전달합니다.
...
글쓰기와 마찬가지로 먼저 로그인 여부를 확인합니다. 이어서 작성자 일치 여부를 검증하는데, 작성자 정보는 게시글을 조회해야 알 수 있으므로 이 과정 역시 서비스에서 처리합니다.
...
- **인증은 "너 누구야"를 밝히고, 인가는 "그걸 할 권한이 있어"를 따집니다.** 이번 챕터에서 인증은 토큰을 확인해 회원 정보를 담는 필터가, 인가는 로그인 여부와 작성자를 확인하는 서비스가 담당합니다. 컨트롤러는 담긴 회원 정보를 꺼내 서비스로 넘깁니다.
