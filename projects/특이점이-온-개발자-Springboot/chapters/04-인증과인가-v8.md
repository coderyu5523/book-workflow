...
게시글 기능이 완성되었으니, 다음은 회원을 추가해 **인증 게시판**을 만듭니다.

오픈이는 회원 테이블을 만들고 회원가입과 로그인 기능을 구상했습니다. 하지만 로그인이 성공한 이후가 문제였습니다. 게시글의 작성자와 요청을 보낸 사용자가 같은지를 어떻게 구분할지 떠오르지 않았습니다.
...
- **인증과 인가**의 차이를 이해할 수 있습니다
- **JWT**를 발급하고 서명을 검증해, 서버가 로그인 상태를 저장하지 않고도 사용자를 식별할 수 있습니다
- **1:N 관계**로 회원과 게시글이 연결되는 구조를 이해할 수 있습니다
:::
...
│   │   └── util/
│   │       ├── JwtProvider.java                 # [참고] 토큰 추출·유저 복원
...
└── resources/db/data.sql                        # [참고] 회원·게시글 더미 데이터
```
...
지금까지 만든 게시판에는 게시글을 작성한 회원 정보가 없습니다. 회원 기능을 추가하면 서버는 요청을 보낸 사용자가 누구인지 검증해야 합니다.

이를 회사의 보안 시스템에 비유해 보겠습니다. 우리가 출근할 때 게이트에 사원증을 찍어 이 회사의 직원임을 증명하듯, 서버에서 요청을 보낸 사람의 신원을 확인하는 과정을 **인증(Authentication)** 이라고 합니다.

하지만 로비를 통과했다고 해서 회사의 모든 곳에 들어갈 수 있는 것은 아닙니다. 자료실처럼 권한을 부여받은 사람만 출입할 수 있는 곳이 따로 있습니다. 서버도 마찬가지입니다. 인증을 마친 사용자라도 해당 동작을 수행할 자격이 있는지 확인해야 하며, 이 과정을 **인가(Authorization)** 라고 합니다.
...
이처럼 안전한 시스템을 구축하려면 서버가 요청마다 인증과 인가를 검증해야 합니다. 서버가 수많은 사용자의 요청을 매번 검증하기 위해 사용하는 대표적인 기술이 **토큰(Token)** 기반 인증입니다.

## 4.2 JWT

### 4.2.1 토큰 인증

토큰을 활용한 인증은 두 단계로 이뤄집니다. 먼저 클라이언트가 서버에 인증을 요청해 성공하면, 서버는 토큰을 발급하여 클라이언트에 전달하고, 클라이언트는 이 토큰을 자신의 기기에 저장합니다.
...
서버가 토큰을 검증하는 방법은 토큰의 종류에 따라 다릅니다. 일반적으로 토큰은 무작위로 만들어진 문자열이라, 서버는 값을 검증하기 위해 발급한 토큰을 저장하고 있어야 합니다. 반면 **JWT(JSON Web Token)** 는 내부에 사용자 정보를 보관하고 있습니다. 그래서 서버에 저장하지 않고 JWT 검증만으로도 사용자를 식별할 수 있습니다.

이처럼 서버가 사용자의 상태나 정보를 따로 기억해 두지 않고, 매번 요청이 들어올 때마다 새롭게 검증하는 방식을 **무상태(Stateless)** 라고 부릅니다.

### 4.2.2 상태 유지와 무상태

그렇다면 서버가 사용자 정보를 기억하는 것과 그렇지 않은 것은 실제 시스템 환경에서 어떤 차이를 만들까요?

로드밸런서를 두고 클라이언트의 요청을 여러 대의 서버로 나누어 처리한다고 가정해 보겠습니다. 서버가 로그인 정보를 보관하는 **상태 유지(Stateful)** 에서는 인증 정보가 특정 서버에 종속되기 때문에, 그 서버만 요청을 정상적으로 처리할 수 있습니다. 로드밸런서가 부하 분산을 위해 다른 서버로 요청을 전달하면, 그 서버에는 인증 정보가 없으므로 요청이 실패합니다.
...
반면 **무상태(Stateless)** 방식에서는 서버가 클라이언트로부터 전달받은 JWT만 검증하면 되므로, 로드밸런서가 어떤 서버로 요청을 전달하더라도 동일하게 인증이 처리되어 정상적인 응답을 받을 수 있습니다.
...
서버는 로그인한 사용자마다 세션이라는 전용 보관 공간을 만들고, 이 공간을 가리키는 번호를 클라이언트에 전달합니다. 웹 브라우저는 이 번호를 **쿠키(Cookie)** 에 저장해 두었다가 서버에 요청을 보낼 때마다 함께 전송하고, 서버는 이 번호를 대조해 사용자를 확인합니다. 단, 이 구조는 동시 접속자가 늘어날수록 서버가 유지해야 할 세션도 함께 늘어나 서버에 큰 부담을 준다는 단점이 있습니다.
:::

### 4.2.3 JWT의 구조

이번에는 JWT의 구조를 알아보겠습니다. JWT는 점(.)을 기준으로 세 부분으로 나뉘며, 각각 **헤더(Header)**, **페이로드(Payload)**, **서명(Signature)** 을 의미합니다.
...
**해시(Hash)** 는 원본 데이터를 고정된 길이의 값으로 변환하는 단방향 함수입니다. 입력값이 같으면 항상 동일한 결과가 나오지만, 단 한 글자만 달라져도 전혀 다른 결과가 출력됩니다. 또한 결과값만으로는 원본 데이터를 복구할 수 없습니다.

이러한 특성 덕분에 해시는 데이터가 중간에 위변조되지 않았음을 증명하는 데 널리 쓰입니다.
:::

서버는 요청에 실려 온 JWT를 점(.)으로 나눠 헤더와 페이로드를 꺼낸 뒤, 비밀키와 함께 같은 방식으로 해시합니다. 그 결과를 JWT의 서명과 대조해 위변조 여부를 검증합니다.

Base64로 인코딩된 페이로드는 디코딩하면 누구나 읽을 수 있으므로 비밀번호 같은 민감한 정보는 넣지 않습니다. 다만 비밀키가 없으면 서명을 다시 만들 수 없어 내용을 위조할 수 없습니다.

## 4.3 테이블 관계

인증 기능을 구현하려면 회원 정보를 저장할 테이블이 필요합니다. 테이블을 생성하기 전에, 새로운 회원 데이터가 기존 게시글 테이블과 어떤 관계를 맺는지 알아보겠습니다.

H2와 같은 **관계형 데이터베이스(RDBMS)** 는 데이터를 행(Row)과 열(Column)로 구성된 테이블에 저장하며, 테이블을 서로 연결하는 **관계**를 통해 전체 데이터를 관리합니다. 테이블과 테이블 사이의 관계는 **1:1**, **1:N**, **N:M(다대다)** 세 가지입니다.

### 4.3.1 1:N 관계

먼저 1:N은 **한 테이블의 데이터 하나에 다른 테이블의 데이터 여러 개가 연결되는 구조**입니다. 재고가 단 하나뿐인 제품을 판매한다고 가정해 보겠습니다. 고객 한 명은 **여러 제품을 살 수 있지만**, 특정 제품은 **오직 한 명의 고객에게만 판매됩니다**.
...
<svg viewBox="0 0 800 250" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="고객 테이블과 제품 테이블의 1대 N 관계. 고객 ssar 한 행에 사과, 수박, 딸기 세 제품이 대응한다.">
  ...
  <text x="610" y="72" text-anchor="middle" font-size="13" fill="#3730a3">1. 사과, 1개</text>
  <text x="610" y="118" text-anchor="middle" font-size="13" fill="#3730a3">2. 수박, 1개</text>
  <text x="610" y="164" text-anchor="middle" font-size="13" fill="#3730a3">3. 딸기, 1개</text>
  <text x="610" y="210" text-anchor="middle" font-size="13" fill="#334155">4. 포도, 1개</text>
...
전체 구조로 보면 **소비자(1)** 와 **제품(N)** 은 **1:N 관계**에 해당합니다. 관계형 데이터베이스에서 이 두 테이블의 연관 관계를 맺어주려면 **외래 키(Foreign Key)** 가 필요합니다. 외래 키는 다른 테이블의 기본 키와 연결되는 특수한 컬럼으로, 1:N 관계에서는 **N(다수)** 쪽인 제품 테이블에 외래 키를 정의하여 두 테이블을 연결합니다.

:::tip
**제1정규형**

관계형 데이터베이스는 테이블을 체계적으로 구성하기 위해 **정규화(Normalization)** 과정을 거칩니다. 가장 기본이 되는 **제1정규형(First Normal Form)** 은 '하나의 컬럼에 하나의 값만 존재해야 한다'는 규칙입니다. 아래 그림처럼 고객 테이블의 제품 컬럼에 여러 값을 한꺼번에 넣으면 이 원칙에 어긋납니다. 따라서 제품을 별도 테이블로 나누고 외래 키로 연결해 올바른 구조를 만들어야 합니다.
:::

<div class="svg-figure">
<svg viewBox="0 0 800 160" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="한 컬럼에 값이 여러 개 들어간 형태. 고객 ssar 행의 제품 칸에 사과, 수박, 딸기 세 값이 한꺼번에 들어가 있다.">
...
*그림 4-9. 한 컬럼에 값이 여러 개 들어간 형태*

### 4.3.2 N:M 관계

이번에는 일반적인 상황처럼 제품 테이블에 넉넉한 재고 수량이 있는 경우입니다. 고객 한 명은 여러 제품을 구매할 수 있습니다.
...
반대로 제품 역시 재고가 있는 만큼 여러 고객이 구매할 수 있습니다.
...
이처럼 양쪽 모두가 여럿과 연결되는 구조를 N:M 관계라고 합니다. 관계형 데이터베이스에서는 N:M 관계를 직접 연결할 수 없으므로, 사이에 **중간 테이블**을 만들어 두 개의 1:N 관계로 나눠야 합니다. **구매**라는 중간 테이블을 두면, **고객 한 명이 여러 번 구매하는 구조(1:N)** 와 **제품 하나가 여러 고객에게 구매되는 구조(1:N)** 로 어떤 고객이 무엇을 구매했는지, 어떤 제품을 누가 구매했는지 조회할 수 있습니다.
...
회원 테이블과 게시글 테이블의 관계로 살펴보면, 회원 한 명은 여러 개의 게시글을 작성할 수 있고, 하나의 게시글은 작성자가 한 명입니다. 따라서 **회원 테이블과 게시글 테이블은 1:N 관계**이고, **N**에 해당하는 게시글에 외래 키를 추가해야 합니다.

## 4.4 테이블 만들기

### 4.4.1 회원 엔티티

이제 회원 테이블 추가를 위한 엔티티를 만들어 보겠습니다.

`user/User.java`를 열어 아래와 같이 작성합니다.
...
`username`에 붙은 `@Column(unique = true)`는 **user_tb**를 생성할 때 `username` 컬럼에 유니크 제약을 추가합니다.

### 4.4.2 게시글에 작성자 연결

회원 테이블을 만들었으니 이제 게시글 테이블에 회원 테이블을 연결할 외래 키가 필요합니다. 스프링에서는 외래 키 추가를 위해 **@ManyToOne** 어노테이션을 사용합니다.
...
데이터베이스에서는 참조할 테이블의 기본 키를 외래 키로 사용합니다. 하지만 JPA에서는 **User** 같은 엔티티를 필드로 선언할 수 있습니다. 이렇게 하면 연관된 회원 데이터를 함께 조회할 수 있다는 장점이 있습니다.

:::tip
**외래 키 컬럼명 생성 규칙**

JPA가 만드는 외래 키 컬럼명은 **게시글 엔티티에 추가한 필드명(user)** 과 **회원 테이블의 기본 키(id)** 를 언더스코어로 합친 **user_id**입니다. 만약 컬럼명을 명시하고 싶다면 JoinColumn 어노테이션을 사용해 원하는 컬럼명을 정할 수도 있습니다.
:::

### 4.4.3 더미 데이터

더미 데이터에 회원을 추가하고, 게시글에는 외래 키 값을 추가합니다.
...
로그인한 사용자를 식별하려면 토큰을 발급하고, 이후 요청에 포함된 토큰을 검증하는 로직이 필요합니다. `core` 폴더에 있는 인증 관련 클래스는 다음과 같습니다.

| 파일 | 역할 |
|------|------|
| **core/util/JwtUtil.java** | 회원 정보로 토큰을 생성하고, 토큰을 검증해 회원 정보를 복원합니다 |
| **core/util/JwtProvider.java** | 요청 헤더에서 토큰을 꺼내고, 유효하면 회원 정보를 반환합니다 |
| **core/filter/JwtAuthenticationFilter.java** | 토큰이 유효하면 회원 정보를 요청에 보관합니다 |

### 4.5.1 JwtUtil

**JwtUtil**은 로그인한 회원 정보로 JWT를 만들어 서버만 아는 비밀키로 서명합니다. 이후 요청이 오면 같은 비밀키로 서명을 검증해 회원 정보를 복원합니다.
...
JWT를 만들고 검증하는 기능은 **com.auth0:java-jwt** 라이브러리가 제공하며, `build.gradle`에 추가되어 있습니다.

### 4.5.2 JwtProvider

**JwtProvider**는 요청 헤더에서 토큰을 꺼낸 뒤 **JwtUtil**로 검증하고 회원 정보를 반환합니다.
...
**JwtAuthenticationFilter**는 이 지점에서 **JwtProvider**로 회원 정보를 얻어 요청 객체에 저장합니다.
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        // 2. 유효하면 회원 정보를 요청에 담아 둔다 (막지는 않는다)
        if (token != null) {
...
회원가입은 클라이언트가 입력한 회원 정보를 데이터베이스에 저장한 뒤, 저장된 회원 정보를 응답합니다.
...
```java [실습 3] user/UserRepository.java. 유저네임으로 조회
...
`user/UserRequest.java`를 열어 아래와 같이 작성합니다.

```java [실습 4] user/UserRequest.java. 회원가입 요청 DTO
...
이어서 회원 정보를 응답할 DTO를 만들어 보겠습니다. 비밀번호를 제외한 컬럼만 필드로 선언합니다.

`user/UserResponse.java`를 열어 아래와 같이 작성합니다.

```java [실습 5] user/UserResponse.java. 회원가입 응답 DTO
...
```java [실습 6] user/UserService.java. 회원가입
@RequiredArgsConstructor
@Service
...
비밀번호를 입력받은 그대로 저장하면 데이터베이스에 접근할 수 있는 사람은 누구나 다른 사람의 비밀번호를 알게 됩니다. 그래서 실무에서는 되돌릴 수 없는 해시로 변환해 저장합니다. 이 책은 인증 흐름에 집중하기 위해 비밀번호를 평문으로 다룹니다.
:::
...
```java [실습 7] user/UserController.java. 회원가입 엔드포인트
@RequiredArgsConstructor
@RestController
...
로그인은 유저네임과 비밀번호를 받아 회원 일치 여부를 확인하고, 일치하면 JWT를 응답합니다.

### 4.7.1 요청 DTO

로그인 요청을 담을 DTO를 `user/UserRequest.java`에 아래와 같이 추가합니다.

```java [실습 8] user/UserRequest.java. 로그인 요청 DTO
...
전달받은 유저네임과 비밀번호로 회원을 조회해 비밀번호 일치 여부를 확인합니다.
...
유저네임과 비밀번호가 일치하면 `JwtUtil.create()`로 생성한 JWT를 응답합니다.
...
클라이언트는 이 JWT를 저장해 두었다가 이후 요청마다 함께 전송합니다.

더미 데이터의 ssar 계정으로 로그인 API를 호출합니다.
...
게시글 상세에는 게시글 정보와 작성자 정보가 함께 응답되어야 합니다. 그러려면 게시글 엔티티와 회원 엔티티를 한 번의 조회로 가져와야 합니다.
...
일반 SQL과 달리 JPQL의 기본 조인은 FROM 절 엔티티만 반환합니다. 게시글의 `user_id`는 **board_tb**의 컬럼이라 함께 조회되지만, 그 값으로 **User** 엔티티를 채우려면 회원을 조회하는 쿼리가 한 번 더 실행됩니다.

이때 사용하는 것이 **FETCH** 키워드입니다. 이 키워드를 붙이면 JOIN 절 엔티티까지 한 번에 조회해, FROM 절 엔티티의 연관 필드를 실제 데이터로 채워서 반환합니다.
...
게시글 상세에서 사용할 데이터를 조회해 보겠습니다.

`board/BoardRepository.java`를 열어 **@Query**로 작성자를 함께 조회하는 메서드를 아래와 같이 작성합니다.

```java [실습 11] board/BoardRepository.java. 작성자를 함께 가져오는 조회
...
`findBy` 규칙으로는 조인 방식까지 지정할 수 없으므로, JPQL을 직접 적을 때는 **@Query**를 사용합니다.

### 4.8.3 응답 DTO

`board/BoardResponse.java`를 열어 **DetailDTO**에 작성자의 아이디와 유저네임, 그리고 본인 여부를 알려 주는 값을 아래와 같이 추가합니다.

```java [실습 12] board/BoardResponse.java. 상세 응답에 작성자와 본인 여부 추가
...
`isOwner`는 요청자가 이 게시글의 작성자인지를 참과 거짓으로 나타냅니다. 로그인하지 않았으면 `loginUser`가 null이므로 false가 되고, 로그인했다면 요청자의 아이디와 작성자의 아이디를 비교해 일치할 때 true가 됩니다. 클라이언트는 이 값으로 수정·삭제 버튼을 보여줄지 결정하고, 서버는 요청이 들어올 때 401과 403으로 다시 검증합니다.

### 4.8.4 서비스

작성자를 함께 조회하도록 `board/BoardService.java`의 `게시글상세()`를 아래와 같이 변경합니다.

```java [실습 13] board/BoardService.java. 상세 조회에 fetch 조인과 본인 여부
...
컨트롤러는 필터를 거쳐 요청 객체에 전달된 회원 정보를 꺼내 사용합니다.

`board/BoardController.java`의 상세 메서드를 아래와 같이 변경합니다.

```java [실습 14] board/BoardController.java. 상세 요청에서 로그인 유저 꺼내기
    @GetMapping("/{boardId}")
...
```java [실습 15] board/BoardRequest.java. 작성자를 연결하는 toEntity
...
요청 DTO를 엔티티로 변환해 저장하도록 `board/BoardService.java`의 `게시글쓰기()`를 아래와 같이 변경합니다.

```java [실습 16] board/BoardService.java. 로그인을 확인하고 작성자를 담아 저장
    @Transactional
...
`board/BoardController.java`의 쓰기 메서드를 아래와 같이 변경합니다.

```java [실습 17] board/BoardController.java. 쓰기 요청에서 로그인 유저 꺼내기
    @PostMapping
...
게시글 수정은 현재 사용자와 게시글 작성자가 일치하는지 검증 후, 게시글을 수정해야 합니다. 수정은 동일하게 더티 체킹으로 진행합니다.

`board/BoardService.java`의 `게시글수정()`을 아래와 같이 변경합니다.

```java [실습 18] board/BoardService.java. 로그인 확인과 소유자 검증
    @Transactional
...
```java [실습 19] board/BoardController.java. 수정 요청에서 로그인 유저 꺼내기
    @PutMapping("/{boardId}")
...
ssar이 작성한 1번 게시글의 수정 API를 호출합니다.
...
<!-- [CAPTURE NEEDED: 06_board-update
  ...
  desc: 토큰을 Authorization 헤더에 담고 보낸 PUT /api/boards/1 요청에 대한 200 응답. { "status": 200, "msg": "성공", "body": { "boardId": 1, "title": "수정한 제목", "content": "수정한 내용" } } 형태로, 본인이 쓴 게시글이 수정된 화면. Hoppscotch 또는 브라우저 응답.
] -->
...
```java [실습 20] board/BoardService.java. 삭제의 로그인 확인과 소유자 검증
    @Transactional
...
```java [실습 21] board/BoardController.java. 삭제 요청에서 로그인 유저 꺼내기
...
ssar이 작성한 2번 게시글의 삭제 API를 호출합니다.
...
<!-- [CAPTURE NEEDED: 07_board-delete
  ...
  desc: 토큰을 Authorization 헤더에 담고 보낸 DELETE /api/boards/2 요청에 대한 200 응답. { "status": 200, "msg": "성공", "body": null } 형태로, 돌려줄 데이터가 없어 body가 null인 화면. Hoppscotch 또는 브라우저 응답.
] -->
![](../assets/CH4/terminal/07_board-delete.png)
*그림 4-18. 게시글 삭제 응답*

이전까지 실습한 게시판은 누가 요청을 보냈는지 알 수 없는 익명 상태였습니다. 하지만 이번 챕터에서는 회원 기능을 추가하여 요청자를 식별하는 구조로 수정하였습니다. 서버가 세션처럼 로그인 상태를 따로 기억할 필요 없이, 요청에 포함된 JWT만으로 회원을 식별할 수 있습니다. 그리고 JWT를 활용해 사용자의 인증과 인가도 함께 구현했습니다.

다음 챕터에서는 게시글에 필요한 댓글과, 이를 위해 회원, 게시글, 댓글 테이블간의 관계를 알아보겠습니다.

:::remember
**이것만은 기억하자**

- 인증은 사용자의 신원을 확인하는 과정이고, 인가는 특정 작업을 수행할 권한이 있는지 검증하는 과정입니다.
- 클라이언트는 요청마다 JWT를 함께 전달합니다. JWT에는 회원 정보가 포함되어 있어, 서버는 사용자 정보를 서버에 저장하지 않아도 JWT를 통해 사용자를 식별할 수 있습니다.
:::