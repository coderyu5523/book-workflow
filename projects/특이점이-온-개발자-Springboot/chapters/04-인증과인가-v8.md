...
지금까지 우리가 만든 게시판의 게시글에는 작성한 회원 정보가 없습니다. 회원 기능을 추가하면 서버는 요청을 보낸 사람이 누구인지에 대한 검증이 필요합니다.

서버의 검증은 회사 건물의 출입 절차와 같습니다. 회사에 출근하면 가장 먼저 게이트에서 사원증을 찍어 이 회사 직원임을 증명합니다. 서버에서도 요청을 보낸 사람이 누구인지 확인하는 과정을 **인증(Authentication)** 이라고 합니다.
...
서버가 사용자의 요청을 검증하려면 로그인에 성공한 사용자에게 인증 수단을 제공해야 합니다. 이때 서버가 발급하는 인증 수단이 바로 **토큰(Token)** 입니다.
