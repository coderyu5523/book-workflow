...
앞 챕터에서 오픈이는 스프링의 정체를 알았습니다. 등록한 적 없는 메서드가 저절로 실행되던 것은 마법이 아니라 리플렉션 위의 규칙이었습니다. 원리를 알고 나니, 이제 이 뼈대 위에 진짜 무언가를 얹어 보고 싶어졌습니다.

마침 이번 팀 과제는 게시판입니다. 화면은 같은 팀 동료가 맡기로 했고, 오픈이는 뒤에서 데이터를 저장하고 꺼내 주는 부분을 맡았습니다. 글을 쓰면 저장되고, 목록을 부르면 지금까지 쓴 글이 돌아오는 서버입니다.

*화면 없이 데이터만 주고받는 서버라니, 어디서부터 손대야 하지.*

옆자리 선배에게 물었습니다.

**선배**: "요즘 서버는 화면을 직접 안 그려요. 데이터만 딱 정리해서 넘겨주면, 화면은 프론트가 알아서 그리거든요. 그 데이터 주고받는 규칙이 REST예요."

화면을 그리지 않고 데이터만 넘긴다는 말이 새로웠습니다. 오픈이가 만들 것은 눈에 보이는 페이지가 아니라, 글이라는 자원을 다루는 서버였습니다.

오픈이가 이번 챕터에서 만들 게시판의 큰 그림을 먼저 펼쳐 보겠습니다. 지금은 이름과 흐름만 눈에 담아 두면 됩니다.
...
선배의 말처럼, 오픈이가 만들 서버도 화면을 돌려주지 않습니다. 대신 글 데이터를 JSON이라는 형식으로 주고받습니다. JSON(JavaScript Object Notation)은 데이터를 키와 값의 쌍으로 표현하는, 사람이 읽기 쉬운 텍스트 형식입니다. 게시글 하나는 이런 모습입니다.
...
같은 주소를 두고도, 무엇을 하고 싶은지는 HTTP 메서드로 구분합니다. 조회할 때와 저장할 때 주소는 같지만 메서드가 다릅니다.
...
게시판에 필요한 동작이 이 네 가지에 그대로 들어맞습니다. 새로 쓰고(Create), 읽고(Read), 고치고(Update), 지우는(Delete) 이 네 가지를 앞 글자만 따 CRUD라고 부릅니다. 이번 챕터에서 만들 것이 바로 게시글의 CRUD API입니다.
...
이 파일들이 앞의 한눈에 보기 그림에서 본 세 층을 이룹니다. 각 클래스가 맡는 역할을 먼저 정리하면 다음과 같습니다.
...
파일을 채우기 전에, 스프링이 데이터베이스를 어떻게 다루는지부터 짚어야 합니다. 오픈이가 저장할 글은 결국 데이터베이스의 한 줄로 남습니다. 그런데 오픈이가 자바로 다루는 것은 `Board`라는 객체입니다. 이 둘 사이에는 생각보다 큰 간격이 있습니다.

## 2.3 두 세계의 불일치와 하이버네이트

소프트웨어를 만들 때 개발자는 서로 다른 철학을 가진 두 세계를 동시에 마주합니다. 데이터의 정확성을 지키려는 관계형 데이터베이스와, 현실의 복잡함을 표현하려는 객체지향 언어입니다.

데이터베이스의 목표는 데이터를 안전하게 보관하고 빠르게 꺼내는 것입니다. 그래서 모든 데이터를 행과 열로 이루어진 평평한 표에 값으로만 담습니다. 다른 표를 가리키려면 외래 키(Foreign Key)라는 값을 공유해 조인으로 이어 붙일 뿐, 표 안에 다른 표를 통째로 넣지는 못합니다. 상속 같은 개념도 없습니다.

반면 자바는 현실의 사물을 객체로 다룹니다. 객체는 값뿐 아니라 행동(메서드)을 가지고, 다른 객체를 필드로 품어 참조(Reference)로 서로를 가리킵니다. 상속과 다형성으로 표현하는 방법도 풍부합니다. 같은 관계를 두고도 한쪽은 값으로 잇고, 다른 쪽은 참조로 품습니다.
...
이 차이가 어디서 부딪히는지, 선배가 점심을 시키다 말고 예를 들었습니다.

**선배**: "자바에선 햄버거 세트 하나 들면 그 안에 햄버거, 콜라, 감자가 다 딸려 오잖아요. 그런데 DB는 세트라는 상자 안에 음식을 못 넣어요. 햄버거 표, 콜라 표, 감자 표를 따로 만들어 두고 '우린 같은 세트야'라고 값으로 연결만 해 둬요."

자바에서는 `햄버거세트` 객체 하나가 `햄버거`, `콜라`, `감자` 객체를 품습니다(has-a). 세트 하나만 들고 다니면 그 안의 내용물에 바로 닿습니다. 데이터베이스는 그렇게 못 합니다. 햄버거, 콜라, 감자를 각각 다른 테이블로 두고, 외래 키로 "같은 세트"라고 연결만 해 둡니다. 하나로 품는 세계와 값으로 나눠 잇는 세계, 이 어긋남이 두 세계의 근본적인 불일치입니다.
...
ORM이 없던 시절에는 개발자가 이 간극을 손으로 메웠습니다. 자바 개발자인 동시에 수동 번역가였던 셈입니다. 테이블이 하나 늘 때마다 비슷한 SQL과 매핑 코드를 수십 줄씩 반복해 썼고, 조회 결과를 객체에 한 칸씩 옮겨 담느라 정작 중요한 로직 짤 시간이 줄었습니다. 컬럼 하나만 바뀌어도 관련된 SQL을 전부 찾아 고쳐야 했습니다.

**선배**: "그 번역을 대신 해 주는 게 JPA예요. 자바 객체로 말하면 SQL로 바꿔서 DB에 전하고, DB가 준 결과는 다시 객체로 돌려주죠. 개발자는 객체만 다루면 돼요."

이렇게 객체와 관계형 테이블 사이를 자동으로 이어 주는 기술을 ORM(Object-Relational Mapping)이라고 합니다. JPA(Java Persistence API)는 자바가 정한 ORM 표준이고, 하이버네이트(Hibernate)는 그 표준을 실제로 구현한 대표적인 도구입니다. 개발자는 복잡한 조인과 외래 키를 일일이 신경 쓰지 않고 객체를 다루듯 코드를 짜고, 두 세계를 오가는 번역은 하이버네이트에 맡깁니다. 덕분에 객체는 객체답게 참조와 상속을 쓰고, DB는 DB답게 정규화된 표를 유지합니다.
...
`@Entity`는 이 클래스가 데이터베이스 테이블과 연결되는 엔티티임을 표시하고, `@Table(name = "board_tb")`로 연결할 테이블 이름을 지정합니다. 각 필드는 테이블의 칸이 됩니다. `@Id`는 각 행을 구분하는 기본 키를 뜻하고, `@GeneratedValue(strategy = GenerationType.IDENTITY)`는 그 값을 데이터베이스가 자동으로 1씩 늘려 채우게 합니다. `@CreationTimestamp`는 글이 처음 저장될 때 현재 시간을 `createdAt`에 자동으로 넣어 줍니다.
...
클래스 위의 `@Data` 하나가 남았습니다. 오픈이는 코드를 작성하다 잠시 멈췄습니다.

*게터랑 세터를 하나도 안 썼는데. `board.getTitle()`은 대체 어디서 나온 거지?*

`Board` 어디에도 `getTitle()`이나 `setTitle()` 메서드가 보이지 않습니다. 그런데 뒤에서 이 메서드들을 태연히 호출하게 됩니다. 답은 `@Data`에 있습니다. `@Data`는 롬복(Lombok)이 제공하는 어노테이션으로, 컴파일 시점에 게터, 세터, `toString` 같은 반복 코드를 대신 만들어 줍니다. 눈에 보이는 소스에는 없지만, 컴파일된 결과물에는 이 메서드들이 들어 있습니다. 개발자가 손으로 쓰던 지루한 코드를 롬복이 표식 하나로 대신 채워 줍니다.
...
인메모리라 껐다 켜면 데이터가 사라집니다. 그래서 시작할 때 `data.sql`의 내용을 넣어 초기 상태를 맞춥니다. `data.sql`에는 실습에서 바로 확인할 수 있도록 `title1`, `title2` 두 건이 미리 들어 있습니다. `show-sql`을 켜 두면 JPA가 실제로 어떤 SQL을 내보내는지 콘솔에서 볼 수 있어, 앞서 이야기한 번역이 눈에 드러납니다.

## 2.5 리포지토리와 EntityManager

엔티티와 데이터베이스가 준비됐으니, 이제 둘 사이에서 실제로 저장하고 꺼내는 부분을 만듭니다. 이 역할을 맡는 것이 리포지토리(Repository)로, 데이터의 조회·저장·수정·삭제를 담당하는 계층입니다. 리포지토리가 데이터베이스 작업에 쓰는 도구가 `EntityManager`입니다. `EntityManager`는 JPA에서 데이터베이스 작업을 총괄하는 핵심 객체로, 엔티티의 저장·조회·삭제를 담당합니다.
...
`@Repository`는 이 클래스를 데이터 접근을 담당하는 빈으로 등록합니다. `@RequiredArgsConstructor`는 롬복이 `final` 필드를 받는 생성자를 대신 만들어 주는 어노테이션입니다. 스프링은 그 생성자를 통해 `EntityManager`를 넣어 줍니다. 앞 챕터에서 개념만 짚었던 의존성 주입이 여기서 실제로 일어납니다. 스프링 환경에서는 `EntityManager`가 이미 빈으로 등록되어 있어, 직접 만들지 않고 주입받아 씁니다.

네 메서드가 각각 `EntityManager`의 명령을 부릅니다. `em.find`는 기본 키로 한 건을 찾아오고, `em.persist`는 새 엔티티를 저장 대상으로 등록하며, `em.remove`는 삭제 대상으로 표시합니다.

`findAll`만 조금 다릅니다. 여러 건을 조회할 때는 조건에 맞는 대상을 질의해야 하는데, 이때 쓰는 것이 JPQL(Java Persistence Query Language)입니다. JPQL은 테이블이 아니라 엔티티와 필드를 기준으로 작성하는 JPA의 SQL입니다. `select b from Board b`는 SQL과 닮았지만, `board_tb` 테이블이 아니라 `Board` 엔티티를 대상으로 삼는다는 점이 다릅니다. 이렇게 객체를 향해 질의하면, JPA가 이것을 실제 SQL로 번역해 데이터베이스에 보내고 결과를 `List<Board>`로 돌려줍니다.

## 2.6 영속성 컨텍스트

리포지토리 코드를 보면 `em.persist`나 `em.find`를 부를 뿐, 데이터베이스에 직접 SQL을 던지는 부분이 없습니다. JPA가 그 사이에 엔티티를 보관하고 관리하는 공간을 하나 두기 때문입니다. 이 공간을 영속성 컨텍스트(Persistence Context)라고 합니다. `EntityManager`가 관리하는 엔티티 저장소로, `em.persist`나 `em.find`로 엔티티가 등록되거나 조회되는 순간 그 엔티티는 영속 상태가 됩니다.

영속성 컨텍스트가 하는 일은 크게 세 가지입니다. 하나씩 그림으로 따라가 보겠습니다.

첫째는 캐싱입니다. 엔티티를 조회하면 영속성 컨텍스트는 데이터베이스로 곧장 가지 않고 먼저 캐시를 확인합니다. 캐시에 없으면(miss) 그제야 데이터베이스에서 읽어 와 영속 상태로 캐시에 담습니다. 이후 같은 엔티티를 다시 조회하면(hit) 데이터베이스까지 가지 않고 캐시에서 바로 돌려줍니다. 그래서 같은 글을 두 번 조회해도 SQL은 한 번만 실행됩니다.
...
둘째는 쓰기 지연입니다. `em.persist`로 새 엔티티를 저장하라고 해도 곧바로 데이터베이스로 보내지 않습니다. INSERT 문을 영속성 컨텍스트 안의 버퍼에 모아 두었다가, `flush` 시점에 한꺼번에 내보냅니다.
...
셋째는 더티체킹입니다. `em.find`로 엔티티를 조회하면 영속성 컨텍스트는 그때의 상태를 스냅샷으로 찍어 둡니다. 이후 엔티티의 값을 바꾸면 스냅샷과 지금 상태가 달라지고, 영속성 컨텍스트는 그 차이를 감지해 UPDATE 문을 버퍼에 만들어 둡니다. 이 UPDATE 문 역시 `flush` 시점에 데이터베이스로 나갑니다. 개발자가 저장 명령을 따로 내리지 않아도, 값을 바꾸기만 하면 변경이 감지되는 것입니다.
...
여기서 `flush`가 자주 등장합니다. 실제 코드에서는 개발자가 `flush`를 직접 부르지 않아도 됩니다. 뒤에서 서비스에 붙일 `@Transactional`이 트랜잭션을 정상적으로 끝낼 때 자동으로 `flush`를 호출하기 때문입니다. 그 순간 버퍼에 모인 INSERT, UPDATE가 데이터베이스로 나갑니다. 이 셋 중에서도 더티체킹은 곧 게시글 수정에서 다시 만나게 됩니다. 저장 명령을 한 줄도 쓰지 않았는데 수정이 반영되는 장면이 바로 이 원리입니다.

## 2.7 서비스와 컨트롤러

리포지토리가 데이터를 다루는 통로라면, 그 통로를 언제 어떻게 쓸지 결정하는 것은 서비스의 몫입니다. 그리고 바깥의 요청을 받아 서비스로 넘기는 입구가 컨트롤러입니다. 도입부의 그림 2-1에서 본 세 층이 바로 이 구조입니다. 바깥 요청을 받는 컨트롤러(Controller), 실제 처리 흐름을 맡는 서비스(Service), 데이터베이스를 다루는 리포지토리(Repository)로 나뉘며, 이 구조를 3계층 아키텍처라고 합니다.
...
먼저 서비스입니다. 서비스는 리포지토리를 불러 목록, 상세, 작성, 삭제를 처리합니다. 수정은 더티체킹과 함께 다음 절에서 따로 다루므로, 여기서는 네 가지만 채웁니다.
...
`@Service`는 이 클래스를 서비스 빈으로 등록하고, 생성자를 통해 `BoardRepository`가 주입됩니다. 작성과 삭제에는 스프링이 제공하는 `@Transactional`이 붙어 있습니다. 트랜잭션(Transaction)은 여러 작업을 하나로 묶어 전부 성공하거나 전부 없던 일로 되돌리는 단위입니다. 데이터를 바꾸는 작업은 트랜잭션 안에서 이뤄져야 하므로, 쓰기가 일어나는 메서드에만 이 표식을 붙입니다. 반면 목록과 상세는 읽기만 하므로 붙이지 않습니다. 앞 절에서 이야기한 자동 `flush`가 바로 이 `@Transactional`이 끝나는 순간에 일어납니다.

`게시글추가`를 보면, `new Board()`로 만든 엔티티는 아직 영속성 컨텍스트가 모르는 상태입니다. `boardRepository.save(board)`를 호출하는 순간 영속 상태가 됩니다. 메서드 이름을 한글로 둔 것은 어색해 보일 수 있지만, 어떤 동작을 하는지 그대로 읽히도록 예제에서 택한 방식입니다.
...
`@RestController`는 이 클래스의 반환값을 JSON으로 내보내는 컨트롤러임을 뜻합니다. `@RequestMapping("/api/boards")`로 이 컨트롤러가 담당할 주소의 공통 앞부분을 정합니다. 각 메서드의 `@GetMapping`, `@PostMapping`, `@DeleteMapping`이 2.1에서 본 HTTP 메서드에 그대로 대응합니다.

주소에서 값을 꺼내는 두 어노테이션이 있습니다. `@PathVariable`은 `/api/boards/1`처럼 주소에 박힌 값을 꺼내 파라미터로 받고, `@RequestBody`는 요청 본문으로 들어온 JSON을 자바 객체로 바꿔 받습니다. 여기서 받는 `SaveDTO`는 요청 데이터를 담는 그릇으로, `board/BoardRequest.java`에 정의합니다.
...
CRUD 중 아직 수정이 남았습니다. 오픈이는 앞의 흐름대로라면 값을 바꾼 뒤 `boardRepository.save()`를 불러야 저장된다고 생각했습니다. 그래서 수정 메서드를 짜다가, 완성 코드에서 이상한 점을 발견했습니다.

*조회해서 값만 바꿨는데. 저장하는 코드가 없잖아?*
...
정말로 저장하는 호출이 없습니다. `findById`로 가져온 `board`의 값을 바꾸고 그대로 반환할 뿐입니다. 그런데도 수정은 데이터베이스에 반영됩니다. 2.6에서 살펴본 더티체킹이 여기서 동작하기 때문입니다.

`findById`로 가져온 순간 `board`는 영속 상태가 되고, 영속성 컨텍스트가 그때의 상태를 스냅샷으로 찍어 둡니다. `setTitle`, `setContent`로 값을 바꾸면 영속 엔티티는 달라지지만 스냅샷은 그대로입니다. 메서드에 붙은 `@Transactional`이 끝나 트랜잭션이 닫히면서 자동으로 `flush`가 호출되는 순간, JPA는 스냅샷과 지금 상태를 비교해 달라진 부분을 UPDATE 문으로 내보냅니다. 개발자가 저장 명령을 내리지 않아도, 영속 엔티티의 변화를 JPA가 스스로 감지해 반영하는 것입니다.

이 수정 메서드를 컨트롤러의 PUT 엔드포인트에 이어 줍니다. `board/BoardController.java`에 아래 메서드를 추가합니다.

```java [실습 6] board/BoardController.java. 수정 엔드포인트
    // 수정 (PUT /api/boards/1)
    @PutMapping("/{boardId}")
...
지금까지는 애플리케이션 전체를 띄워 API로 결과를 봤습니다. 하지만 리포지토리 하나가 제대로 도는지 확인하려고 매번 서버를 띄우고 요청을 보내는 것은 번거롭습니다. 오픈이가 고민하자 선배가 커피를 내리다 말고 예를 들었습니다.

**선배**: "커피 머신에 콩을 넣었는데 커피가 안 나와요. 그럼 콩 가는 부분이 문제인지, 커피 뽑는 부분이 문제인지 어떻게 알아요? 둘을 한 번에 돌리면 몰라요. 하나씩 따로 돌려 봐야 알죠."

커피 머신에는 두 기능이 있습니다. 커피콩을 1cm로 갈아 주는 분쇄기와, 그 콩으로 커피를 뽑는 추출기입니다. 둘을 한 통에 넣고 한 번에 돌리면 커피가 안 나올 때 어느 쪽이 문제인지 알기 어렵습니다. 각 기능을 따로 떼어 독립된 환경에서 검증하면, 분쇄기에서 문제가 나면 분쇄기만 고치면 됩니다. 이렇게 가장 작은 단위를 외부 의존 없이 따로 검증하는 것이 단위 테스트(Unit Test)이고, 검증된 기능들을 결합해 전체 흐름을 확인하는 것이 통합 테스트입니다.
...
```java [실습 7] BoardRepositoryTest.java. 더티체킹 검증
...
`@Import(BoardRepository.class)`로 검증할 리포지토리를 테스트에 올리고, `@DataJpaTest`가 JPA 관련 부분만 띄웁니다. `@Autowired`는 스프링이 해당 타입에 맞는 객체를 찾아 자동으로 넣어 주는 어노테이션입니다. `data.sql`의 두 건이 들어와 있으므로, `id=2`인 글을 기준으로 확인합니다.

`update_test`가 더티체킹을 눈으로 보게 해 줍니다. `findById`로 글을 가져와 제목과 내용을 바꾼 뒤 `save()`는 부르지 않습니다. 대신 `em.flush()`로 지금까지의 변경을 데이터베이스에 밀어 넣고, `em.clear()`로 영속성 컨텍스트를 강제로 비웁니다. 컨텍스트가 비었으니 이어지는 `findById`는 2.6에서 본 캐시가 아니라 데이터베이스에서 새로 읽어 옵니다. 그렇게 다시 읽은 글의 제목이 `title-update`로 바뀌어 있다면, 저장 호출 없이도 변경이 반영됐다는 증거입니다.
...
테스트가 초록색으로 통과했습니다. 오픈이는 화면을 열어 목록을 부르는 대신, 테스트 하나로 리포지토리가 제대로 도는지 확인하는 방법을 손에 넣었습니다.
...
| 데이터베이스 앞의 저장소 | 영속성 컨텍스트 | EntityManager가 엔티티를 보관·관리하는 공간. 캐싱, 쓰기 지연, 더티체킹이 여기서 일어남 |
| 스냅샷과 지금을 비교 | 더티체킹 | 영속 엔티티의 변경을 감지해 flush 시점에 UPDATE로 반영하는 것 |
