...
앞 챕터 끝에서 동료가 물었습니다. 없는 번호로 상세를 부르면 어떻게 되느냐는 것이었습니다. 오픈이는 답을 짐작만 했을 뿐, 눈으로 확인한 적은 없었습니다.

*직접 넣어 보면 되잖아.*

오픈이는 서버를 다시 실행하고, 아직 없는 999번 글을 불러 봤습니다. 목록에는 글이 두 건뿐이니 999번은 존재하지 않습니다. 잠시 뒤 화면에 돌아온 것은 성공이 아니었습니다. 지금까지 맞춰 온 `status`, `msg`, `body` 형식도 없이, 스프링이 기본으로 뱉는 낯선 오류가 상태 코드 500과 함께 떠 있었습니다.

*역시 터졌구나.*

이번엔 잘 되는 1번 글을 불러 응답을 다시 들여다봤습니다. 목록도 상세도 잘 나옵니다. 그런데 응답 안에는 `createdAt` 같은 내부 기록 필드가 그대로 실려 나가고 있었습니다. 지금은 사소해 보입니다. 하지만 뒤 챕터에서 엔티티에 작성자나 비밀번호가 붙으면, 응답을 감싸지 않는 한 그 값까지 바깥으로 새어 나가게 됩니다.

두 개의 구멍이 한꺼번에 드러났습니다. 없는 글에 서버가 터지는 것과, 내부 엔티티가 응답으로 새어 나가는 것입니다. 이번 챕터는 이 둘을 어떻게 막는지 먼저 한 장으로 펼쳐 보겠습니다.

<div class="svg-figure">
<svg viewBox="0 0 1000 360" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="챕터 3 한눈에 보기. 구멍 1은 없는 글을 조회하면 500으로 서버가 터지던 것을, Optional과 예외와 전역 처리로 404 응답으로 바꾼다. 구멍 2는 엔티티를 통째로 내보내 createdAt까지 새어 나가던 것을, 응답 DTO로 감싸 boardId·title·content 세 값만 내보낸다.">
  <defs>
  ...
  <text x="265" y="112" text-anchor="middle" font-size="13" font-weight="700" fill="#c2410c">500 서버가 터짐</text>
  <text x="265" y="132" text-anchor="middle" font-size="11" fill="#6b7280">null이 흘러가다 예외</text>
...
먼저 없는 글에 터지는 첫 번째 구멍입니다. 오픈이가 부른 999번 요청과 그 응답은 이랬습니다.
...
```json
{
  "timestamp": "2026-07-21T09:12:44.019+00:00",
  "status": 500,
...
500은 서버 안에서 미처 처리하지 못한 오류가 났다는 뜻입니다. 원인은 조회 코드에 있습니다. 앞 챕터의 `findById`는 없는 글을 찾으면 글 대신 `null`을 돌려줍니다. 그 `null`이 응답을 만드는 과정으로 그대로 흘러 들어가 문제를 일으키고, 스프링은 이를 500 오류로 감싸 내보냅니다. 게다가 이 응답에는 앞 챕터에서 맞춰 온 `status`, `msg`, `body` 형식조차 없습니다. 스프링이 기본으로 뱉는 낯선 모양이 그대로 나갔습니다.

두 번째 구멍은 오히려 잘 되는 요청에 숨어 있습니다. 정상으로 존재하는 1번 글을 부르면 응답은 이렇게 돌아옵니다.
...
이 둘을 이번 챕터에서 차례로 막습니다. 먼저 엔티티가 새어 나가는 쪽을 응답 그릇으로 막고, 그다음 서버가 터지는 쪽을 예외로 처리합니다. 이번 챕터에서 새로 만들거나 손보는 클래스는 다음과 같습니다.
...
새어 나가는 구멍부터 막습니다. 오픈이는 응답에 엔티티를 통째로 실어 보내는 게 마음에 걸려 선배에게 물었습니다.

**오픈이**: "글을 응답할 때 엔티티를 그대로 내보내고 있는데, 이래도 되나요?"<br>
**선배**: "주방 냄비를 그대로 손님상에 올리지는 않잖아요. 접시에 덜어서 내가요."

오픈이는 냄비와 접시를 떠올렸습니다. 엔티티는 주방 안에서 쓰는 냄비입니다. 데이터베이스와 맞닿아 온갖 정보를 담고, 값을 넣고 빼며 요리하는 그릇입니다. 손님상에 나가는 것은 접시라야 합니다. 접시에는 손님이 볼 것만 덜어 담습니다.
...
그릇을 목록용 `DTO`와 상세용 `DetailDTO`로 나눠 둔 데는 이유가 있습니다. 지금은 두 그릇의 내용이 똑같습니다. 그런데 상세 화면은 앞으로 댓글 같은 정보가 더 붙어 목록과 달라집니다. 미리 나눠 두면 그때 상세 그릇만 손보면 되고, 목록은 건드릴 필요가 없습니다.
...
그릇을 만들었으니 이제 서비스가 엔티티를 접시에 담아 넘기게 고칩니다. 그런데 그 전에 첫 번째 구멍, 없는 글에 서버가 터지는 문제를 같이 다뤄야 합니다. 두 문제가 모두 조회 메서드에서 만나기 때문입니다.

터지는 뿌리는 `findById`가 없는 글에 `null`을 돌려주는 데 있습니다. `null`은 아무 표시가 없습니다. 받는 쪽은 그것이 진짜 글인지 빈손인지 열어 보기 전엔 모릅니다. 자바에는 이 애매함을 걷어내는 문법이 있습니다. 값이 있을 수도, 없을 수도 있는 것을 상자에 담아 표현하는 `Optional`입니다.
...
한 메서드 안에서 두 가지가 바뀌었습니다. 하나는 조회입니다. `findById`가 돌려준 상자에 `orElseThrow`를 걸어, 없는 글이면 `null`을 넘기는 대신 `Exception404`를 던집니다. 이제 999번을 부르면 `null`이 흘러가다 엉뚱한 곳에서 터지는 게 아니라, 조회하는 순간 "없다"는 신호가 분명하게 올라옵니다. 다른 하나는 반환입니다. 조회한 엔티티를 `DetailDTO`에 담아 돌려줍니다. 냄비 대신 접시가 나가는 것입니다.
...
여기서 던지는 `Exception404`는 우리가 만들 커스텀 예외입니다. 아직 없는 클래스라 잠시 뒤 3.4에서 정의합니다. 트랜잭션도 짚고 넘어가면, 값을 바꾸는 `게시글추가`, `게시글수정`, `게시글삭제`에만 `@Transactional`을 붙입니다. 조회만 하는 목록과 상세에는 붙이지 않습니다. 2.5에서 정한 대로, 작업대를 여는 트랜잭션 경계는 쓰기가 일어나는 메서드에만 그어 둡니다.
...
없는 글을 부른 상황은 404에 해당합니다. 앞에서 본 500은 서버가 예상하지 못하고 넘어졌다는 뜻이라, 없는 글에는 어울리지 않습니다. 없는 글은 서버의 잘못이 아니라 찾는 자원이 없는 것이니, 404로 응답해야 정확합니다. 이 표의 상태 코드마다 `Exception400`, `Exception401`처럼 짝이 되는 커스텀 예외를 하나씩 두는 것이 흔한 방식입니다. 이번 챕터는 없는 글을 다루니 `Exception404`만 만들고, 401과 403은 다음 챕터에서 인증과 권한을 붙이며 다시 만듭니다.
...
스프링에는 이 일을 한 곳에 몰아주는 장치가 있습니다. 건물 어느 층에서 문제가 터지든 경비실 한 곳으로 신고가 모이는 것과 같습니다. 컨트롤러 어디에서 예외가 던져지든, 그 예외를 가로채 응답으로 바꾸는 창구를 하나 두는 것입니다. 이 창구가 `@RestControllerAdvice`입니다.
...
응답을 만드는 `Resp.fail`은 새로 만든 것이 아닙니다. 앞 챕터에서 `Resp.ok`와 함께 만들어 둔 실패용 정적 메서드입니다. 상태 코드와 메시지를 받아, 성공 응답과 똑같이 `status`, `msg`, `body` 세 칸을 가진 모양으로 돌려줍니다. 덕분에 오류 응답도 성공 응답과 같은 형식을 지킵니다.
...
이제 999번 글을 다시 불러 봅니다.

```bash [터미널] 없는 글 다시 조회
GET http://localhost:8080/api/boards/999
```

500 대신 앞뒤가 맞는 404가 돌아옵니다.
...
*그림 3-4. 없는 글을 조회하면 500이 아니라 상태 코드 404를 담은 JSON이 돌아옵니다*

오픈이는 목록 API를 받아 간 동료를 다시 불렀습니다.

**오픈이**: "없는 번호 넣으면 어떻게 되냐고 물었잖아요. 이제 안 터져요."<br>
**동료**: "아, 3번 불러 볼게요. 없다고 딱 나오네요. 아까는 이상한 화면이 떴는데."

없는 글을 부르면 서버가 넘어지던 곳에, 이제 "찾을 수 없다"는 응답이 형식을 갖춰 돌아옵니다. 응답으로 나가는 것도 엔티티가 아니라 접시에 담긴 값뿐입니다. 두 구멍이 모두 막혔습니다.
