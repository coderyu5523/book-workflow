...
*그림 4-1. 로그인하면 JWT가 발급되고, 이후 요청은 필터에서 신원이 담긴 뒤 서비스에서 자격이 가려집니다*
...
*그림 4-4. 인증은 사원증으로 누구인지를 확인하고, 인가는 사원증에 적힌 부서의 문만 열립니다*

이번 챕터에서 두 확인은 서로 다른 자리가 맡습니다. 신원을 밝히는 인증은 요청을 받는 자리에서 토큰을 확인하는 필터가 담당하고, 자격을 따지는 인가는 요청을 처리하는 서비스가 담당합니다. 컨트롤러는 담긴 신원을 꺼내 서비스로 넘길 뿐입니다. 먼저 신원을 무엇으로 밝힐지부터 정해야 합니다.
...
로그인은 유저네임과 비밀번호를 받아 회원이 맞는지 확인하고, 맞으면 토큰을 응답합니다.
...
유저네임과 비밀번호가 일치하면 `JwtUtil.create()`로 만든 토큰을 응답합니다.
...
게시글 상세에서는 게시글 정보와 작성자가 함께 응답되어야 합니다. 그러려면 데이터베이스에서 게시글 엔티티와 회원 엔티티를 한 번에 가져와야 합니다.

### 4.8.1 JPQL 조인문

이때 쓰는 문법이 JOIN입니다. 조인은 두 테이블의 행을 옆으로 이어 붙여 한 번의 조회로 가져오고, 어떤 행끼리 붙일지는 조건으로 정합니다. SQL은 그 조건을 `on` 절에 적지만, JPQL은 연관관계 필드 이름만 적습니다.
...
`isOwner`는 요청을 보낸 사람이 이 게시글의 작성자인지를 참과 거짓으로 알려 줍니다. 요청자의 아이디와 작성자의 아이디가 같으면 true, 로그인하지 않아 `loginUser`가 null이면 false입니다.
...
조회를 `findByIdJoinUser()`로 바꾸어 작성자를 함께 가져오고, 요청자를 **DetailDTO**에 넘겨 `isOwner`를 채웁니다.
...
게시글 상세는 로그인하지 않아도 볼 수 있어야 하므로 유저가 없다고 해서 막지 않습니다. 대신 `loginUser`에 null을 담아 넘깁니다.
...
그런데 필터는 토큰이 없는 요청도 막지 않고 통과시키므로 넘어온 유저가 null일 수 있습니다. 그 값을 받아 쓰는 자리가 서비스이니, 로그인 여부도 서비스에서 확인합니다.
...
**Exception401**을 발생시키면 챕터 3에서 만든 전역 예외 처리기가 이를 받아 401로 응답합니다.

### 4.9.3 컨트롤러

컨트롤러는 필터가 담아 둔 유저를 꺼내 서비스로 넘깁니다. `board/BoardController.java`의 쓰기 메서드를 아래처럼 고칩니다.
...
꺼낸 유저가 null이어도 그대로 넘깁니다. 막을지 말지는 서비스가 정합니다.
...
쓰기와 마찬가지로 로그인부터 확인합니다. 이어서 작성자를 대조하는데, 어느 게시글의 작성자인지는 게시글을 조회해 봐야 알 수 있으므로 이 대조도 서비스에서 합니다. 작성자가 아니면 **Exception403**을 발생시킵니다. `board/BoardService.java`의 수정 메서드를 아래처럼 고칩니다.
...
두 검사를 모두 통과했을 때만 제목과 내용을 바꿉니다. 저장 코드는 없지만 더티체킹이 변경을 반영합니다.

### 4.10.2 컨트롤러

컨트롤러는 쓰기와 같은 모양으로, 꺼낸 유저를 그대로 서비스에 넘깁니다.
...
삭제 메서드가 로그인한 사람 `loginUser`를 하나 더 받습니다. 로그인을 확인한 뒤 `findByIdJoinUser()`로 게시글을 작성자와 함께 조회해, 게시글의 작성자와 요청자의 아이디가 다르면 작성자가 아니므로 **Exception403**을 발생시킵니다.

### 4.11.2 컨트롤러

컨트롤러도 꺼낸 유저를 그대로 넘깁니다.
...
세 메서드가 모두 필터가 담아 둔 유저를 꺼내 서비스로 넘기는 같은 모양이 되었습니다. 컨트롤러는 요청에서 값을 꺼내 넘기고 응답을 돌려줄 뿐, 막는 일은 서비스가 맡습니다.
...
응답 창에 403이 돌아왔습니다. 게시글을 수정할 권한이 없다는 메시지도 함께였습니다. 로그인하지 않은 쓰기는 401로, 작성자가 아닌 수정은 403으로 갈려 멈췄습니다. 오픈이는 선배를 찾아가 응답 창을 다시 보여줬습니다.

**오픈이**: "이제 로그인하지 않으면 게시글이 안 써지고, 로그인해도 다른 사람이 쓴 게시글은 못 고칩니다."<br>
**선배**: "아무 정보 없이 통과하던 요청이 이제 401과 403으로 갈려서 막히네요."

확인하는 자리가 비어 있던 컨트롤러 앞에는 이제 신원을 밝히는 필터가 놓였고, 서비스는 담긴 신원으로 자격을 따집니다.
...
- **인증은 "너 누구야"를 밝히고, 인가는 "그걸 할 권한이 있어"를 따집니다.** 이번 챕터에서 인증은 토큰을 확인해 신원을 담는 필터가, 인가는 로그인 여부와 작성자를 확인하는 서비스가 맡습니다. 컨트롤러는 담긴 신원을 꺼내 서비스로 넘깁니다.
...
- **상세 응답의 `isOwner`는 화면이 수정·삭제 버튼을 감추는 데 씁니다.** 버튼을 감춰도 요청은 보낼 수 있으므로, 실제로 막는 일은 서비스의 401과 403이 맡습니다.
:::
