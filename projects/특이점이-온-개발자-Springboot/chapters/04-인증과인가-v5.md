...
앞에서는 리포지토리가 직접 만든 클래스라 `@DataJpaTest`의 자동 등록 대상이 아니었습니다. 그래서 테스트에 `@Import(BoardRepository.class)`를 붙여 함께 올렸습니다. 지금은 `JpaRepository`를 상속한 인터페이스라 `@DataJpaTest`가 자동으로 찾아 올립니다.
:::

## 4.5 core 폴더 구현

로그인한 사람을 알아보려면 토큰을 만들고, 요청에 실려 온 토큰을 확인하는 코드가 필요합니다. 이 코드는 게시판이나 회원 어느 한쪽에 속하지 않고 모든 요청에 걸쳐 동작하므로 `core` 아래에 둡니다.

| 파일 | 역할 |
|------|------|
| `core/util/JwtUtil.java` | 토큰을 만들고 검증합니다 |
| `core/util/JwtProvider.java` | 요청 헤더에서 토큰을 꺼내 회원으로 되살립니다 |
| `core/filter/JwtAuthenticationFilter.java` | 요청마다 신원을 확인해 담아 둡니다 |
| `core/config/WebMvcConfig.java` | CORS 설정입니다. `[참고]` 파일이라 미리 채워져 있습니다 |

### 4.5.1 JwtUtil

토큰을 만들고 확인하는 코드에는 상수가 여럿 등장합니다.
...
`core/util/JwtUtil.java`를 열고 아래 코드를 작성합니다. 위 상수 네 개는 클래스 맨 위에 필드로 선언되어 있습니다.

```java [실습 6] core/util/JwtUtil.java. JWT 발급과 검증
    // 1. 유저 정보를 담아 토큰을 만든다
...
`JwtUtil`은 토큰 문자열만 다룹니다. 요청 헤더에 담겨 온 토큰을 꺼내 `JwtUtil`에 넘기는 코드가 하나 더 필요합니다. `core/util/JwtProvider.java`를 열고 아래 코드를 작성합니다.

```java [실습 7] core/util/JwtProvider.java. 토큰 추출과 유저 복원
@Component
...
요청 헤더에서 토큰을 꺼내 유저로 되살리고, 토큰이 없거나 어긋나면 `null`을 돌려줍니다. 다음에 만들 필터는 이 결과만 보고 신원을 판단합니다.

### 4.5.3 인증 필터

앞의 두 클래스를 쓰는 자리가 필요합니다. 인증과 인가로 나눈 둘 중 신원을 밝히는 쪽으로, 요청마다 헤더의 토큰을 확인해 유효하면 신원을 요청에 담아 두는 일입니다.
...
```java [실습 8] core/filter/JwtAuthenticationFilter.java. 인증 필터
...
필터는 신원을 밝혀 두는 데서 멈춥니다. 토큰을 꺼내 유저로 복원하면, `request.setAttribute`로 복원한 유저를 요청에 `loginUser`라는 이름으로 담아 둡니다. 뒤에 오는 컨트롤러가 이 이름으로 "이 요청을 보낸 사람"을 알 수 있습니다. 여기에 담긴 값은 요청 하나가 처리되는 동안만 살아 있고 응답이 나가면 사라집니다. 다음 요청은 토큰을 다시 확인해 신원을 새로 담으므로, 서버가 로그인 상태를 들고 있지 않습니다.

토큰이 없거나 틀려도 필터는 요청을 막지 않고 `filterChain.doFilter`로 그대로 통과시킵니다. 신원이 확인되면 유저가 요청에 담기고, 확인되지 않으면 담기지 않은 채 지나갑니다. 막는 일은 인증이 아니라 인가가 맡습니다.

토큰을 발급하는 로그인은 아직 없습니다. 회원가입과 로그인을 만든 뒤에 이 필터가 담아 둔 신원을 꺼내 씁니다.
...
`JwtUtil`을 쓰는 첫 자리가 로그인입니다. 로그인 요청을 담을 DTO를 `UserRequest`에 추가합니다.

```java [실습 13] user/UserRequest.java. 로그인 요청 DTO
...
```java [실습 14] user/UserService.java. 로그인
    // 비밀번호가 맞으면 JWT를 발급한다
...
```java [실습 15] user/UserController.java. 로그인 엔드포인트
...
```java [실습 16] board/BoardRepository.java. 작성자를 함께 가져오는 조회
...
```java [실습 17] board/BoardResponse.java. 상세 응답에 작성자와 본인 여부 추가
...
`isOwner`는 요청을 보낸 사람이 이 게시글의 작성자인지를 참과 거짓으로 알려 줍니다. `checkOwner`가 요청자의 아이디와 작성자의 아이디를 견줘 같으면 `true`를 돌려주고, 로그인하지 않아 `loginUserId`가 `null`이면 `false`를 돌려줍니다.

`board/BoardService.java`를 열고 `게시글상세`를 아래처럼 고칩니다.

```java [실습 18] board/BoardService.java. 상세 조회에 fetch 조인과 본인 여부
...
```java [실습 19] board/BoardController.java. 상세 요청에서 로그인 유저 꺼내기
    @GetMapping("/{boardId}")
...
게시글 상세는 로그인하지 않아도 볼 수 있어야 하므로 유저가 없다고 해서 막지 않습니다. 대신 `loginUserId`에 `null`을 담아 넘깁니다.

서버를 실행하고 `ssar`로 로그인해 받은 토큰을 헤더에 담아 1번 게시글에 상세 조회 API를 호출합니다.
...
`isOwner`는 화면이 버튼을 감추는 데 쓰는 값입니다. 버튼이 보이지 않아도 API 주소를 알면 요청은 그대로 보낼 수 있으므로, 실제로 걸러 내는 일은 서버가 맡아야 합니다.

거르는 자리는 둘로 나뉩니다. 게시글 목록과 상세는 로그인하지 않은 사람도 볼 수 있어야 하지만, 게시글을 쓰거나 고치거나 지우는 요청은 로그인한 사람만 보낼 수 있어야 합니다. 로그인 여부는 요청을 받는 컨트롤러에서 확인하고, 로그인하지 않은 요청은 `Exception401`을 발생시켜 끊습니다.

로그인했더라도 다른 사람이 쓴 게시글을 고치거나 지우는 요청은 막아야 합니다. 어느 게시글의 작성자인지는 게시글을 조회해 봐야 알 수 있으므로 이 확인은 서비스에서 합니다. 작성자가 아니면 `Exception403`을 발생시켜 끊습니다. 앞에서 만든 전역 예외 처리기가 둘을 받아 401과 403으로 응답합니다.
...
게시글을 새로 쓰는 `게시글추가`는 주인을 확인할 필요가 없습니다. 아직 주인이 없는 새 게시글입니다. 대신 로그인한 사람을 이 게시글의 작성자로 정해서 저장합니다. 그래서 `BoardRequest`의 `toEntity`가 작성자를 받도록 바뀝니다. `board/BoardRequest.java`를 열고 아래처럼 고칩니다.

```java [실습 20] board/BoardRequest.java. 작성자를 연결하는 toEntity
...
```java [실습 21] board/BoardService.java. 작성자를 담아 저장
    @Transactional
...
`board/BoardController.java`의 쓰기 메서드를 아래처럼 고칩니다.

```java [실습 22] board/BoardController.java. 쓰기 요청에서 로그인 확인
    @PostMapping
...
토큰이 없었거나 검증을 통과하지 못했다면 `null`이 나오므로 요청이 여기서 끊깁니다.


## 4.11 게시글 수정

수정은 로그인 확인과 소유자 검증을 모두 거칩니다. 먼저 서비스에서 작성자를 대조합니다. `board/BoardService.java`의 수정 메서드를 아래처럼 고칩니다.

```java [실습 23] board/BoardService.java. 소유자 검증
    @Transactional
...
```java [실습 24] board/BoardController.java. 수정 요청에서 로그인 확인
    @PutMapping("/{boardId}")
...
삭제도 같은 순서입니다. `board/BoardService.java`의 삭제 메서드를 아래처럼 고칩니다.

```java [실습 25] board/BoardService.java. 삭제 소유자 검증
    @Transactional
...
```java [실습 26] board/BoardController.java. 삭제 요청에서 로그인 확인
