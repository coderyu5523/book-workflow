...
스프링이 내 코드를 어떻게 찾아 실행하는지 원리를 깨친 오픈이는 이제 본격적으로 스프링을 활용해 보기로 합니다. 첫 번째 목표는 웹 서비스의 가장 기본이 되는 '게시판' 만들기입니다. 새 글을 작성하고, 전체 목록을 조회하며, 작성한 글을 수정하거나 삭제하는 필수 기능들을 직접 구현해 볼 계획입니다.
...
REST API의 핵심은 **무엇을(주소)**, **어떻게(메서드)** 할 것인가를 명확하게 나누는 데 있습니다. 주소(URI)가 조작할 대상(자원)을 가리킨다면, 그 대상에 어떤 작업을 할지는 HTTP 메서드가 결정합니다.
...
- **헤더**: 바디에 담긴 데이터의 형식(`Content-Type`) 등 서버가 참고할 부가 정보가 담깁니다.
...
- **바디**: 클라이언트가 요청한 실제 데이터가 담깁니다. 게시글 하나를 요청했다면 앞에서 본 JSON이 이 바디에 담겨 옵니다.
...
JPA에는 데이터를 수정하는 메서드없이, 조회해 온 엔티티 객체의 값만 변경하면 데이터베이스에 자동으로 수정이 반영됩니다. 이를 더티 체킹(변경 감지)이라 부르며, 자세한 원리는 영속성 컨텍스트 챕터에서 다루겠습니다.
...
이를 위해 `board/BoardService.java`를 열어 아래와 같이 작성합니다.
...
이어서 요청을 받을 컨트롤러를 작성합니다.

이를 위해 `board/BoardController.java`를 열어 아래와 같이 작성합니다.
...
`new Board()`로 생성한 엔티티는 비영속 상태이고, `boardRepository.save(board)`를 호출하는 순간 영속 상태가 됩니다. 게시글 추가는 데이터를 바꾸는 작업이므로 **@Transactional**을 붙였습니다. 읽기만 하는 목록·상세에는 붙이지 않습니다.
