...
<svg viewBox="0 0 1000 296" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="예외가 발생하면 어디서 받는가. 요청은 클라이언트에서 디스패처 서블릿, 컨트롤러, 서비스를 거쳐 리포지토리로 간다. 서비스에서 예외가 발생하면 컨트롤러를 거쳐 디스패처 서블릿까지 전달된다. 디스패처 서블릿은 예외 처리를 전역 예외 처리기에 맡기고, RestControllerAdvice가 붙은 전역 예외 처리기가 예외를 JSON 응답으로 바꿔 클라이언트에게 돌려준다.">
  ...
  <line x1="135" y1="84" x2="198" y2="84" stroke="#475569" stroke-width="1.7" marker-end="url(#c3f-g)"/>
  <text x="166" y="74" text-anchor="middle" font-size="11" font-weight="600" fill="#475569">요청</text>
  ...
  <line x1="365" y1="84" x2="428" y2="84" stroke="#475569" stroke-width="1.7" marker-end="url(#c3f-g)"/>
  ...
  <line x1="575" y1="84" x2="638" y2="84" stroke="#475569" stroke-width="1.7" marker-end="url(#c3f-g)"/>
  ...
  <line x1="785" y1="84" x2="848" y2="84" stroke="#475569" stroke-width="1.7" marker-end="url(#c3f-g)"/>
  ...
  <line x1="638" y1="106" x2="577" y2="106" stroke="#ff7849" stroke-width="1.9" marker-end="url(#c3f-s)"/>
  <line x1="428" y1="106" x2="367" y2="106" stroke="#ff7849" stroke-width="1.9" marker-end="url(#c3f-s)"/>
  <text x="712" y="142" text-anchor="middle" font-size="12" font-weight="700" fill="#c2410c">예외 발생</text>
...
데이터베이스에 저장되는 대상은 영속성 컨텍스트가 관리하는 엔티티입니다. 따라서 요청으로 받은 DTO를 데이터베이스에 저장하려면 엔티티로 변환하는 과정이 필요합니다. 이를 처리할 수 있도록 먼저 **Board** 클래스에 생성자를 추가합니다.
...
**SaveDTO**는 값만 담은 일반 객체라 영속 상태가 되지 못합니다. 그래서 DTO 안에 엔티티로 바꾸는 `toEntity()`를 추가합니다.
...
응답 DTO는 기능마다 따로 만드는 것이 좋습니다. 다만 이 책에서는 편의를 위해 공통 DTO를 사용하고, 게시글 상세만 별도의 DTO를 만듭니다.
...
이제 앞에서 살펴본 전역 예외 처리기를 직접 만들어 보겠습니다. 클래스에 **@RestControllerAdvice**를 적용하면 그 클래스가 전역 예외 처리기가 됩니다. 이후 예외가 발생하면 디스패처 서블릿이 이 클래스 내부에서 예외 타입에 맞는 처리 메서드를 찾아 자동으로 실행합니다.
...
리포지토리가 반환한 **Optional**에서 값을 꺼낼 때는 `orElseThrow()`를 사용합니다. 이 메서드는 값이 있으면 게시글을 그대로 반환하고, 비어 있으면 인자로 받은 예외를 발생시킵니다.

### 3.5.2 컨트롤러

컨트롤러도 엔티티 대신 DTO로 값을 주고받도록 변경합니다.
...
이제 프로젝트를 실행해 결과를 확인해 보겠습니다. 먼저 1번 게시글을 조회하면 정상적인 데이터를 응답받습니다.
...
챕터 2에서는 엔티티를 직접 요청과 응답에 사용했지만, 이번 챕터에서는 DTO를 활용해 데이터를 전달하는 구조로 개선했습니다. 또한 예외 처리를 추가하여, 처리 과정에서 문제가 발생하더라도 클라이언트가 상태 코드와 실패 사유를 응답받을 수 있습니다.

다음 챕터에서는 회원 엔티티를 추가해 게시글과 작성자를 연결하고, 이를 바탕으로 인증과 인가 기능을 구현해 보겠습니다.

:::remember
**이것만은 기억하자**

- **엔티티를 요청과 응답에 직접 사용하지 않고 DTO로 주고받습니다.** 테이블 구조가 바뀌어도 클라이언트와 주고받는 형식은 영향을 받지 않습니다.
- **요청이 실패하면 상태 코드와 사유를 담아 응답합니다.** 서비스가 예외를 발생시키면 전역 예외 처리기가 한 곳에서 받아 상태 코드에 맞는 JSON으로 바꿉니다.
:::