...
챕터 2의 리포지토리는 **EntityManager**를 통해 데이터베이스에 접근합니다. 조회 결과가 없으면 null을 반환하므로, 서비스에서 매번 null인지 확인하는 예외 처리를 추가해야 하는 번거로움이 있습니다.

이러한 불편함을 해결하기 위해 **JpaRepository**를 도입합니다. 이 인터페이스를 상속하면 스프링이 기본 메서드를 대신 구현해 코드가 간결해집니다. 또한 조회 결과를 **Optional** 타입으로 받으므로, 번거로운 null 체크 없이 데이터가 없을 때의 예외 처리를 작성할 수 있습니다.
