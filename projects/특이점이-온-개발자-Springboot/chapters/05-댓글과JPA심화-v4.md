...
포스트잇 비유에는 하나가 더 있습니다. 게시글이 사라지면 거기 붙은 포스트잇도 같이 사라져야 한다는 것입니다. 게시글을 지울 때 달린 댓글이 데이터베이스에 그대로 남으면 주인 없는 댓글이 됩니다. 그래서 게시글을 지우면 딸린 댓글까지 함께 지워지도록 삭제를 댓글에 전이시킵니다. 이렇게 부모 엔티티의 작업을 자식 엔티티에 함께 적용하는 것을 영속성 전이(cascade)라고 합니다.
...
| ReplyService (신규) | 댓글 저장(대상 게시글 조회)과 삭제(소유자 검증)를 처리합니다. |
