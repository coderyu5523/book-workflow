...
이제 이번 챕터의 하이라이트입니다. 보상 트랜잭션 패턴을 직접 구현합니다. 코드를 읽을 때 `decreasedProducts`와 `deliveryCreated` 두 변수를 추적하면서 읽어보세요. 재고 차감에 성공한 상품만 리스트에 담아두고, catch 블록에서 성공한 것만 골라서 복구합니다.
