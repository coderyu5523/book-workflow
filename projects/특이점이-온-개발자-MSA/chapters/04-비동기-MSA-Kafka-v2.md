...
총 10개의 토픽을 사용합니다. `command`가 붙은 토픽은 orchestrator가 각 서비스에 내리는 명령입니다. `command`가 없는 토픽은 각 서비스가 처리 결과를 orchestrator에 보고하는 이벤트입니다.

| 토픽 | 발행 | 구독 | 목적 |
|---|---|---|---|
| `order-created` | order-service | orchestrator | 새 주문 발생 |
| `order-cancelled` | order-service | orchestrator | 주문 취소 이벤트 |
...
**[참고]** 챕터 3 `build.gradle`에 다음 두 줄을 추가합니다.
...
| 토픽 | DTO 클래스 | 주요 필드 |
|---|---|---|
| `order-created` | OrderCreatedEvent | orderId, userId, address, List\<OrderItem\> |
...
`decreasedProductIds`(Set)는 **어떤** 상품이 성공했는지, `processed`(int)는 **몇 개**가 완료됐는지를 추적합니다. 실패 시에는 성공한 상품만 골라 복구해야 하고, 전체 완료 판단에는 개수 비교가 필요하기 때문에 둘 다 필요합니다.
...
orchestrator는 진행 중인 주문의 상태를 **메모리**에 보관합니다. 어떤 상품의 재고가 이미 감소됐는지 추적하여, 실패 시 처리된 항목만 롤백합니다.

> **WorkflowState**: orchestrator가 주문 한 건의 진행 상태를 추적하기 위해 사용하는 내부 객체입니다. 주문 ID, 상품 목록, 이미 처리된 상품 ID 등을 보관합니다.
...
Kafka 서버 1대에만 메시지를 저장하면, 그 서버가 죽었을 때 메시지가 전부 사라집니다. 그래서 실무에서는 Kafka 서버를 여러 대 띄우고 하나의 클러스터로 묶습니다. 같은 메시지를 여러 Kafka 서버에 복제해두면, 서버 1대가 죽어도 다른 서버에 데이터가 남아있습니다.

**KAFKA_NODE_ID**는 각 Kafka 서버를 식별하는 고유 번호입니다. Kafka 서버가 3대면 각각 1, 2, 3처럼 겹치지 않는 번호를 부여합니다. **CLUSTER_ID**는 이 Kafka 서버들을 하나의 클러스터로 묶는 식별자입니다. 같은 CLUSTER_ID를 가진 Kafka 서버끼리 메시지를 공유하고 복제합니다. 이때 서로를 찾는 주소는 **KAFKA_CONTROLLER_QUORUM_VOTERS**에 지정합니다. 우리 실습에서는 Kafka 서버가 1대이지만, 설정 구조는 동일합니다.

![Kafka 클러스터 식별](images/fig-4-kafka-2.png)
*그림 4-10: 같은 CLUSTER_ID를 가진 Kafka 서버끼리 kafka-service:9093으로 통신하며 메시지를 공유한다*
    ...
    spec:
      containers:
        - name: kafka
          ...
          ports:
            - containerPort: 9092  # 클라이언트 통신용
            - containerPort: 9093  # 컨트롤러 통신용
          env:
            # 1. 클러스터 및 노드 식별
            ...
            - name: KAFKA_LISTENERS                  # Kafka가 수신할 포트를 정의
              value: "PLAINTEXT://0.0.0.0:9092,CONTROLLER://0.0.0.0:9093" # PLAINTEXT라는 이름으로 9092, CONTROLLER라는 이름으로 9093 포트를 연다
            - name: KAFKA_INTER_BROKER_LISTENER_NAME # Kafka끼리 메시지를 복제할 때 사용할 포트 지정 (9092)
              value: "PLAINTEXT"                     # KAFKA_LISTENERS에서 정의한 프로토콜 (PLAINTEXT: 평문 통신)
            - name: KAFKA_CONTROLLER_LISTENER_NAMES  # Kafka끼리 토픽 목록 등을 동기화할 때 사용할 포트 지정 (9093)
              value: "CONTROLLER"                    # KAFKA_LISTENERS에서 정의한 리스너 이름 (내부 관리용 9093 포트)
            ...
            - name: KAFKA_CONTROLLER_QUORUM_VOTERS   # Kafka 서버끼리 서로를 찾기 위한 주소록
              value: "1@kafka-service:9093"          # NODE_ID 1번 서버는 kafka-service:9093에서 찾는다
            # 5. 싱글 브로커 환경 설정(replicas: 1일 때)
            - name: KAFKA_OFFSETS_TOPIC_REPLICATION_FACTOR   # 메시지를 몇 대의 서버에 복제할지 설정
              value: "1"                                     # 서버가 1대이므로 1 (기본값은 3)
            - name: KAFKA_TRANSACTION_STATE_LOG_REPLICATION_FACTOR # 메시지 묶음 처리 기록을 몇 대에 복제할지 설정
              value: "1"                                     # 서버가 1대이므로 1 (기본값은 3)
            - name: KAFKA_TRANSACTION_STATE_LOG_MIN_ISR      # 위 기록이 최소 몇 대에 저장돼야 성공으로 처리하는지
              value: "1"                                     # 서버가 1대이므로 1 (기본값은 2)
```

Kafka Pod에 접근할 수 있도록 K8s Service를 정의합니다. kafka-deploy.yml의 `KAFKA_LISTENERS`에서 연 9092, 9093 포트를 외부에 노출합니다.
...
spec:
  ...
  ports:
    - port: 9092
      name: broker       # KAFKA_PROCESS_ROLES의 broker (메시지 저장/전달) 포트
    - port: 9093
      name: controller   # KAFKA_PROCESS_ROLES의 controller (토픽 목록 등 내부 정보 관리) 포트
```
