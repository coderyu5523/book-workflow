...
#show raw.where(block: true): it => {
  set text(size: 8pt, weight: "bold", font: ("Consolas", "Malgun Gothic"))
  block(
    width: 100%,
    fill: white,
    inset: (x: 16pt, y: 14pt),
...
#show raw.where(block: false): it => {
  box(
    ...
    inset: (x: 4pt, y: 2pt),
    radius: 3pt,
    text(size: 8.5pt, fill: rgb("#1e40af"), font: ("Consolas", "Malgun Gothic"))[#it]
  )
}
...
// ── 자동 크기 조절 이미지 ──
#let auto-image(path, alt: none, max-width: 0.7, border: false) = layout(size => context {
  ...
  let img-content = image(path, width: final-width)
  let body = if border {
    block(
      inset: 8pt,
      stroke: (paint: rgb("#5ba8d9"), thickness: 1.2pt, dash: "dashed"),
      img-content
    )
  } else {
    img-content
