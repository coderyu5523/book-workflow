...
**나**: "이거 화살표가 너무 많은데 어디서부터 봐야 돼?"

**동료**: "그 그림 전체를 이해하려고 하지 마. 우리가 하는 건 딱 하나야. 카카오한테 '이 사람 누구야?' 물어보는 거거든."
...
**동료** 가 말한 "카카오한테 '이 사람 누구야?' 물어보는 것"이 Access Token으로 사용자 정보를 조회하는 방식이었습니다. 거기에 ID Token을 더하면 전화를 한 번 줄일 수 있습니다. 두 방식을 직접 만들어 보겠습니다.
...
<img src="../assets/CH02/01_kakao-developer-app.png" width="720" alt="카카오 디벨로퍼 앱 설정 화면">

*그림 2-1: 카카오 디벨로퍼 앱 설정 화면 -- 플랫폼 키 페이지에서 REST API 키를 확인한다*

설정 항목은 세 가지입니다.

| 항목 | 값 | 설명 |
|------|-----|------|
| **REST API 키** | 앱 키 페이지에서 확인 | 카카오에 요청할 때 "나는 이 앱입니다"를 증명하는 키 |
| **Client Secret** | 보안 > Client Secret에서 발급 | 토큰 요청 시 추가 인증 수단. 활성화 후 사용 |
| **Redirect URI** | 카카오 로그인 > Redirect URI에 등록 | 사용자가 카카오 로그인을 마친 뒤 되돌아올 주소 |
...
<!-- [GEMINI PROMPT: 02a_oauth-authorization-code-grant]
path: assets/CH02/02a_oauth-authorization-code-grant.png
...
Three vertical columns representing actors (left to right):
1. stick figure icon labeled '사용자 (Resource Owner)'
2. minimalist server icon labeled 'Spring Boot (Client)'
3. minimalist shield icon labeled '카카오 인증서버 (Authorization Server)'

Numbered arrows between actors:
Arrow 1: 사용자 -> Spring Boot, labeled '1. 카카오 로그인 클릭'
Arrow 2: Spring Boot -> 카카오 인증서버, labeled '2. 인가 코드 요청 (redirect)'
Arrow 3: 카카오 인증서버 -> 사용자, labeled '3. 로그인 + 동의 화면'
Arrow 4: 사용자 -> 카카오 인증서버, labeled '4. 동의'
Arrow 5: 카카오 인증서버 -> Spring Boot, labeled '5. 인가 코드 전달 (redirect)'

Korean labels throughout.
Style: architecture-infographic
-->
![인가 코드 발급 흐름](../assets/CH02/02a_oauth-authorization-code-grant.png)
*그림 2-2a: 인가 코드 발급 — 사용자가 카카오에 동의하면 인가 코드가 서버로 전달된다*

<!-- [GEMINI PROMPT: 02b_oauth-token-exchange]
path: assets/CH02/02b_oauth-token-exchange.png
A minimalist black and white technical diagram with a strict 16:9 aspect ratio
on a solid white background. No shading, no 3D effects, only clean thin line art.
The entire assembly of icons, lines, and text is perfectly centered globally
within the 16:9 frame, leaving generous and equal white space on all sides.
...
Three vertical columns representing actors (left to right):
1. minimalist server icon labeled 'Spring Boot (Client)'
2. minimalist shield icon labeled '카카오 인증서버 (Authorization Server)'
3. minimalist database icon labeled '카카오 API (Resource Server)'

Numbered arrows between actors:
Arrow 1: Spring Boot -> 카카오 인증서버, labeled '6. 인가 코드 + Client ID → Access Token 교환'
Arrow 2: Spring Boot -> 카카오 API, labeled '7. Access Token으로 사용자 정보 요청'
Arrow 3: 카카오 API -> Spring Boot, labeled '8. 사용자 정보 응답 (닉네임 등)'

Korean labels throughout.
Style: architecture-infographic
-->
![토큰 교환 + 사용자 정보 조회 흐름](../assets/CH02/02b_oauth-token-exchange.png)
...
<img src="../assets/CH02/03_kakao-consent-screen.png" width="720" alt="카카오 동의 화면">

*그림 2-3: 카카오 로그인 동의 화면 -- 닉네임 제공에 동의하면 인가 코드가 발급된다*
...
<img src="../assets/CH02/04_access-token-exchange.png" width="720" alt="Access Token 교환 결과">

*그림 2-4: 인가 코드를 Access Token으로 교환한 뒤 콘솔에 출력된 TokenDTO*
...
<!-- [GEMINI PROMPT: 05a_oidc-id-token-issue]
path: assets/CH02/05a_oidc-id-token-issue.png
A minimalist black and white technical diagram with a strict 16:9 aspect ratio
on a solid white background. No shading, no 3D effects, only clean thin line art.
The entire assembly of icons, lines, and text is perfectly centered globally
within the 16:9 frame, leaving generous and equal white space on all sides.

Two vertical columns representing actors (left to right):
1. minimalist server icon labeled 'Spring Boot (Client)'
2. minimalist shield icon labeled '카카오 인증서버'

Numbered arrows:
Arrow 1: Spring Boot -> 카카오 인증서버, labeled '1. 인가 코드 + scope=openid → 토큰 요청'
Arrow 2: 카카오 인증서버 -> Spring Boot, labeled '2. Access Token + ID Token (JWT) 응답'

Korean labels throughout.
Style: architecture-infographic
-->
![OIDC ID Token 발급 흐름](../assets/CH02/05a_oidc-id-token-issue.png)
*그림 2-5a: ID Token 발급 — scope에 openid를 추가하면 ID Token이 함께 응답된다*

<!-- [GEMINI PROMPT: 05b_oidc-jwks-verification]
path: assets/CH02/05b_oidc-jwks-verification.png
A minimalist black and white technical diagram with a strict 16:9 aspect ratio
on a solid white background. No shading, no 3D effects, only clean thin line art.
The entire assembly of icons, lines, and text is perfectly centered globally
within the 16:9 frame, leaving generous and equal white space on all sides.
...
Two vertical columns representing actors (left to right):
1. minimalist server icon labeled 'Spring Boot (Client)'
2. minimalist key icon labeled 'JWKS 엔드포인트'

Numbered arrows:
Arrow 1: Spring Boot -> JWKS 엔드포인트, labeled '3. ID Token의 kid로 공개키 요청'
Arrow 2: JWKS 엔드포인트 -> Spring Boot, labeled '4. RSA 공개키 응답'
Arrow 3 (internal to Spring Boot): labeled '5. 공개키로 ID Token 서명 검증 → 사용자 정보 추출'

Korean labels throughout.
Style: architecture-infographic
-->
![JWKS 공개키 검증 흐름](../assets/CH02/05b_oidc-jwks-verification.png)
...
<img src="../assets/CH02/06_oidc-jwt-verification.png" width="720" alt="OIDC JWT 검증 성공 후 사용자 정보">

*그림 2-6: 카카오 OIDC 로그인 성공 응답 -- ID Token 검증 후 사용자 정보와 JWT가 반환된다*
