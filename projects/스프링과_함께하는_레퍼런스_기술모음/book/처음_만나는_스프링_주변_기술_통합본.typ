// ── Spring Infra 프로젝트 설정 ──
#let book-title = "Spring Infra"
...
#let book-header-title = "Spring Infra"
...
RabbitMQ 관리 콘솔에 메시지가 흘러가고 있었습니다. Producer가 보낸 데이터가 큐를 타고 Consumer 쪽으로 빠져나가는 걸 #strong[오픈이] 는 가만히 지켜봤습니다. 옆 모니터에는 Elasticsearch 대시보드가 떠 있고, 터미널 한쪽에서는 Docker 컨테이너 여섯 개가 조용히 돌아가고 있었습니다.

#emph[3개월 전에는 Docker가 뭔지도 몰랐는데.]
...
입사 첫날. #strong[팀장] 이 보내준 온보딩 문서의 첫 줄은 `docker compose up -d` 였습니다.

#emph[이거 치면 뭐가 되는 거지?]

커서가 깜빡이는 터미널 앞에서 손가락이 굳었습니다. Java를 깔고 MySQL을 깔고 버전이 안 맞아서 다시 지우고. 세 시간째 설치와 삭제를 반복하고 있었습니다. 옆자리 #strong[선배] 가 모니터를 흘끗 보더니 한마디 했어요.

#strong[선배]: "가전제품을 하나하나 사지 말고, 가전이 다 들어 있는 상자째로 가져와."

#emph[상자? 무슨 상자?]

그 상자가 #strong[Docker] 였습니다. 냉장고도 세탁기도 전자레인지도 이미 들어 있는 자급자족 상자. 열면 어디서든 똑같은 환경이 만들어집니다. "내 컴퓨터에서는 됐는데"라는 말이 사라진 첫 번째 주였습니다.

환경이 잡히자 #strong[팀장] 이 과제를 던졌습니다. 표정 하나 없이요.

#strong[팀장]: "카카오 소셜 로그인 붙여줘. 급하지 않으니까 천천히."

급하지 않다는 말이 위안이 될 줄 알았는데 #strong[OAuth 2.0] 이라는 단어 앞에서 발이 멈췄습니다. 인가 코드, 액세스 토큰, ID 토큰. 시퀀스 다이어그램의 화살표가 네 곳을 오갔습니다.

#emph[화살표가 왜 이렇게 많아. 이게 다 뭔데.]

한참을 들여다봐도 화살표가 어디서 출발해서 어디로 도착하는지 읽히질 않았습니다. #strong[선배] 가 옆에 앉더니 대사관에 비유해줬어요. 여행자가 여권을 보여주고 통행증을 받고 입국하는 절차. 화살표가 많아 보여도 결국 "이 사람이 맞는지 확인하고 통행증을 건네는" 흐름이었습니다.

#strong[오픈이]: "아, 그러니까 화살표마다 묻고 답하는 거군요."

#strong[선배]: "그래, 그것만 잡으면 돼."

흐름도를 세 번쯤 다시 그리고 나서야 화살표가 읽히기 시작했습니다.

로그인이 붙었습니다. 그런데 서버를 두 대로 늘리는 순간 문제가 터졌습니다. A 서버에서 로그인했는데 B 서버로 요청이 가면 "누구세요?" 상태.

#emph[방금 로그인했잖아. 왜 모르는 거야?]

세션이 서버 메모리에 갇혀 있었기 때문입니다. #strong[선배] 가 또 한마디 던졌어요.

#strong[선배]: "식당에 직원이 한 명일 때는 머릿속으로 주문을 기억해도 돼. 두 명이 되면?"

#strong[오픈이]: "메모장이요?"

#strong[선배]: "둘이 같이 볼 수 있는 메모장."

#strong[Redis] 가 그 공유 메모장이었습니다. 어떤 직원이 받아도 같은 메모장을 보니까 손님이 주문을 되풀이할 일이 없어졌어요. 한 줄 고쳤을 뿐인데 세션이 풀리지 않는 걸 보고 숨이 좀 트였습니다.

세션 문제를 해결하니 기획팀에서 요청이 왔습니다. "프로필 사진 넣어주세요."

#emph[겨우 하나 넘겼는데 벌써 다음이야?]

이미지를 서버로 보내는 법을 몰랐습니다. 케이크를 택배로 보내야 하는 상황을 떠올렸습니다. 실물 그대로 넣으면 부서지죠. 레시피로 바꿔 보내면 됩니다. 이미지를 문자열로 바꾸는 #strong[Base64] 가 그 레시피였습니다.

이미지가 쌓이기 시작하자 서버 디스크가 비명을 질렀습니다. 90퍼센트. 터미널에 경고가 뜨는 걸 보는 순간 등에 땀이 났습니다.

#emph[서비스 터지면 나한테 연락 오겠지.]

#strong[팀장] 이 "S3에 올려, 근데 서버가 직접 올리지 말고"라고 했습니다. 창고에 짐을 맡기되 매번 본인이 직접 갈 필요는 없습니다. 창고 측에서 허가증만 발급하면 택배 기사가 알아서 가져다 놓죠. #strong[S3] 가 창고이고 #strong[Presigned URL] 이 허가증입니다. #strong[Lambda] 가 이미지를 자동으로 줄여줬습니다. 디스크 사용률이 40퍼센트로 떨어지는 걸 확인하고서야 의자 등받이에 몸을 기댔습니다.

서버가 가벼워졌다고 숨 돌릴 틈은 없었습니다. 기획팀에서 다시 한마디. "새 댓글이 달리면 바로 알림이 떠야 해요."

#emph[실시간? 새로고침 말고 진짜 실시간?]

그 단어가 무거웠습니다. 새로고침 버튼 없이 화면이 혼자 바뀌어야 한다니. #strong[선배] 가 음식점 세 곳을 비유로 꺼냈습니다. 카운터에 가서 "다 됐어요?" 하고 묻는 가게, 진동벨을 주는 가게, 전화를 거는 가게. #strong[Polling] 은 카운터에 묻기, #strong[SSE] 는 진동벨, #strong[WebSocket] 은 전화. 셋 다 만들어보고 나서야 어느 가게가 우리 상황에 맞는지 보였습니다.
...
#strong[선배]: "이사할 때 큰 트럭 하나에 짐을 전부 실으면?"

#strong[오픈이]: "막히면 끝장이죠."

트럭이 막히면 아무것도 도착하지 않습니다. 짐을 작은 상자에 나눠 담고 번호를 매기면 먼저 도착한 상자부터 풀 수 있어요. #strong[HLS] 가 그 번호 매긴 상자였습니다. 쪼개니까 됐습니다. 비유를 듣고 나면 기술이 별것 아닌 것처럼 느껴지는 순간이 있는데, 그게 착각이 아니라 진짜 이해의 시작이었습니다.

영상까지 올라간 서비스에 사용자가 늘면서 검색이 5초나 걸린다는 리포트가 올라왔습니다. 기획팀 슬랙에 "검색 느려요"라는 메시지가 세 개째 쌓여 있었습니다.

#emph[내가 짠 코드 때문인 거 아냐?]

게시글이 수만 건을 넘기면서 데이터베이스가 서가를 처음부터 끝까지 훑고 있었던 겁니다. 도서관에서 책을 찾는 방법은 두 가지입니다. 서가를 한 칸씩 넘기거나, 카드 목록에서 키워드를 찾거나. #strong[Elasticsearch] 는 카드 목록을 만들어줬습니다. 5초 걸리던 검색이 0.2초로 줄었습니다. 슬랙에 "빨라졌어요"라는 답글이 달렸을 때 비로소 어깨에 힘이 풀렸습니다.

마지막 과제는 시스템 간 데이터 동기화였습니다. REST API로 직접 호출하면 되겠다고 생각했는데 상대 시스템이 느려지면 우리도 같이 멈춰 섰습니다.

#strong[선배]: "편지를 직접 전하러 가면, 상대가 집에 없을 때 어쩔 거야?"

#emph[또 비유야. 근데 이제는 비유가 나오면 코드가 같이 떠올라.]

우체국 사서함에 넣어두면 됩니다. #strong[RabbitMQ] 가 그 우체국이었습니다. 보내는 쪽은 사서함에 넣기만 하고 받는 쪽은 준비가 되면 꺼내갑니다. 서로의 사정에 묶이지 않게 됐습니다.
...
돌이켜보면 티켓 아홉 장. 매번 모르는 단어 앞에서 멈췄고 검색하고 틀리고 다시 읽었습니다. 대단한 깨달음 같은 건 없었어요. #strong[팀장] 이 넘겨준 과제를 하나씩 넘겼을 뿐입니다. 다만 한 가지 달라진 게 있습니다. 예전에는 기술 이름만 듣고 겁부터 먹었습니다. 지금은 문제가 보이면 기술이 떠오릅니다. 서버를 늘리니까 세션이 풀려? Redis. 검색이 느려? Elasticsearch. 상대 시스템이 불안정해? 메시지 큐. 이름을 외운 게 아니라 문제와 해결을 한 쌍으로 기억하게 된 겁니다.

이 책은 그 아홉 쌍의 기록입니다. #strong[오픈이] 가 부딪혔던 문제가 있고 #strong[선배] 가 던져준 비유로 감을 잡은 뒤 직접 만들어보며 넘어가는 과정이 있습니다. 처음부터 순서대로 읽으면 스타트업에서 3개월간 성장하는 주니어 개발자의 이야기를 따라가게 되고, 필요한 장만 골라 읽어도 괜찮아요. Docker만 먼저 읽어두면 나머지는 어디서 펼쳐도 됩니다.
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH01/diagram/01_exercise-flow.png", alt: [Docker 실습 흐름], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH01/gemini/01_docker-image-container.png", alt: [Docker 이미지와 컨테이너의 관계], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH01/gemini/02_docker-workflow.png", alt: [Docker 전체 워크플로우], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/diagram/02_exercise-flow.png", alt: [OAuth 실습 흐름], max-width: 0.6)

#emph[그림 2-1: 이번 챕터의 실습 흐름]

=== 2.3 준비 -- 카카오 디벨로퍼 앱 설정
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/setup/01_kakao-developer-main.png", alt: [카카오 디벨로퍼 메인 화면], max-width: 0.6)

#emph[카카오 디벨로퍼 메인 화면]

상단 네비게이션에서 #strong[내 애플리케이션] 을 클릭하고 #strong[앱 추가하기] 를 누릅니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/setup/02_create-app-click.png", alt: [앱 추가하기 버튼], max-width: 0.6)

#emph[앱 추가하기 버튼 위치]

앱 이름을 입력하고 저장합니다. 이름은 자유롭게 지으면 됩니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/setup/03_create-app-form.png", alt: [앱 생성 폼], max-width: 0.6)

#emph[앱 이름 입력 후 저장]

생성된 앱을 클릭해서 대시보드로 이동합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/setup/04_app-dashboard.png", alt: [앱 대시보드], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/setup/05_login-activate-oidc.png", alt: [카카오 로그인 활성화 + OIDC ON], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/setup/06_consent-items.png", alt: [동의항목 목록], max-width: 0.6)

#emph[동의항목 페이지 -- 개인정보 항목 목록이 표시된다]

#strong[닉네임] 항목의 #strong[설정] 을 클릭합니다. 동의 단계를 #strong[필수 동의] 로 변경하고 저장합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/setup/07_nickname-consent-setting.png", alt: [닉네임 동의 설정], max-width: 0.6)

#emph[닉네임 동의 단계를 필수 동의로 변경]

저장 후 닉네임 상태가 #strong[필수 동의] 로 바뀌었는지 확인합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/setup/08_consent-items-done.png", alt: [동의항목 설정 완료], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/setup/10_redirect-uri.png", alt: [Redirect URI 등록], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/setup/09_platform-keys.png", alt: [플랫폼 키 페이지], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/setup/11_client-secret.png", alt: [Client Secret 발급], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/gemini/02a_oauth-authorization-code-grant.png", alt: [인가 코드 발급 흐름], max-width: 0.6)

#emph[그림 2-2a: 인가 코드 발급 --- 사용자가 카카오에 동의하면 인가 코드가 서버로 전달된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/gemini/02b_oauth-token-exchange.png", alt: [토큰 교환 + 사용자 정보 조회 흐름], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/terminal/03_kakao-consent-screen.png", alt: [카카오 동의 화면], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/terminal/04_access-token-exchange.png", alt: [Access Token 교환 결과], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/gemini/05a_oidc-id-token-issue.png", alt: [OIDC ID Token 발급 흐름], max-width: 0.6)

#emph[그림 2-5a: ID Token 발급 --- scope에 openid를 추가하면 ID Token이 함께 응답된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/gemini/05b_oidc-jwks-verification.png", alt: [JWKS 공개키 검증 흐름], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/terminal/06_oidc-jwt-verification.png", alt: [OIDC JWT 검증 성공 후 사용자 정보], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH03/diagram/03_architecture.png", alt: [Redis 세션 공유 아키텍처], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH03/terminal/02_server-ab-role.png", alt: [서버 A와 B가 각각 독립적으로 Redis를 바라보는 구조], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH03/terminal/05_docker-compose-structure.png", alt: [docker-compose 구성 설명], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH03/terminal/09_session-share-summary.png", alt: [왜 Redis 세션 공유가 필요한가 정리], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/diagram/04_upload-flow.png", alt: [Base64 업로드 흐름], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/12_upload-flow-diagram.png", alt: [Postman에서 Base64 문자열을 JSON으로 전송하면 Spring이 디코딩하여 파일로 저장하고 DB에 기록한 뒤 URL을 반환하는 흐름], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/14_entity-design.png", alt: [ImageEntity 엔티티 구조 설명], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/15_dto-design.png", alt: [요청 DTO와 응답 DTO 설명], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/02_base64-encode-site.png", alt: [Base64 인코딩 사이트에서 이미지를 문자열로 변환하는 화면], max-width: 0.6)

#emph[그림 4-5: Base64 인코딩 -- 온라인 변환 사이트에서 이미지를 업로드하면 Base64 문자열이 생성된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/03_base64-encode-upload.png", alt: [Base64 인코딩 결과 문자열], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/05_postman-upload-request.png", alt: [Postman에서 upload 요청을 보내는 화면], max-width: 0.6)

#emph[그림 4-8: Postman 업로드 요청 -- fileName과 fileData를 JSON으로 전송한다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/06_postman-upload-send.png", alt: [Postman 요청 body에 Base64 문자열이 들어간 화면], max-width: 0.6)

#emph[그림 4-9: 요청 Body -- fileData에 Base64 문자열이 담겨 있다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/07_postman-upload-response.png", alt: [Postman 업로드 응답 화면], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/08_browser-image-check.png", alt: [브라우저에서 업로드된 이미지를 확인하는 화면], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/17_static-resource-mapping.png", alt: [정적 리소스 매핑 설명], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/10_postman-get-list.png", alt: [Postman으로 목록 조회하는 화면], max-width: 0.6)

#emph[그림 4-14: 목록 조회 -- 저장된 모든 이미지 목록이 반환된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/11_browser-image-view.png", alt: [브라우저에서 이미지를 확인하는 화면], max-width: 0.6)

#emph[그림 4-15: 브라우저 이미지 확인 -- 정적 리소스 매핑을 통해 이미지가 브라우저에 표시된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/18_browser-static-image.png", alt: [정적 리소스로 제공되는 이미지], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/19_react-upload.png", alt: [React에서 이미지를 업로드하는 화면], max-width: 0.6)

#emph[그림 4-17: React 이미지 업로드 -- 파일을 선택하면 Base64로 변환되어 서버에 전송된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/20_react-list.png", alt: [React에서 이미지 목록을 보는 화면], max-width: 0.6)

#emph[그림 4-18: React 이미지 목록 -- 서버에 저장된 이미지들이 리스트로 표시된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/21_react-detail.png", alt: [React에서 이미지 상세를 보는 화면], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/diagram/05_exercise-flow.png", alt: [Presigned URL 실습 흐름], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/gemini/05_upload-flow.png", alt: [Client가 Spring에 Presigned URL을 요청하고 받은 URL로 S3에 직접 업로드하는 흐름], max-width: 0.6)

#emph[업로드 단계 -- 서버는 URL만 발급하고 파일은 클라이언트가 S3에 직접 올린다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/gemini/05_resize-flow.png", alt: [S3에 이미지가 올라오면 Lambda가 리사이즈하고 Client가 Spring에 완료를 알리는 흐름], max-width: 0.6)
...
환경 변수로 관리하면 코드에 키가 노출되지 않습니다. 환경 준비에서 만든 `.env` 파일의 값이 여기에 주입됩니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/07_env-file.png", alt: [환경 변수 설정 파일], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/08_postman-presign-request.png", alt: [Postman에서 presigned URL 발급을 요청하는 화면], max-width: 0.6)

#emph[Presigned URL 요청 -- fileName과 contentType을 JSON으로 보낸다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/09_postman-presign-response.png", alt: [Postman에서 presigned URL 응답을 확인하는 화면], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/11_postman-s3-put-binary.png", alt: [Postman에서 S3에 바이너리 파일을 첨부하는 화면], max-width: 0.6)

#emph[S3 PUT Body -- Binary 탭에서 이미지 파일을 직접 선택한다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/12_postman-s3-put-response.png", alt: [Postman에서 S3 PUT 응답 200을 확인하는 화면], max-width: 0.6)

#emph[S3 업로드 성공 -- 200 OK가 반환되면 S3에 파일이 올라간 것이다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/13_s3-original-confirm.png", alt: [AWS 콘솔에서 S3 original 폴더에 파일이 올라간 것을 확인하는 화면], max-width: 0.6)

#emph[S3 확인 -- original 폴더에 업로드한 이미지가 저장되어 있다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/14_s3-original-image.png", alt: [S3에 저장된 원본 이미지를 미리보기하는 화면], max-width: 0.6)
...
환경 준비에서 생성한 Lambda 함수의 코드 탭으로 이동합니다. 코드 소스 에디터에 아래 코드를 입력합니다.
...
Pillow는 Lambda에 기본 내장되어 있지 않습니다. 환경 준비에서 Layer를 이미 추가했으므로 코드만 작성하면 됩니다.
...
5.4절에서 S3에 업로드한 이미지가 있다면 AWS S3 콘솔에서 `resized/` 폴더를 확인합니다. `original/` 에 올린 파일과 같은 UUID의 `.jpg` 파일이 `resized/` 에 생성되어 있으면 Lambda가 정상 동작하는 것입니다. 파일이 없다면 3\~5초 정도 기다린 뒤 새로고침합니다. Lambda 콘솔의 모니터링 탭에서 최근 호출 로그를 확인할 수도 있습니다.
]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/20_s3-resized-confirm.png", alt: [S3 resized 폴더에 리사이즈된 이미지가 저장된 것을 확인하는 화면], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/21_postman-complete-request.png", alt: [Postman에서 /complete 요청을 보내는 화면], max-width: 0.6)

#emph[\/complete 요청 -- originalKey를 JSON으로 보낸다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/22_postman-complete-response.png", alt: [Postman에서 /complete 응답을 확인하는 화면], max-width: 0.6)

#emph[\/complete 응답 -- originalUrl과 resizedUrl이 모두 저장되어 반환된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/23_h2-console-confirm.png", alt: [H2 콘솔에서 DB에 저장된 데이터를 확인하는 화면], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/24_postman-list-response.png", alt: [Postman에서 이미지 목록을 조회하는 화면], max-width: 0.6)

#emph[목록 조회 -- 저장된 모든 이미지의 원본 URL과 리사이즈 URL이 반환된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/25_postman-detail-response.png", alt: [Postman에서 이미지 상세를 조회하는 화면], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH06/diagram/06_exercise-flow.png", alt: [실시간 통신 실습 흐름], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH06/terminal/01_polling-devtools.png", alt: [DevTools Network 탭에서 2초마다 /chats 요청이 반복되는 화면], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH06/terminal/03_sse-connected-event.png", alt: [SSE connect 이벤트가 수신된 화면], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH06/terminal/05_sse-realtime-ui.png", alt: [브라우저 두 개에서 실시간으로 채팅이 동기화되는 화면], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH06/terminal/07_ws-subscribe.png", alt: [STOMP SUBSCRIBE 프레임을 확인하는 화면], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH06/terminal/08_ws-send-receive.png", alt: [WebSocket으로 메시지를 보내고 받는 화면], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH07/diagram/07_exercise-flow.png", alt: [HLS 실습 흐름], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH07/gemini/07_hls-flow.png", alt: [업로드부터 브라우저 재생까지의 HLS 스트리밍 흐름], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH07/terminal/01_postman-upload.png", alt: [Postman에서 영상 업로드 후 HLS 변환 완료 응답을 받는 화면], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH07/terminal/02_hls-files-generated.png", alt: [HLS 변환 후 생성된 m3u8과 ts 파일 목록], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH07/terminal/03_postman-m3u8-check.png", alt: [Postman에서 m3u8 엔드포인트 응답을 확인하는 화면], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/diagram/08_exercise-flow.png", alt: [ES 실습 흐름], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/gemini/08_inverted-index.png", alt: [역인덱스 구조도 --- 단어에서 문서로의 매핑], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/01_docker-compose-up.png", alt: [Docker Compose 실행 화면], max-width: 0.6)
...
브라우저에서 `http://localhost:5601` 에 접속하면 Kibana 메인 화면이 나옵니다. 왼쪽 사이드바 하단의 #strong[Management] 메뉴를 열고 #strong[Dev Tools] 를 클릭하면 Elasticsearch에 쿼리를 직접 실행할 수 있는 콘솔이 열립니다. 8.8절에서 이 콘솔을 사용합니다.
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/04_h2-data-confirm.png", alt: [H2 DB 데이터 확인], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/11_postman-normal-search.png", alt: [Postman 정상 검색], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/06_kibana-devtools-open.png", alt: [Kibana Dev Tools 검색], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/10_kibana-fuzzy-search.png", alt: [Kibana Fuzzy 검색 결과], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/12_postman-fuzzy-search.png", alt: [Postman 오타 검색], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/diagram/09_exercise-flow.png", alt: [RabbitMQ 실습 흐름], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/gemini/09_rabbitmq-flow.png", alt: [GitHub에서 Consumer까지의 RabbitMQ 전체 흐름도], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/03_rabbitmq-login.png", alt: [RabbitMQ 로그인 화면], max-width: 0.6)

#emph[RabbitMQ 관리 콘솔 로그인 화면 -- guest/guest로 접속한다]

로그인하면 관리 콘솔 메인 화면이 나옵니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/01_rabbitmq-management-ui.png", alt: [RabbitMQ 관리 콘솔 메인], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/06_producer-bootrun.png", alt: [Producer 서버 실행], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/08_producer-detect-log.png", alt: [Producer 변경 감지 로그], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/13_all-servers-running.png", alt: [전체 서버 실행 상태], max-width: 0.6)

#emph[Docker, Producer, Consumer 세 서버가 모두 실행 중이다]

RabbitMQ 관리 콘솔에서 Exchange 탭을 확인합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/04_exchanges-tab.png", alt: [Exchanges 탭 상태], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/10_queue-detail.png", alt: [Queue 상세], max-width: 0.6)

#emph[Queue 상세 -- repo-updates Queue의 메시지 수와 소비 상태가 보인다]

Binding 정보도 확인합니다. Exchange에서 Queue로의 연결 규칙입니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/11_binding-detail.png", alt: [Binding 상세], max-width: 0.6)

#emph[Binding 확인 -- readme.changed Routing Key로 바인딩되어 있다]

이제 GitHub에서 README를 수정하고 커밋합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/14_github-readme-commit.png", alt: [GitHub README 커밋], max-width: 0.6)

#emph[GitHub에서 README를 수정하고 커밋한다]

Producer가 변경을 감지합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/15_producer-polling-log.png", alt: [Producer 폴링 감지 로그], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/17_consumer-readme-updated.png", alt: [Consumer README 자동 업데이트], max-width: 0.6)

#emph[로컬 README 파일이 GitHub의 내용으로 자동 업데이트되었다]

백업 파일도 생성되었는지 확인합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/18_backup-file-confirm.png", alt: [백업 파일 확인], max-width: 0.6)
