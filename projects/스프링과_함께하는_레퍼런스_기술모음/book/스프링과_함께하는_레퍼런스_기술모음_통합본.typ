...
옆자리 #strong[동료] 가 슬쩍 모니터를 보더니 한마디 했습니다.

#strong[동료]: "그래서 Docker 쓰라니까. 나도 처음에 그랬어."
...
#strong[팀장] 이 "Docker로 환경 맞추세요"라고 말한 건, "이 상자를 열어서 실행만 하세요"라는 뜻이었습니다. 제가 자바를 직접 깔고, MySQL을 직접 잡고, 포트 충돌을 직접 해결한 건 --- 빈집에 배관부터 새로 놓은 셈이었습니다. #strong[동료] 가 docker-compose 명령어 하나를 알려줬습니다. 터미널에 입력하자 서버가 올라왔습니다. 자바 설치도, MySQL 설치도, 포트 설정도 필요 없었습니다. 상자 안에 전부 들어 있었으니까요.
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH01/diagram/01_exercise-flow.png", alt: [Docker 실습 흐름], max-width: 0.85)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH01/gemini/01_docker-image-container.png", alt: [Docker 이미지와 컨테이너의 관계], max-width: 0.7)
...
#emph[\[이미지: docker compose up 실행 --- 이미지 빌드 과정\]]

#emph[\[이미지: Spring Boot 서버가 8080 포트에서 시작된 로그\]]

빌드와 실행이 끝나면 브라우저에서 `http://localhost:8080` 에 접속합니다.

#emph[\[이미지: localhost:8080 접속 결과 --- hello world!. 텍스트가 표시된 브라우저 화면\]]

Spring 서버의 컨트롤러가 응답한 결과입니다.
...
#emph[\[이미지: Docker Desktop --- spring-docker 컨테이너가 실행 중인 화면\]]

자바 설치, 빌드 도구 설정, 포트 설정 --- 이 모든 과정이 `docker compose up --build` 한 줄로 끝났습니다. 컨테이너를 내릴 때는 `docker compose down` 을 실행합니다.

#emph[\[이미지: docker compose down 실행 결과 --- 컨테이너와 네트워크가 정리된 화면\]]

=== 1.4 Docker 전체 흐름

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH01/gemini/02_docker-workflow.png", alt: [Docker 전체 워크플로우], max-width: 0.7)
...
옆자리 #strong[동료] 에게 물었습니다.

#strong[나]: "이거 화살표가 너무 많은데 어디서부터 봐야 돼?"

#strong[동료]: "그 그림 전체를 이해하려고 하지 마. 우리가 하는 건 딱 하나야. 카카오한테 '이 사람 누구야?' 물어보는 거거든."
...
#strong[동료] 가 말한 "카카오한테 '이 사람 누구야?' 물어보는 것"이 Access Token으로 사용자 정보를 조회하는 방식이었습니다. 거기에 ID Token을 더하면 전화를 한 번 줄일 수 있습니다. 두 방식을 직접 만들어 보겠습니다.
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/diagram/02_exercise-flow.png", alt: [OAuth 실습 흐름], max-width: 0.85)

#emph[그림 2-1: 이번 챕터의 실습 흐름]

=== 2.1 준비 -- 카카오 디벨로퍼 앱 설정

카카오 소셜 로그인을 사용하려면 카카오 디벨로퍼 사이트에서 앱을 등록해야 합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/terminal/01_kakao-developer-app.png", alt: [카카오 디벨로퍼 앱 설정 화면], max-width: 0.6)

#emph[그림 2-1: 카카오 디벨로퍼 앱 설정 화면 -- 플랫폼 키 페이지에서 REST API 키를 확인한다]

설정 항목은 세 가지입니다.

#table(
    columns: (1fr, 1fr, 2fr),
    table.header([항목], [값], [설명],),
    [#strong[REST API 키]], [앱 키 페이지에서 확인], [카카오에 요청할 때 "나는 이 앱입니다"를 증명하는 키],
    [#strong[Client Secret]], [보안 \> Client Secret에서 발급], [토큰 요청 시 추가 인증 수단. 활성화 후 사용],
    [#strong[Redirect URI]], [카카오 로그인 \> Redirect URI에 등록], [사용자가 카카오 로그인을 마친 뒤 되돌아올 주소],
  )

Redirect URI는 `http://localhost:8080/oauth/callback` 으로 등록합니다.
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/gemini/02a_oauth-authorization-code-grant.png", alt: [인가 코드 발급 흐름], max-width: 0.7)

#emph[그림 2-2a: 인가 코드 발급 --- 사용자가 카카오에 동의하면 인가 코드가 서버로 전달된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/gemini/02b_oauth-token-exchange.png", alt: [토큰 교환 + 사용자 정보 조회 흐름], max-width: 0.7)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/terminal/03_kakao-consent-screen.png", alt: [카카오 동의 화면], max-width: 0.45)

#emph[그림 2-3: 카카오 로그인 동의 화면 -- 닉네임 제공에 동의하면 인가 코드가 발급된다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/terminal/04_access-token-exchange.png", alt: [Access Token 교환 결과], max-width: 0.7)

#emph[그림 2-4: 인가 코드를 Access Token으로 교환한 뒤 콘솔에 출력된 TokenDTO]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/gemini/05a_oidc-id-token-issue.png", alt: [OIDC ID Token 발급 흐름], max-width: 0.7)

#emph[그림 2-5a: ID Token 발급 --- scope에 openid를 추가하면 ID Token이 함께 응답된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/gemini/05b_oidc-jwks-verification.png", alt: [JWKS 공개키 검증 흐름], max-width: 0.7)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH02/terminal/06_oidc-jwt-verification.png", alt: [OIDC JWT 검증 성공 후 사용자 정보], max-width: 0.65)

#emph[그림 2-6: 카카오 OIDC 로그인 성공 응답 -- ID Token 검증 후 사용자 정보와 JWT가 반환된다]
...
30분 뒤 #strong[동료] 가 고개를 돌렸습니다.

#strong[동료]: "나 방금 로그인했는데 페이지 새로고침하니까 다시 로그인하래."
...
#strong[나]: "서버마다 세션이 따로 있어서 그래. Redis에 세션을 모으면 돼."

#strong[동료]: "아 그러면 서버가 10대가 돼도 상관없겠네."
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH03/diagram/03_architecture.png", alt: [Redis 세션 공유 아키텍처], max-width: 0.7)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH03/terminal/01_architecture-overview.png", alt: [Client에서 Nginx를 거쳐 Spring A, Spring B로 분기되고 두 서버 모두 Redis를 바라보는 구조], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH03/terminal/02_server-ab-role.png", alt: [서버 A와 B가 각각 독립적으로 Redis를 바라보는 구조], max-width: 0.65)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH03/terminal/04_session-share-code.png", alt: [세션 공유 코드 설명], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH03/terminal/03_nginx-routing.png", alt: [Nginx 경로 기반 라우팅 설명], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH03/terminal/05_docker-compose-structure.png", alt: [docker-compose 구성 설명], max-width: 0.7)
...
빌드가 끝나면 브라우저에서 확인합니다.

먼저 `localhost/app1/` 에 접속합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH03/terminal/06_app1-count1.png", alt: [\/app1 접속 시 count=1], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH03/terminal/07_app2-count2.png", alt: [\/app2 접속 시 count=2], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH03/terminal/08_app1-count3.png", alt: [다시 /app1 접속 시 count=3], max-width: 0.6)

#emph[그림 3-8: 다시 /app1 접속 -- count가 3으로 이어진다. 서버가 바뀌어도 세션이 유지된다]

count가 3입니다. 서버 A -\> 서버 B -\> 서버 A로 요청이 오갔지만 세션이 끊기지 않았습니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH03/terminal/09_session-share-summary.png", alt: [왜 Redis 세션 공유가 필요한가 정리], max-width: 0.65)
...
#strong[나]: "이미지를 서버에 보내려면 어떻게 해?"

#strong[동료]: "Multipart로 보내거나 Base64로 인코딩해서 보내거나."

#strong[나]: "Base64가 뭔데?"

#strong[동료]: "사진을 글자로 바꾸는 거야."
...
#strong[동료] 에게 다시 말했습니다.

#strong[나]: "아 그러면 프론트에서 이미지를 텍스트로 바꿔서 JSON으로 보내면 되는 거네."

#strong[동료]: "맞아. 서버에서 다시 파일로 복원해서 저장하면 끝이야."
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/diagram/04_upload-flow.png", alt: [Base64 업로드 흐름], max-width: 0.7)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/12_upload-flow-diagram.png", alt: [Postman에서 Base64 문자열을 JSON으로 전송하면 Spring이 디코딩하여 파일로 저장하고 DB에 기록한 뒤 URL을 반환하는 흐름], max-width: 0.65)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/14_entity-design.png", alt: [ImageEntity 엔티티 구조 설명], max-width: 0.7)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/15_dto-design.png", alt: [요청 DTO와 응답 DTO 설명], max-width: 0.65)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/16_upload-api-flow.png", alt: [업로드 API 내부 흐름 설명], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/02_base64-encode-site.png", alt: [Base64 인코딩 사이트에서 이미지를 문자열로 변환하는 화면], max-width: 0.45)

#emph[그림 4-5: Base64 인코딩 -- 온라인 변환 사이트에서 이미지를 업로드하면 Base64 문자열이 생성된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/03_base64-encode-upload.png", alt: [Base64 인코딩 결과 문자열], max-width: 0.45)

#emph[그림 4-6: 인코딩 결과 -- 이미지가 긴 텍스트 문자열로 변환된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/04_base64-encode-result.png", alt: [Base64 문자열 복사], max-width: 0.6)

#emph[그림 4-7: 문자열 복사 -- 이 문자열을 Postman의 JSON body에 붙여넣는다]

변환된 문자열을 Postman에서 JSON에 담아 보냅니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/05_postman-upload-request.png", alt: [Postman에서 upload 요청을 보내는 화면], max-width: 0.65)

#emph[그림 4-8: Postman 업로드 요청 -- fileName과 fileData를 JSON으로 전송한다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/06_postman-upload-send.png", alt: [Postman 요청 body에 Base64 문자열이 들어간 화면], max-width: 0.7)

#emph[그림 4-9: 요청 Body -- fileData에 Base64 문자열이 담겨 있다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/07_postman-upload-response.png", alt: [Postman 업로드 응답 화면], max-width: 0.7)

#emph[그림 4-10: 업로드 응답 -- id, uuid, fileName, url, createdAt이 반환된다]

응답에 `url` 이 `/uploads/UUID.png` 형태로 돌아옵니다. 이 경로로 브라우저에서 이미지를 볼 수 있습니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/08_browser-image-check.png", alt: [브라우저에서 업로드된 이미지를 확인하는 화면], max-width: 0.7)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/17_static-resource-mapping.png", alt: [정적 리소스 매핑 설명], max-width: 0.65)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/09_postman-get-by-id.png", alt: [Postman으로 단건 조회하는 화면], max-width: 0.6)

#emph[그림 4-13: 단건 조회 -- id로 특정 이미지 정보를 조회한다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/10_postman-get-list.png", alt: [Postman으로 목록 조회하는 화면], max-width: 0.45)

#emph[그림 4-14: 목록 조회 -- 저장된 모든 이미지 목록이 반환된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/11_browser-image-view.png", alt: [브라우저에서 이미지를 확인하는 화면], max-width: 0.45)

#emph[그림 4-15: 브라우저 이미지 확인 -- 정적 리소스 매핑을 통해 이미지가 브라우저에 표시된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/18_browser-static-image.png", alt: [정적 리소스로 제공되는 이미지], max-width: 0.7)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/19_react-upload.png", alt: [React에서 이미지를 업로드하는 화면], max-width: 0.45)

#emph[그림 4-17: React 이미지 업로드 -- 파일을 선택하면 Base64로 변환되어 서버에 전송된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/20_react-list.png", alt: [React에서 이미지 목록을 보는 화면], max-width: 0.4)

#emph[그림 4-18: React 이미지 목록 -- 서버에 저장된 이미지들이 리스트로 표시된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH04/terminal/21_react-detail.png", alt: [React에서 이미지 상세를 보는 화면], max-width: 0.4)
...
#strong[나]: "이미지 저장소를 밖으로 빼야 할 것 같은데 어떻게 해?"

#strong[동료]: "클라우드 스토리지 써. AWS S3 같은 거. 그리고 #strong[Presigned URL] 쓰면 서버가 파일을 직접 받지도 않아."

#strong[나]: "서버가 파일을 안 받는다고?"

#strong[동료]: "서버는 허가증만 발급해. 파일은 클라이언트가 직접 S3에 올려."
...
#strong[나]: "그러면 서버는 출입증 발급이랑 결과 기록만 하면 되는 거네."

#strong[동료]: "맞아. 참고로 이 장은 AWS 계정이 필요해. 프리 티어로 충분하니까 없으면 하나 만들어."
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/diagram/05_exercise-flow.png", alt: [Presigned URL 실습 흐름], max-width: 0.85)

#emph[그림 5-1: 이번 챕터의 실습 흐름]

=== 5.1 아키텍처 구조

전체 흐름은 5단계입니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/gemini/05_upload-flow.png", alt: [Client가 Spring에 Presigned URL을 요청하고 받은 URL로 S3에 직접 업로드하는 흐름], max-width: 0.65)

#emph[그림 5-1a: 업로드 단계 -- 서버는 URL만 발급하고 파일은 클라이언트가 S3에 직접 올린다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/gemini/05_resize-flow.png", alt: [S3에 이미지가 올라오면 Lambda가 리사이즈하고 Client가 Spring에 완료를 알리는 흐름], max-width: 0.65)

#emph[그림 5-1b: 후처리 단계 -- Lambda가 자동으로 리사이즈하고 Spring은 결과만 기록한다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/07_env-file.png", alt: [환경 변수 설정 파일], max-width: 0.65)

#emph[그림 5-2: 환경 변수 설정 -- AWS 키를 환경 변수로 관리하면 코드에 노출되지 않는다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/08_postman-presign-request.png", alt: [Postman에서 presigned URL 발급을 요청하는 화면], max-width: 0.7)

#emph[그림 5-3: Presigned URL 요청 -- fileName과 contentType을 JSON으로 보낸다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/09_postman-presign-response.png", alt: [Postman에서 presigned URL 응답을 확인하는 화면], max-width: 0.45)

#emph[그림 5-4: Presigned URL 응답 -- presignedUrl과 key가 반환된다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/10_postman-s3-put-header.png", alt: [Postman에서 S3 PUT 요청 헤더를 설정하는 화면], max-width: 0.6)

#emph[그림 5-5: S3 PUT 헤더 -- Content-Type을 presigned 요청과 동일하게 맞춘다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/11_postman-s3-put-binary.png", alt: [Postman에서 S3에 바이너리 파일을 첨부하는 화면], max-width: 0.7)

#emph[그림 5-6: S3 PUT Body -- Binary 탭에서 이미지 파일을 직접 선택한다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/12_postman-s3-put-response.png", alt: [Postman에서 S3 PUT 응답 200을 확인하는 화면], max-width: 0.7)

#emph[그림 5-7: S3 업로드 성공 -- 200 OK가 반환되면 S3에 파일이 올라간 것이다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/13_s3-original-confirm.png", alt: [AWS 콘솔에서 S3 original 폴더에 파일이 올라간 것을 확인하는 화면], max-width: 0.65)

#emph[그림 5-8: S3 확인 -- original 폴더에 업로드한 이미지가 저장되어 있다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/14_s3-original-image.png", alt: [S3에 저장된 원본 이미지를 미리보기하는 화면], max-width: 0.7)

#emph[그림 5-9: 원본 이미지 확인 -- S3에 저장된 이미지를 미리보기로 확인할 수 있다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/05_lambda-create.png", alt: [AWS Lambda 함수를 생성하는 화면], max-width: 0.6)

#emph[그림 5-10: Lambda 함수 생성 -- Python 3.x 런타임으로 함수를 만든다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/16_lambda-layer-add.png", alt: [Lambda에 Layer를 추가하는 화면], max-width: 0.7)

#emph[그림 5-11: Pillow Layer 추가 -- Lambda에 이미지 처리 라이브러리를 Layer로 추가한다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/17_lambda-layer-arn.png", alt: [Pillow Layer ARN을 입력하는 화면], max-width: 0.65)

#emph[그림 5-12: Layer ARN -- Pillow Layer의 ARN을 지정한다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/18_lambda-layer-confirm.png", alt: [Layer가 추가된 것을 확인하는 화면], max-width: 0.7)

#emph[그림 5-13: Layer 확인 -- Pillow Layer가 정상적으로 추가되었다]

S3 이벤트 트리거를 설정합니다. `original/` 폴더에 파일이 올라오면 Lambda가 자동으로 실행되도록 연결합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/06_lambda-trigger.png", alt: [Lambda에 S3 트리거를 설정하는 화면], max-width: 0.45)

#emph[그림 5-14: S3 트리거 설정 -- original/ prefix로 ObjectCreated 이벤트를 연결한다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/15_lambda-code-deploy.png", alt: [Lambda 코드를 배포하는 화면], max-width: 0.6)

#emph[그림 5-15: Lambda 배포 -- 코드를 작성하고 Deploy 버튼으로 배포한다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/19_lambda-test.png", alt: [Lambda 테스트 실행 결과], max-width: 0.7)

#emph[그림 5-16: Lambda 테스트 -- 테스트 이벤트로 함수가 정상 실행되는지 확인한다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/20_s3-resized-confirm.png", alt: [S3 resized 폴더에 리사이즈된 이미지가 저장된 것을 확인하는 화면], max-width: 0.45)

#emph[그림 5-17: 리사이즈 확인 -- resized 폴더에 축소된 이미지가 자동으로 생성되었다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/21_postman-complete-request.png", alt: [Postman에서 /complete 요청을 보내는 화면], max-width: 0.65)

#emph[그림 5-18: /complete 요청 -- originalKey를 JSON으로 보낸다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/22_postman-complete-response.png", alt: [Postman에서 /complete 응답을 확인하는 화면], max-width: 0.45)

#emph[그림 5-19: /complete 응답 -- originalUrl과 resizedUrl이 모두 저장되어 반환된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/23_h2-console-confirm.png", alt: [H2 콘솔에서 DB에 저장된 데이터를 확인하는 화면], max-width: 0.7)

#emph[그림 5-20: DB 확인 -- H2 콘솔에서 원본 URL과 리사이즈 URL이 저장된 것을 확인한다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/24_postman-list-response.png", alt: [Postman에서 이미지 목록을 조회하는 화면], max-width: 0.45)

#emph[그림 5-21: 목록 조회 -- 저장된 모든 이미지의 원본 URL과 리사이즈 URL이 반환된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/25_postman-detail-response.png", alt: [Postman에서 이미지 상세를 조회하는 화면], max-width: 0.65)

#emph[그림 5-22: 상세 조회 -- 특정 이미지의 전체 정보가 반환된다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/01_aws-s3-bucket.png", alt: [AWS S3 버킷 생성 화면], max-width: 0.6)

#emph[그림 5-23: S3 버킷 생성 -- 리전을 선택하고 버킷 이름을 지정한다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/02_s3-folder-structure.png", alt: [S3 버킷 내 original, resized 폴더 구조], max-width: 0.7)

#emph[그림 5-24: 폴더 구조 -- original과 resized 두 개의 폴더를 만든다]

#table(
    columns: 2,
    table.header([설정 항목], [값],),
    [버킷 이름], [프로젝트에 맞게 지정],
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/03_iam-user-create.png", alt: [IAM 사용자 생성 화면], max-width: 0.6)

#emph[그림 5-25: IAM 사용자 생성 -- S3 접근용 사용자를 만든다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/04_iam-access-key.png", alt: [IAM 액세스 키 생성 화면], max-width: 0.45)

#emph[그림 5-26: 액세스 키 발급 -- Access Key와 Secret Key를 발급받아 환경 변수에 설정한다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH05/terminal/26_s3-cors-setting.png", alt: [S3 CORS 설정 화면], max-width: 0.45)

#emph[그림 5-27: S3 CORS -- 프론트엔드에서 S3에 직접 요청할 수 있도록 CORS를 설정한다]
...
#strong[나]: "알림이요? 지금도 새로고침하면 보이긴 하는데…"
...
#strong[나]: "실시간 알림을 만들어야 하는데 어떻게 해?"

#strong[동료]: "방법이 세 가지 있어. 쉬운 것부터 말해줄게."

#strong[동료] 가 화이트보드에 그림을 그리기 시작했습니다.
...
#strong[동료]: "단순한 알림이면 진동벨이면 충분해. 근데 채팅처럼 서로 주고받아야 하면 전화가 필요하고."

#strong[나]: "세 가지를 다 만들어 보면 감이 오겠다."

#strong[동료]: "이건 세 가지 방식을 각각 따로 만들어 볼 거야. 같은 채팅 기능인데 구현이 전부 달라."
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH06/diagram/06_exercise-flow.png", alt: [실시간 통신 실습 흐름], max-width: 0.85)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH06/gemini/06_communication-compare.png", alt: [Polling, SSE, WebSocket 세 방식의 통신 방향 비교], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH06/terminal/01_polling-devtools.png", alt: [DevTools Network 탭에서 2초마다 /chats 요청이 반복되는 화면], max-width: 0.45)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH06/terminal/02_sse-connect.png", alt: [DevTools에서 SSE 연결 요청을 확인하는 화면], max-width: 0.6)

#emph[그림 6-3: SSE 연결 -- /chats/connect 요청이 한 번만 나가고 연결이 유지된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH06/terminal/03_sse-connected-event.png", alt: [SSE connect 이벤트가 수신된 화면], max-width: 0.7)

#emph[그림 6-4: connect 이벤트 -- 연결 즉시 서버가 "connected" 메시지를 보내온다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH06/terminal/04_sse-chat-event.png", alt: [SSE chat 이벤트로 새 메시지가 수신된 화면], max-width: 0.6)

#emph[그림 6-5: chat 이벤트 -- 다른 사용자가 채팅을 보내면 서버가 자동으로 밀어준다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH06/terminal/05_sse-realtime-ui.png", alt: [브라우저 두 개에서 실시간으로 채팅이 동기화되는 화면], max-width: 0.7)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH06/terminal/06_ws-devtools.png", alt: [DevTools에서 WebSocket 연결을 확인하는 화면], max-width: 0.6)

#emph[그림 6-7: WebSocket 연결 -- /ws 엔드포인트로 한 번 연결되면 끊기지 않는다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH06/terminal/07_ws-subscribe.png", alt: [STOMP SUBSCRIBE 프레임을 확인하는 화면], max-width: 0.7)

#emph[그림 6-8: STOMP 구독 -- /topic/chats를 구독하면 서버가 보내는 메시지를 받을 준비가 된다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH06/terminal/08_ws-send-receive.png", alt: [WebSocket으로 메시지를 보내고 받는 화면], max-width: 0.45)

#emph[그림 6-9: 양방향 통신 -- 클라이언트가 보낸 메시지가 구독자 전원에게 전달된다]

SSE와 달리 클라이언트도 서버에 메시지를 보낼 수 있습니다. 채팅, 실시간 협업 편집, 게임처럼 양쪽이 수시로 데이터를 주고받아야 하는 경우에 적합합니다.

=== 6.5 세 방식 비교 정리

같은 채팅 기능을 세 가지 방식으로 만들어 봤습니다.
...
#strong[나]: "영상이면 지난번에 만든 WebSocket으로 스트리밍하면 되지 않나요?"
...
#strong[동료]: "그거 통째로 보내면 안 돼. 잘게 쪼개서 보내야 해."

#strong[나]: "쪼갠다고?"

#strong[동료]: "유튜브 생각해 봐. 영상 틀면 바로 재생되잖아. 전체를 다 받아서 트는 게 아니야."
...
#strong[동료]: "이삿짐을 상자로 나눠서 번호 매기는 거랑 똑같아. 목록표 한 장이랑 상자 여러 개."

\(목록표가 m3u8이고 상자가 ts 파일이구나.)

#strong[나]: "그러면 영상을 쪼개는 건 누가 해?"

#strong[동료]: "FFmpeg이라는 도구가 해. 영상을 넣으면 알아서 잘라줘."
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH07/diagram/07_exercise-flow.png", alt: [HLS 실습 흐름], max-width: 0.85)

#emph[그림 7-1: 이번 챕터의 실습 흐름]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH07/gemini/07_hls-flow.png", alt: [업로드부터 브라우저 재생까지의 HLS 스트리밍 흐름], max-width: 0.65)

#emph[그림 7-1: 업로드부터 브라우저 재생까지의 HLS 흐름]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH07/terminal/01_postman-upload.png", alt: [Postman에서 영상 업로드 후 HLS 변환 완료 응답을 받는 화면], max-width: 0.65)

#emph[그림 7-2: 영상 업로드 -- Postman으로 파일을 보내면 HLS 변환이 시작된다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH07/terminal/02_hls-files-generated.png", alt: [HLS 변환 후 생성된 m3u8과 ts 파일 목록], max-width: 0.45)

#emph[그림 7-3: HLS 파일 생성 -- m3u8 목록 파일과 ts 조각 파일이 만들어졌다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH07/terminal/03_postman-m3u8-check.png", alt: [Postman에서 m3u8 엔드포인트 응답을 확인하는 화면], max-width: 0.65)

#emph[그림 7-4: m3u8 엔드포인트 확인 -- 재생 목록이 정상적으로 반환된다]

720p와 1080p의 m3u8 응답입니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH07/terminal/04_m3u8-720p-response.png", alt: [720p m3u8 응답 내용], max-width: 0.6)

#emph[그림 7-5: 720p m3u8 응답 -- ts 조각 파일 목록과 재생 시간이 기록되어 있다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH07/terminal/05_m3u8-1080p-response.png", alt: [1080p m3u8 응답 내용], max-width: 0.6)

#emph[그림 7-6: 1080p m3u8 응답 -- 같은 구조지만 해상도가 다르다]

브라우저에서 실제로 재생하면서 DevTools Network 탭을 열어 봅니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH07/terminal/06_browser-player-devtools.png", alt: [브라우저에서 HLS 영상이 재생되며 DevTools에 ts 파일 요청이 나타나는 화면], max-width: 0.6)

#emph[그림 7-7: 브라우저 재생 -- ts 조각이 순차적으로 요청되며 영상이 끊기지 않고 재생된다]
...
#strong[동료]: "LIKE에 앞뒤로 % 붙이면 인덱스를 못 타. 테이블 전체를 처음부터 끝까지 훑어야 해."

#strong[나]: "그러면 인덱스를 걸면 되지 않아?"

#strong[동료]: "앞에 %가 붙으면 인덱스가 소용없어. 데이터베이스 인덱스는 앞글자 기준으로 정렬해 놓은 거거든. '갤럭시'로 시작하는 건 찾을 수 있어도 중간에 '갤럭시'가 들어간 건 못 찾아."

\(그러면 어떻게 해야 하지?)

#strong[동료]: "검색 전용 엔진을 따로 두는 거야."
...
#strong[동료]: "맞아. 데이터베이스에도 넣고 검색 엔진에도 넣어. 저장은 데이터베이스가 하고 검색은 검색 엔진이 하는 거야."
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/diagram/08_exercise-flow.png", alt: [ES 실습 흐름], max-width: 0.85)

#emph[그림 8-1: 이번 챕터의 실습 흐름]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/gemini/08_inverted-index.png", alt: [역인덱스 구조도 --- 단어에서 문서로의 매핑], max-width: 0.65)

#emph[그림 8-1: 역인덱스 구조 --- 단어 목록에서 해당 문서를 바로 찾는다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/01_docker-compose-up.png", alt: [Docker Compose 실행 화면], max-width: 0.65)

#emph[그림 8-1: docker-compose up -- Elasticsearch, Kibana, Spring Boot 앱이 함께 올라간다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/02_es-health-check.png", alt: [ES 9200 응답 확인], max-width: 0.6)

#emph[그림 8-2: localhost:9200 응답 -- Elasticsearch가 정상 동작하고 있다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/03_postman-bulk-save.png", alt: [Postman 데이터 저장 요청], max-width: 0.6)

#emph[그림 8-3: Postman으로 데이터 저장 -- 요청이 성공하면 저장된 Document가 응답으로 온다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/04_h2-data-confirm.png", alt: [H2 DB 데이터 확인], max-width: 0.45)

#emph[그림 8-4: H2 콘솔 확인 -- RDB에도 같은 데이터가 들어가 있다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/11_postman-normal-search.png", alt: [Postman 정상 검색], max-width: 0.45)

#emph[그림 8-5: Postman 검색 -- "갤럭시"로 검색하면 관련 데이터가 반환된다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/05_kibana-home.png", alt: [Kibana 메인 화면], max-width: 0.6)

#emph[그림 8-6: Kibana 메인 화면 -- 왼쪽 메뉴에서 Dev Tools를 찾는다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/06_kibana-devtools-open.png", alt: [Kibana Dev Tools 검색], max-width: 0.7)

#emph[그림 8-7: Dev Tools 진입 -- 검색 쿼리를 직접 실행할 수 있는 콘솔이 열린다]

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/07_kibana-devtools-console.png", alt: [Dev Tools 콘솔 화면], max-width: 0.6)

#emph[그림 8-8: Dev Tools 콘솔 -- 왼쪽에 쿼리를 작성하고 오른쪽에서 결과를 확인한다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/10_kibana-fuzzy-search.png", alt: [Kibana Fuzzy 검색 결과], max-width: 0.45)

#emph[그림 8-9: Kibana Fuzzy 검색 -- "갈럭시"로 검색해도 "갤럭시" 문서가 나온다]

Kibana에서 직접 문서를 삽입할 수도 있습니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/08_kibana-insert-doc.png", alt: [Kibana 문서 삽입], max-width: 0.6)

#emph[그림 8-10: Kibana에서 문서 직접 삽입 -- PUT 요청으로 ES에 문서를 넣을 수 있다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/09_h2-no-kibana-insert.png", alt: [Kibana 삽입 후 H2 미반영], max-width: 0.6)

#emph[그림 8-11: H2 확인 -- Kibana에서 넣은 데이터는 RDB에 없다]

Postman에서 오타 검색도 확인합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH08/terminal/12_postman-fuzzy-search.png", alt: [Postman 오타 검색], max-width: 0.45)

#emph[그림 8-12: Postman Fuzzy 검색 -- 앱을 통한 오타 검색도 정상 동작한다]
...
#strong[동료]: "그러면 반영하는 서버가 내려가 있으면 어떻게 돼?"

#strong[나]: "요청이 실패하겠지."

#strong[동료]: "실패한 건 누가 다시 보내? 감지 서버가 재시도 로직까지 갖고 있어야 해. 반영 서버가 두 대로 늘어나면? 셋으로 늘어나면?"
...
#strong[동료]: "메시지 큐를 쓰면 보내는 쪽은 큐에 넣기만 하면 돼. 받는 쪽이 죽어 있어도 메시지는 큐에 남아 있고, 살아나면 그때 가져가."
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/diagram/09_exercise-flow.png", alt: [RabbitMQ 실습 흐름], max-width: 0.85)

#emph[그림 9-1: 이번 챕터의 실습 흐름]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/gemini/09_rabbitmq-flow.png", alt: [GitHub에서 Consumer까지의 RabbitMQ 전체 흐름도], max-width: 0.7)

#emph[그림 9-1: 전체 흐름 --- GitHub 변경이 Producer를 거쳐 RabbitMQ를 통해 Consumer에 전달된다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/03_rabbitmq-login.png", alt: [RabbitMQ 로그인 화면], max-width: 0.65)

#emph[그림 9-1: RabbitMQ 관리 콘솔 로그인 화면]

로그인하면 관리 콘솔 메인 화면이 나옵니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/01_rabbitmq-management-ui.png", alt: [RabbitMQ 관리 콘솔 메인], max-width: 0.45)

#emph[그림 9-2: RabbitMQ 관리 콘솔 -- 연결, 채널, Exchange, Queue 상태를 한눈에 볼 수 있다]

GitHub에 테스트용 레포를 하나 만들어 둡니다. 레포 이름은 `config-readme` 로 하고 README.md 파일을 생성합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/02_github-config-readme-repo.png", alt: [config-readme 레포 생성], max-width: 0.6)

#emph[그림 9-3: GitHub에 config-readme 레포 생성 -- 이 레포의 README 변경을 감지할 것이다]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/06_producer-bootrun.png", alt: [Producer 서버 실행], max-width: 0.7)

#emph[그림 9-4: Producer 서버 실행 -- 60초마다 GitHub을 폴링한다]

GitHub에서 README를 수정하고 커밋합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/07_github-readme-edit.png", alt: [GitHub README 수정], max-width: 0.6)

#emph[그림 9-5: GitHub에서 README 수정]

Producer 로그에서 변경 감지를 확인합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/08_producer-detect-log.png", alt: [Producer 변경 감지 로그], max-width: 0.65)

#emph[그림 9-6: Producer가 SHA 변경을 감지하고 메시지를 발행한 로그]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/13_all-servers-running.png", alt: [전체 서버 실행 상태], max-width: 0.7)

#emph[그림 9-7: Docker, Producer, Consumer 세 서버가 모두 실행 중이다]

RabbitMQ 관리 콘솔에서 Exchange 탭을 확인합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/04_exchanges-tab.png", alt: [Exchanges 탭 상태], max-width: 0.45)

#emph[그림 9-8: Exchanges 탭 -- Producer가 등록한 github.events Exchange가 보인다]

Exchange 상세에서 Binding 정보를 확인합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/09_exchange-detail.png", alt: [Exchange 상세], max-width: 0.6)

#emph[그림 9-9: Exchange 상세 -- repo-updates Queue에 readme.changed Routing Key로 바인딩되어 있다]

Queue 상세에서 메시지 수와 상태를 확인합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/10_queue-detail.png", alt: [Queue 상세], max-width: 0.65)

#emph[그림 9-10: Queue 상세 -- repo-updates Queue의 메시지 수와 소비 상태가 보인다]

Binding 정보도 확인합니다. Exchange에서 Queue로의 연결 규칙입니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/11_binding-detail.png", alt: [Binding 상세], max-width: 0.45)

#emph[그림 9-11: Binding 확인 -- readme.changed Routing Key로 바인딩되어 있다]

이제 GitHub에서 README를 수정하고 커밋합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/14_github-readme-commit.png", alt: [GitHub README 커밋], max-width: 0.45)

#emph[그림 9-12: GitHub에서 README를 수정하고 커밋한다]

Producer가 변경을 감지합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/15_producer-polling-log.png", alt: [Producer 폴링 감지 로그], max-width: 0.7)

#emph[그림 9-13: Producer가 SHA 변경을 감지하고 메시지를 발행한다]

Consumer가 메시지를 수신합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/16_consumer-receive-log.png", alt: [Consumer 메시지 수신 로그], max-width: 0.6)

#emph[그림 9-14: Consumer가 메시지를 수신하고 파일 반영을 시작한다]

로컬 README 파일이 자동으로 업데이트됩니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/17_consumer-readme-updated.png", alt: [Consumer README 자동 업데이트], max-width: 0.65)

#emph[그림 9-15: 로컬 README 파일이 GitHub의 내용으로 자동 업데이트되었다]

백업 파일도 생성되었는지 확인합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/18_backup-file-confirm.png", alt: [백업 파일 확인], max-width: 0.45)

#emph[그림 9-16: 이전 내용이 백업 파일로 저장되어 있다]

관리 콘솔에서 Queue에 들어온 메시지 내용을 직접 확인할 수도 있습니다. Queue 상세 화면에서 Get Messages를 클릭합니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/스프링과_함께하는_레퍼런스_기술모음/assets/CH09/terminal/12_get-messages-payload.png", alt: [Queue 메시지 payload 확인], max-width: 0.6)

#emph[그림 9-17: Queue 메시지 payload -- repo, sha, content, timestamp가 JSON으로 들어 있다]
