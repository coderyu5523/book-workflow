...
세 가지 전환점이 이야기를 이끕니다. 1. #strong[CH01] --- LLM 환각을 보고 RAG가 필요하다는 걸 깨닫습니다. 2. #strong[CH06] --- RAG만으로는 부족하고 DB와 문서를 합쳐야 한다는 걸 깨닫습니다. 3. #strong[CH08] --- 기본 RAG로는 한계가 있고 튜닝이 필요하다는 걸 깨닫습니다.
...
#emph[\[이미지: 컨텍스트 넘침\]] #emph[그림 1-2: 문서를 통째로 넣는 방식의 한계. 문서가 늘어나면 프롬프트 창이 넘친다.]
...
#emph[\[이미지: 청킹 비교 결과\]] #emph[그림 1-8: 청킹 여부에 따른 검색 결과 비교. 조각으로 나누면 관련 문서만 정확히 찾는다.]
...
#callout-box([], [한 줄 요약: AI 비서가 조회할 사내 시스템을 실행해보고 구조를 파악한다. API는 웨이터처럼 요청을 받아 DB에서 데이터를 가져다준다. \
...
#grid(
  columns: (1fr, 1fr),
  column-gutter: 12pt,
  auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH02/terminal/02_swagger-ui.png", max-width: 1.0),
  auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH02/terminal/02_api-test-employee.png", max-width: 1.0),
)
...
#callout-box([], [한 줄 요약: AI에게 좋은 답을 원하면 좋은 문서를 넣어야 한다. 도서관처럼 분류하고 라벨을 붙이자. \
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH03/diagram/03_parser-flow.png", alt: [그림 3-2: 파서가 다양한 형식을 Markdown 텍스트로 통일한다. 그래야 청킹하고 검색할 수 있다.], max-width: 0.85)
...
#callout-box([], [한 줄 요약: 문서는 조각내야 찾는다. 손질(파싱), 다지기(청킹), 양념(임베딩), 냉장고(벡터DB). \
...
#callout-box([], [한 줄 요약: 검색은 재료, 답변은 요리다. LCEL 파이프라인이 이 레시피다. \
...
#callout-box([], [한 줄 요약: AI 비서는 안내데스크다. 질문을 듣고, 맞는 담당자를 찾아 정보를 가져온다. \
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH06/diagram/06_sequence-crud.png", alt: [시퀀스 다이어그램 --- 정형 데이터 조회 흐름], max-width: 0.45)
...
#callout-box([], [한 줄 요약: 사서에게 기억력과 업무 일지를 줬다. 같은 질문엔 바로 답하고, 하루 몇 건 처리했는지 기록한다. \
...
#callout-box([], [한 줄 요약: 검색이 바뀌면 답변이 바뀐다. RAG의 품질은 LLM이 아니라 Retriever가 결정한다. \
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH08/08_chunk-size.png", alt: [청크 크기별 비교 결과], max-width: 0.7)
...
#callout-box([], [한 줄 요약: 검색 전에 질문을 바꾸고, 검색 후에 결과를 가공한다. 검색 엔진의 앞뒤를 튜닝하는 장. \
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH09/diagram/09_sequence-pipeline.png", alt: [전체 파이프라인 시퀀스: 질의 준비 → 검색 실행 → 결과 가공 → 최종 답변], max-width: 0.45)
...
#callout-box([], [한 줄 요약: 측정해야 개선할 수 있다. 느낌이 아니라 숫자로 품질을 측정해야 진짜 개선할 수 있다. \
