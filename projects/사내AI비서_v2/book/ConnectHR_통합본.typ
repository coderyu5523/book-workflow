...
#strong[팀장]: "AI로 사내 문서 검색 시스템 만들어봐."

입사 첫 주였습니다. 사수도 없었습니다.

#strong[1일차.] 일단 ChatGPT에 물어봤습니다. "우리 회사 연차 규정이 어떻게 되나요?" AI는 자신 있게 답했습니다. 연차는 15일이고 3일 전까지 신청하면 된다고요. 우리 회사 규정을 열어보니 연차는 입사일 기준으로 달랐고 신청 기한도 일주일 전이었습니다. AI가 그럴듯하게 거짓말을 한 겁니다. 그날 하나를 깨달았습니다. #strong[AI는 우리 회사 문서를 읽은 적이 없습니다.] 그래서 직접 넣어주기로 했습니다. 문서를 통째로 프롬프트에 복사해봤습니다. 작은 문서 하나는 됐습니다. 규정집 200페이지를 복사하니 토큰 한도를 넘겼습니다. 그때 RAG라는 걸 알게 됐습니다. "오픈북 시험"이라고 생각하면 됩니다. AI가 사내 문서 전체를 외울 필요 없이 질문할 때마다 관련 문서만 찾아서 보여주면 됩니다.

#strong[2일차.] 문서를 넣기 전에 AI 비서가 조회할 사내 시스템부터 만들어야 했습니다. "팀원 연차 몇 개 남았어?"는 문서가 아니라 데이터베이스에서 답이 나오는 질문이니까요. 직원 정보와 연차, 매출 테이블을 만들고 API를 열었습니다. 식당에 비유하면 웨이터를 고용한 셈입니다. 손님이 주문하면 주방에서 요리를 가져다주는 것처럼요.

#strong[3일차.] 사내 시스템은 됐습니다. 이제 문서를 넣을 차례입니다. 공유 드라이브를 열었는데 한숨이 나왔습니다. PDF와 DOCX, XLSX, 심지어 HWP까지. 최신 문서와 3년 전 폐기된 문서가 한 폴더에 섞여 있었습니다. (이걸 전부 넣으면?) 쓰레기를 넣으면 쓰레기가 나옵니다. AI도 마찬가지입니다. 도서관을 떠올렸습니다. 사서가 새 책을 받으면 분류하고 라벨을 붙이고 서가에 꽂습니다. 같은 원리입니다. 넣을 문서를 골라내고 형식을 통일하고 메타데이터를 붙이는 것. "신입사원에게 줘도 되는 문서인가?"를 필터링 기준으로 삼았습니다.

#strong[4일차.] 문서를 골랐으면 검색 가능한 지식으로 바꿔야 합니다. 요리에 비유하면 네 단계입니다. 재료를 손질하고(파싱) 먹기 좋게 다지고(청킹) 양념을 입히고(임베딩) 냉장고에 넣습니다(벡터DB). "연차"와 "유급 휴가"가 같은 뜻이라는 걸 숫자 배열로 바꿔야 기계가 알아듣습니다.

#strong[5일차.] 서가가 완성됐습니다. 동료는 서가를 뒤지고 싶어 하지 않았습니다. "검색 결과 다섯 개를 던져주지 말고, 그냥 답을 알려줘." 사서가 책을 찾아와서 읽어서 정리해서 답해주는 게 필요했습니다. 검색은 재료이고 답변은 요리입니다. 레시피를 만들었습니다. 검색, 질문 조합, AI가 읽고 답변, 정리. 이 네 단계를 하나의 흐름으로 연결했습니다. 출처 없는 답변은 근거 없는 주장이니까요. "어떤 문서에서 이 답을 찾았는지" 반드시 보여주게 만들었습니다. 한 번 물어보고 끝이 아니라 대화를 이어갈 수 있는 기억력도 붙여줬습니다. 드디어 질문하면 답해주는 AI 비서가 된 겁니다.

#strong[6일차.] 동료가 이런 질문을 했습니다. "A 사원 연차 몇 개? 그리고 연차 신청 절차는?" 한 문장에 두 가지가 섞여 있었습니다. "연차 몇 개"는 데이터베이스를 조회해야 하고 "신청 절차"는 문서를 검색해야 합니다. RAG만으로는 절반밖에 답할 수 없었습니다. AI 비서를 "안내데스크"로 바꿔야 했습니다. 질문이 들어오면 유형을 분류하고 맞는 담당자를 호출하는 구조입니다. DB를 조회하는 담당자, 문서를 검색하는 담당자, 둘 다 호출해서 합치는 담당자. 이렇게 해서 #strong[ConnectHR] 이 태어났습니다. 정형 데이터와 비정형 문서를 한번에 다루는 사내 AI 비서입니다.

#strong[7일차.] ConnectHR을 동료에게 풀어줬습니다. 처음에는 신기해했습니다. 같은 질문을 할 때마다 20초씩 기다려야 했습니다. "아까도 물어봤는데 또 기다려?" 사서에게 기억력을 줬습니다. 한 번 답한 내용은 메모해두고 같은 질문이 오면 메모를 보여줍니다. 업무 일지도 쥐어줬습니다. 하루에 토큰을 얼마나 쓰는지, 비용은 얼마인지, 느린 질문은 어떤 건지.

#strong[8일차.] 운영은 안정됐습니다. 진짜 문제가 터졌습니다. "병가 규정을 물어봤는데 출장 규정이 나왔다." 검색이 엉뚱한 문서를 가져오고 있었습니다. LLM 탓이 아니었습니다. 사서가 서가에서 잘못된 책을 꺼내온 겁니다. 사서의 검색 실력을 키워야 했습니다. 가위로 일정하게 자르던 걸 의미 단위로 바꾸고 검색 결과를 심사위원에게 다시 줄 세우게 하고 키워드 검색과 의미 검색을 합쳤습니다. 같은 질문인데 답이 달라졌습니다.

#strong[9일차.] 검색이 좋아지니 이번엔 질문이 문제였습니다. 동료가 "WFH 정책 알려줘"라고 물었는데 ConnectHR이 멈칫했습니다. 문서에는 "재택근무"라고 적혀 있었으니까요. "휴가 규정 알려줘"도 마찬가지였습니다. 문서에는 "연차유급휴가"라고 돼 있었습니다. 사서에게 언어 능력을 줬습니다. 약어를 풀어쓰고 동의어를 확장하고 한 질문을 여러 표현으로 바꿔서 각각 검색하게 했습니다. 답변에 근거도 붙였습니다. "이 문서의 3페이지에서 찾았습니다"라고 원본 이미지와 함께.

#strong[10일차.] 마지막 관문이 남았습니다. 조직도 PDF를 넣었는데 텍스트만 나왔습니다. 박스와 화살표가 사라지고 "대표이사경영지원본부기술개발본부"가 한 줄로 붙었습니다. 사서에게 눈을 달아줘야 했습니다. 이미지를 보고 의미를 이해하는 능력을 준 겁니다. 성적표도 만들었습니다. "찾아온 문서 중 몇 개가 정답이었나", "정답 문서를 몇 개나 찾았나", "AI가 지어낸 답변은 없었나". 이 지표로 ConnectHR의 실력을 숫자로 측정했습니다. 튜닝 전과 후의 차이가 눈에 보였습니다. 어디를 더 개선해야 하는지도 보이기 시작했습니다.

== 한눈에 보기

#table(
    columns: (1fr, 1fr, 2fr),
    table.header([챕터], [핵심 질문], [끝나면 뭐가 보이나],),
    ...
    [CH06], [DB 조회와 문서 검색을 한 번에 하려면?], [#strong[ConnectHR 탄생]],
    [CH07], [캐시, 비용 추적, 에러 처리는?], [운영 안정화],
    [CH08], [왜 엉뚱한 문서를 가져올까?], [튜닝 전후 비교],
    [CH09], [같은 뜻인데 왜 못 찾을까?], [쿼리 변환 + 근거],
    [CH10], [PDF 이미지는? 품질은 어떻게 측정할까?], [#strong[성적표 + v1.0 완성]],
  )

세 가지 전환점이 이야기를 이끕니다.

+ #strong[CH01] --- LLM 환각을 보고 RAG가 필요하다는 걸 깨닫습니다.
+ #strong[CH06] --- RAG만으로는 부족하고 DB와 문서를 합쳐야 한다는 걸 깨닫습니다.
+ #strong[CH08] --- 기본 RAG로는 한계가 있고 튜닝이 필요하다는 걸 깨닫습니다.

모든 챕터는 #strong[실패에서 시작] 합니다. 에러를 만나고 원인을 찾고 해결하는 과정이 곧 학습입니다.
...
#table(
    columns: 3,
    table.header([항목], [권장 사양], [비고],),
    [#strong[Python]], [3.10 \~ 3.12], [3.13 이상은 일부 호환성 문제 가능],
    [#strong[RAM]], [16GB 이상], [임베딩 모델 로딩 시 8GB 이상 사용],
    [#strong[디스크]], [10GB 여유], [모델 파일 + Docker 이미지],
    [#strong[Docker]], [Docker Desktop 설치], [PostgreSQL 실행에 필요],
    [#strong[LLM]], [Ollama + llama3.1:8b], [로컬에서 무료로 실행],
    [#strong[Vision LLM]], [Ollama + qwen2.5vl:7b], [CH10 이미지 파싱에 필요],
  )

CH06 이후 에이전트 기능(Tool Calling)을 사용하려면 Ollama 기준 `llama3.1:8b` 이상 모델이 필요합니다. CH10에서 PDF 이미지를 분석하려면 비전 모델 `qwen2.5vl:7b` 이 필요합니다.

#strong[로컬 환경이 어려운 경우] OpenAI API(gpt-4o-mini)를 대안으로 쓸 수 있습니다. 각 예제의 `.env` 파일에서 `LLM_PROVIDER=openai` 로 변경하고 API 키만 설정하면 동일하게 동작합니다. Ollama 설치나 GPU 없이도 모든 챕터를 진행할 수 있습니다.

자세한 환경 설정 방법은 #strong[부록 A. 환경 설정] 을 참고하세요.
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH02/diagram/02_api-waiter.png", alt: [그림 2-1: API는 식당의 웨이터다. 손님(프론트엔드)과 주방(DB)을 연결한다.], max-width: 0.7)

우리가 실행해볼 사내 시스템도 똑같습니다. 나중에 AI 비서가 손님 역할을 맡게 돼요. "팀원 연차 몇 개?"라고 물으면 API(웨이터)가 DB(주방)에서 찾아다 줍니다.

#callout-box([], [#strong[참고: AI는 어떻게 API를 호출할까?] 사람이 UI에서 버튼을 누르듯 AI 비서도 API를 호출합니다. CH06에서 #strong[MCP(Model Context Protocol)] 라는 도구를 통해 AI가 직접 API를 호출하는 법을 다룹니다. 지금은 "AI가 쓸 시스템을 먼저 확인해두는 것"에 집중하겠습니다.])
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH02/gemini/02_crud-menu.png", alt: [그림 2-2: API의 메뉴판. 네 가지 동작이면 거의 모든 데이터를 다룰 수 있다.], max-width: 0.45)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH02/gemini/02_erd-diagram.png", alt: [그림 2-3: 사내 시스템의 세 테이블. 직원을 중심으로 연차가 연결되고, 매출은 부서 단위로 독립 관리된다.], max-width: 0.45)
...
%%DUAL\_IMAGE\_START%% #auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH02/terminal/02_swagger-ui.png", max-width: 0.45) #auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH02/terminal/02_api-test-employee.png", max-width: 0.45) %%DUAL\_IMAGE\_END%%
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH02/terminal/02_admin-dashboard.png", alt: [그림 2-6: Jinja2 템플릿으로 만든 관리자 대시보드. 직원, 연차, 매출 현황을 한눈에 볼 수 있다.], max-width: 0.7)

직원 관리 메뉴에서 사번과 이름, 부서, 직급, 입사일을 입력하고 등록하면 아래 목록에 바로 나타납니다. 기존 직원 5명에서 홍길동 사원이 추가된 걸 확인할 수 있어요.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH02/terminal/02_admin-employee-create.png", alt: [그림 2-7: 웹 UI에서 직원을 등록하면 목록에 바로 반영된다. API를 몰라도 CRUD가 된다.], max-width: 0.65)
...
= Ch.3: "어떤 문서를 넣을까" --- 사내 문서 수집 전략과 표준 (ex03)

#callout-box([], [이번 버전: ex02 → ex03 \
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH03/gemini/03_chapter-opening.png", alt: [공유 드라이브의 문서 더미], max-width: 0.4)

=== 1.1 "이걸 다 넣어야 해?"

CH02에서 사내 시스템은 만들었습니다. 직원, 연차, 매출 데이터는 API로 조회할 수 있고요. 이제 AI 비서의 나머지 절반인 #strong[문서 검색] 을 본격적으로 준비할 차례입니다.

CH01에서 더미 문서 3개로 RAG를 돌려봤습니다. 잘 됐죠. 그런데 이제 실제 사내 문서를 넣어야 합니다.
...
사내 문서도 마찬가지예요. 벡터 DB에 넣기 전에 #strong[분류하고 라벨을 붙이고 정리하는 단계] 가 필요합니다. 이 단계를 건너뛰면 검색 품질이 엉망이 됩니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH03/gemini/03_document-pipeline.png", alt: [그림 3-1: 사내 문서를 벡터 DB에 넣기까지. 정리 없이 넣으면 검색 품질이 떨어진다.], max-width: 0.7)

=== 1.3 첫 번째 원칙: 넣을 문서와 뺄 문서
...
#callout-box([], [#strong[Tip]: 반드시 Markdown이어야 하는 건 아닙니다. 일반 텍스트(plain text)나 JSON으로 변환해도 벡터 DB에 넣을 수 있습니다. 다만 Markdown은 제목·표·목록 같은 #strong[문서 구조를 보존하면서도 가볍다]는 점에서 RAG 파이프라인에 가장 널리 쓰입니다.])

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH03/diagram/03_parser-flow.png", alt: [그림 3-2: 파서가 다양한 형식을 Markdown 텍스트로 통일한다. 그래야 청킹하고 검색할 수 있다.], max-width: 0.4)

=== 1.5 세 번째 원칙: 라벨이 없으면 검색할 수 없다

도서관에서 책에 붙이는 정보가 있죠. 청구기호, 저자, 분야, 출판년도. 문서 세계에서는 이걸 #strong[메타데이터] 라고 부릅니다.
...
=== 1.6 네 번째 원칙: 잘라야 찾는다 --- 청킹 전략

CH01에서 이미 경험했습니다. 문서를 통째로 넣으면 검색 정확도가 떨어져요. 적절한 크기로 #strong[조각(chunk)] 내야 합니다.

조각 크기를 어떻게 정할까요?

너무 작으면 문맥이 잘립니다. "신입사원은 3년 동안 연차가 없다" 다음 줄에 "대신 리프레시 데이를 제공한다"가 있는데 잘못 자르면 "연차가 없다"만 나와요.
...
#callout-box([], [이 책에서는 CH04에서 #strong[고정 크기(500자 + 100자 오버랩)] 으로 시작하고, CH08(검색 품질 튜닝)에서 의미 기준 청킹과 비교 실험을 합니다.])

=== 1.7 다섯 번째 원칙: 문서는 계속 바뀐다 --- 재인덱싱 전략
...
#strong[전체 재인덱싱] --- 모든 문서를 지우고 처음부터 다시 넣습니다. 간단하지만 시간이 오래 걸려요.

#strong[증분 재인덱싱] --- 변경된 문서만 업데이트합니다. 빠르지만 "어떤 문서가 변경됐는지" 추적해야 합니다.

#callout-box([], [이 책에서는 문서 수가 적으므로 #strong[전체 재인덱싱]으로 충분합니다. 문서가 수천 개 이상이면 증분 방식을 고려합니다.])
...
#callout-box([], [HWP, PPT는 이 책에서 다루지 않습니다. 가능하면 PDF로 변환 후 사용하세요.])
...
#callout-box([], [#strong[Tip]: 메타데이터 필터링은 벡터 DB마다 문법이 다릅니다. ChromaDB는 `where={"key": "value"}` 딕셔너리 방식이고 Pinecone은 `filter={"key": {"$eq": "value"}}` 처럼 MongoDB 스타일 연산자를 씁니다. PostgreSQL 기반 pgvector는 아예 SQL `WHERE` 절로 필터링하고요. 문법만 다를 뿐 "메타데이터로 검색 범위를 좁힌다"는 개념은 동일합니다.])
...
= Ch.4: "문서를 '지식'으로 바꾸다" --- VectorDB 구축 (ex04)

#callout-box([], [이번 버전: ex03 → ex04 \
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH04/gemini/04_chapter-opening.png", alt: [요리 준비 --- 문서를 지식으로], max-width: 0.4)

=== 1.1 "정리는 했는데, 어떻게 넣지?"

CH03에서 사내 문서를 정리했습니다. 분류도 했고 메타데이터 라벨도 붙였죠. 이제 이 문서들을 벡터 DB에 넣어서 AI 비서가 검색할 수 있게 만들 차례입니다.

그런데 문서를 그냥 통째로 넣으면 될까요? CH01에서 이미 경험했습니다. 더미 문서 3개는 괜찮았어요. 하지만 실제 사내 문서는 다릅니다. 취업규칙만 해도 수십 페이지인데, 이걸 통째로 넣으면 "연차 몇 일이야?"라고 물었을 때 보안규정이랑 매출 현황까지 딸려옵니다.

문서를 #strong[검색 가능한 지식] 으로 바꾸려면 몇 단계를 거쳐야 합니다.
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH04/gemini/04_vectordb-pipeline.png", alt: [그림 4-1: 문서를 벡터 DB에 넣는 과정은 요리의 손질 → 다지기 → 양념 → 냉장고 정리와 같다.], max-width: 0.65)

=== 1.3 1단계: 손질 --- 문서에서 텍스트 꺼내기 (파싱)

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH04/gemini/04_example_hr_rules.png", alt: [취업규칙 PDF 원본 확인], max-width: 0.45)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH04/diagram/04_parser-select.png", alt: [그림 4-2: 파일 확장자에 따라 적절한 파서를 선택한다. 통합 함수 하나로 자동 분기.], max-width: 0.7)
...
=== 1.5 3단계: 양념 --- 의미를 숫자로 바꾸기 (임베딩)

여기가 마법이 일어나는 곳입니다.

"연차 사용 규정"이라는 텍스트를 컴퓨터가 이해할 수 있을까요? 컴퓨터는 글자를 모릅니다. 숫자만 알아요.

#strong[임베딩(Embedding)] 은 텍스트의 #strong[의미] 를 숫자 벡터(768개의 숫자 리스트)로 바꾸는 과정입니다. 여기서 핵심은 단순히 글자를 숫자로 바꾸는 게 아니라 #strong[의미가 비슷한 텍스트는 비슷한 숫자] 가 된다는 점입니다.

"연차 사용 규정" → \[0.12, -0.34, 0.87, …\] "휴가 관련 정책" → \[0.11, -0.33, 0.85, …\] ← 비슷! "매출 현황 보고서" → \[-0.45, 0.22, -0.11, …\] ← 완전 다름

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH04/gemini/04_embedding-concept.png", alt: [그림 4-3: 임베딩은 의미가 비슷한 텍스트를 가까운 좌표에 배치한다. "연차"를 검색하면 의미상 가까운 문서들이 먼저 발견된다.], max-width: 0.45)

벡터 검색의 핵심이 바로 이겁니다. "연차 사용 규정"을 검색하면 숫자가 비슷한 "휴가 관련 정책" 청크를 찾아올 수 있습니다. 키워드가 정확히 일치하지 않아도 의미가 가까우면 찾아내요.

그런데 "숫자가 비슷하다"는 걸 어떻게 판단할까요? 768개나 되는 숫자를 하나하나 비교할 수는 없습니다.

여기서 #strong[코사인 유사도(Cosine Similarity)] 라는 방법을 씁니다. 벡터를 좌표 위의 #strong[화살표] 라고 생각해보세요. 두 화살표가 같은 방향을 가리키면 의미가 비슷한 거고, 반대 방향이면 의미가 다른 겁니다. 코사인 유사도는 이 #strong[두 화살표 사이의 각도] 를 측정합니다.

- 같은 방향(각도 0°) → 유사도 #strong[1] (완전히 같은 의미)
- 직각(90°) → 유사도 #strong[0] (관련 없음)
- 반대 방향(180°) → 유사도 #strong[\-1] (반대 의미)

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH04/gemini/04_cosine-similarity.png", alt: [그림 4-4: 코사인 유사도의 각도에 따른 값의 변화], max-width: 0.7)
...
임베딩 모델은 여러 가지가 있습니다. OpenAI의 text-embedding-ada-002도 있고 다국어 모델도 있어요. 우리는 #strong[ko-sroberta-multitask] 를 선택했습니다.
...
#callout-box([], [CH08(검색 품질 튜닝)에서 다른 임베딩 모델과 비교 실험을 해봅니다. 지금은 ko-sroberta로 시작하고, 나중에 더 나은 선택지가 있는지 확인합니다.])

=== 1.6 4단계: 냉장고 정리 --- 벡터 DB에 저장

양념까지 끝난 재료를 냉장고에 정리합니다. 라벨 붙이고 구분해서 나중에 바로 꺼낼 수 있게.

#strong[ChromaDB] 가 우리의 냉장고입니다. CH01에서 이미 써봤지만 그때는 `Chroma.from_documents()` 한 줄로 끝냈죠. 이번에는 직접 넣습니다.

ChromaDB에 저장하는 항목은 네 가지입니다.
...
#callout-box([], [기본 환경(Python 3.12)이 없다면 #strong[부록(환경 설정)] 을 먼저 참고하세요.])
...
#callout-box([], [ko-sroberta-multitask 모델은 최초 실행 시 HuggingFace에서 자동 다운로드됩니다 (약 400MB). 이후에는 로컬 캐시를 사용합니다.])
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH04/diagram/04_exercise-flow.png", alt: [실습 순서], max-width: 0.85)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH04/terminal/04_pipeline-result.png", alt: [그림 4-5: 6개 사내 문서가 마크다운으로 변환된 뒤, 18개 청크로 잘려 벡터 DB에 저장됐다.], max-width: 0.45)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH04/terminal/04_cli-search.png", alt: [그림 4-6: "휴가 규정"을 검색하자 취업규칙에서 글로벌 노마드, 스마트 휴가 승인 등의 관련 내용을 찾아냈다.], max-width: 0.45)
...
#callout-box([], [#strong[참고: 검색 결과는 다를 수 있습니다] 임베딩 모델의 버전이나 PC 환경 등에 따라 검색된 청크의 순서나 유사도 수치(%)는 책의 예시와 조금 다르게 나올 수 있습니다. 최상위(Top 1\~3) 검색 결과에 취업규칙의 휴가/연차 관련 내용이 잘 나왔다면 정상적으로 벡터 DB가 구축된 것입니다.])
...
#callout-box([], [청크 크기와 오버랩을 바꾸면 검색 결과가 달라집니다. `--chunk-size 300 --overlap 50` 으로 다시 돌려보고, 검색 결과가 어떻게 바뀌는지 직접 비교해 보세요. CH08(검색 품질 튜닝)에서 이 실험을 본격적으로 다룹니다.])
...
+ #strong[메타데이터 정제] --- ChromaDB는 메타데이터에 `None` 이나 `bool` 을 허용하지 않습니다. `None` → 빈 문자열, `bool` → 문자열로 변환해야 해요. 빠뜨리면 저장할 때 에러가 납니다.
+ #strong[`normalize_embeddings=True`] --- 벡터를 단위 벡터로 정규화합니다. 코사인 유사도 계산에 필요하거든요.
...
= Ch.5: "드디어 답해준다" --- RAG Q&A 엔진 (ex05)

#callout-box([], [이번 버전: ex04 → ex05 \
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH05/gemini/05_chapter-opening.png", alt: [사서가 답해준다], max-width: 0.4)
...
검색 결과 5개를 받아서 직접 읽고 "아, 3일 미만이면 증빙 불필요고 3일 이상이면 진단서가 필요하구나"라고 해석하는 건 결국 사람 몫이었습니다. 벡터 검색은 #strong[재료를 찾아주는 것]이지 #strong[요리를 해주는 것]이 아니었어요.

이번 챕터에서는 그 "요리"를 해줄 #strong[RAG Q&A 엔진]을 만들겠습니다. 질문하면 문서를 검색하고 검색 결과를 읽어서 자연어로 답변해주는 시스템이에요.

=== 1.2 도서관 사서가 필요하다

지금까지 만든 벡터 검색은 #strong[도서관의 검색 시스템]과 비슷합니다. "한국 역사"를 검색하면 관련 책이 어디 있는지 알려주죠. 하지만 그 책을 직접 꺼내서 읽어보고 핵심을 정리해서 답해주지는 않습니다.

우리에게 필요한 건 #strong[사서]입니다.

사서에게 "조선시대 과거 제도가 뭐야?"라고 물으면 이런 일이 벌어집니다.
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH05/diagram/05_rag-qa-flow.png", alt: [RAG Q&A 흐름], max-width: 0.7)
...
=== 1.3 파이프라인이라는 이름의 레시피

사서가 일하는 순서를 코드로 옮기려면 어떻게 해야 할까요? 검색 → 프롬프트 → LLM → 파싱, 각 단계를 직접 이어 붙여도 되지만 이걸 편하게 해주는 도구가 있습니다. #strong[LangChain] 입니다. 검색, 프롬프트, LLM 호출 같은 단계를 부품으로 제공하고 이 부품을 조립할 수 있게 해주는 Python 프레임워크예요. RAG 시스템을 만들 때 가장 널리 쓰입니다.

LangChain은 이 조립을 #strong[파이프(|) 연산자]로 합니다. LCEL(LangChain Expression Language)이라고 부르죠.

파이프는 주방의 레시피와 같습니다.

#callout-box([], [질문 → #strong[검색]\(벡터 DB에서 문서 조각 찾기) → #strong[프롬프트]\(찾은 문서 + 질문을 합치기) → #strong[LLM]\(읽고 답변 생성) → #strong[파싱]\(답변 텍스트 추출)])

각 단계가 파이프(|)로 연결되고, 앞 단계의 출력이 다음 단계의 입력이 됩니다. 요리 레시피 순서와 똑같아요.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH05/diagram/05_lcel-pipeline.png", alt: [LCEL 파이프라인 구조], max-width: 0.7)
...
CH01에서 LLM이 그럴듯하게 거짓말하는 걸 봤습니다. RAG로 문서를 넣어줬다고 환각이 완전히 사라지는 건 아니에요. LLM은 문서에 없는 내용을 지어내기도 합니다.

그래서 프롬프트에 규칙을 넣습니다.

#callout-box([], ["반드시 제공된 문서에서만 답하세요. 답변 마지막에 출처를 명시하세요. 문서에서 찾을 수 없으면 '확인되지 않습니다'라고 답하세요."])
...
사람 사서라면 당연히 기억합니다. 하지만 LLM은 기본적으로 #strong[기억력이 없습니다]. 매 요청이 독립적이라 이전에 뭘 물어봤는지 모릅니다.

그래서 #strong[대화 히스토리]를 직접 관리해야 합니다. 이전 대화를 메모해뒀다가 새 질문이 올 때마다 같이 넘겨주는 거예요. 다만 모든 대화를 다 기억할 수는 없으니 #strong[최근 5턴만 유지]하는 슬라이딩 윈도우 방식을 씁니다.

도서관 사서가 메모장을 들고 있다고 생각하면 됩니다. 새 메모가 들어오면 가장 오래된 메모를 지우고 항상 최근 5장만 남겨요.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH05/gemini/05_sliding-window.png", alt: [그림 5-3: 메모장에 6번째 메모가 들어오면 가장 오래된 1번째가 빠진다. 항상 최근 5장만 유지.], max-width: 0.65)
...
ex04에서 재료를 다 모았습니다. 이번 챕터(ex05)에서 드디어 #strong[요리]해요. 검색만 하던 시스템이 답변을 해줍니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH05/gemini/05_rag-qa-result.png", alt: [그림 5-4: 드디어 답해준다. 문서를 검색하고, 읽어서, 출처와 함께 자연어로 답변한다.], max-width: 0.6)
...
#callout-box([], [기본 환경(Python 3.12, Ollama)이 없다면 #strong[부록(환경 설정)] 을 먼저 참고하세요.])
...
#callout-box([], [#strong[팁: LLM 선택] --- 기본값은 Ollama + `deepseek-r1:8b`입니다. 이후 챕터 부터는`.env`에서 `LLM_PROVIDER=openai`로 바꾸면 GPT-4o-mini도 쓸 수 있습니다. (단, API 비용이 발생합니다. .env 파일에 OPENAI\_API\_KEY=sk-xxxxxx 형태로 key를 등록해서 사용하세요.)])
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH05/diagram/05_exercise-flow.png", alt: [실습 순서], max-width: 0.85)
...
#callout-box([], [#strong[주의: 메모리 기반입니다] `ConversationManager`의 세션 데이터는 서버 메모리에만 존재합니다. 서버를 재시작하면 대화 히스토리가 사라져요. 실전 운영 환경에서는 Redis 같은 외부 저장소를 쓰는 게 일반적입니다. (CH07에서 운영을 위한 캐시 개념을 배우지만, 실습 환경의 복잡도를 낮추기 위해 외부 서비스(Redis) 연동 대신 인메모리 방식을 유지합니다.)])

#callout-box([], [#strong[팁: 다른 메모리 방식도 있습니다] 슬라이딩 윈도우(최근 N턴 유지)는 가장 단순한 방식입니다. 대화가 길어지면 앞부분이 통째로 사라지는 단점이 있죠. 다른 접근도 있습니다. - #strong[Summary Memory] --- LLM이 이전 대화를 요약해서 저장. 긴 대화도 맥락을 유지하지만 요약할 때마다 LLM을 한 번 더 호출해요. - #strong[Token Buffer Memory] --- 턴 수가 아니라 토큰 수 기준으로 제한. LLM의 컨텍스트 창을 효율적으로 사용합니다. - #strong[Summary Buffer Memory] --- 최근 대화는 원문 그대로, 오래된 대화는 요약. 위 두 방식의 절충안이에요.

이 책에서는 구현이 간단하고 LLM 추가 호출이 없는 슬라이딩 윈도우를 씁니다.])

=== 2.9 실행 결과

핵심 코드를 모두 살펴봤습니다. 이제 서버를 실행하고 브라우저에서 직접 질문해 봅시다.
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH05/terminal/05_server-start.png", alt: [그림 5-5: 서버가 시작되면 채팅 UI 주소가 함께 출력된다.], max-width: 0.7)

브라우저에서 `http://localhost:8000/chat`을 엽니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH05/terminal/05_chat-ui.png", alt: [그림 5-6: 브라우저에서 바로 질문할 수 있는 채팅 창구. 사서가 자리에 앉았다.], max-width: 0.6)

"병가 쓸 때 증빙 서류가 필요한가요?"를 입력하고 잠시 기다립니다.

#callout-box([], [#strong[참고: 첫 답변 대기 시간] 로컬 환경에서 모델을 처음 메모리에 올리고 추론하는 과정에서 약 60\~120초 정도 시간이 소요될 수 있습니다. 응답이 올 때까지 조금만 기다려주세요!])

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH05/terminal/05_chat-response.png", alt: [그림 5-7: 드디어 답해준다. 문서를 검색하고, 읽어서, 출처와 함께 자연어로 답변한다.], max-width: 0.65)

이어서 "그러면 연차로 대체할 수 있어?"를 입력해 봅니다. 이전 대화를 기억하고 병가 맥락을 이어서 답변해요. 사서가 메모장을 보고 있다는 뜻이죠.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH05/terminal/05_chat-response-followup.png", alt: [그림 5-8: 이전 대화 맥락(병가)을 기억하고 답변을 이어나간다.], max-width: 0.65)

#callout-box([], [`Ctrl + C`를 눌러 서버를 종료합니다.])
...
= Ch.6: "'연차 몇 개, 규정은?' 동시에" --- 통합 에이전트 설계 (ex06)

#callout-box([], [이번 버전: ex05 → ex06 \
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH06/gemini/06_chapter-opening.png", alt: [안내데스크 --- 통합 에이전트], max-width: 0.4)

=== 1.1 RAG가 답 못 하는 질문

CH05에서 RAG Q&A 엔진을 완성했습니다.

질문을 입력하면 사내 문서에서 내용을 찾아 답해주는 엔진입니다. 출처까지 같이 나오니 꽤 만족스러웠습니다.

옆자리 동료가 다가옵니다.

#strong[동료]: "이게 그 AI 비서예요? 한번 써봐도 되죠?"

#strong[나]: "당연하죠. 뭐든 물어보세요."
...
RAG는 사내 문서에서 'A 사원'을 찾으려고 했습니다. 당연히 없죠. 직원 연차 정보는 문서가 아니라 DB에 있으니까요. CH02에서 PostgreSQL에 저장해둔 데이터입니다.

문서 검색이 아니라 DB 조회가 필요한 질문인데 AI 비서는 그 차이를 몰랐습니다.
...
회사에 처음 입사한 날을 떠올려 봅니다.

낯선 건물에 들어서자마자 1층 안내데스크로 향했습니다.
...
질문을 보고 DB가 필요한지 문서가 필요한지 아니면 둘 다인지 판단합니다. 판단이 끝나면 각 담당자(도구)에게 작업을 맡기고 결과를 하나로 묶어서 답해줍니다.

이번 챕터에서 만들 #strong[통합 에이전트] 가 바로 이 안내데스크입니다.

=== 1.4 안내데스크: QueryRouter
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH06/gemini/06_query-router.png", alt: [그림 6-1: 대부분의 질문은 1단계 키워드 매칭에서 끝난다. 모호할 때만 더 비싼 방법을 쓴다.], max-width: 0.7)

=== 1.5 담당자들: 도구(Tool)

라우터가 방향을 정하면 에이전트가 도구 목록에서 맞는 것을 골라 씁니다.
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH06/diagram/06_tool-vs-mcp.png", alt: [Tool vs MCP 방식 비교], max-width: 0.65)

핵심 개념은 같습니다. "AI가 함수의 이름과 설명을 보고 스스로 호출한다." 이 책에서는 `@tool` 로 원리를 익히겠습니다. 원리를 이해하면 MCP로 확장하는 건 어렵지 않습니다.

각 도구는 `@tool` 데코레이터로 선언되어 있습니다.
...
=== 1.6 생각하고, 행동하고, 확인한다: ReAct

도구가 준비됐습니다. 이제 에이전트가 이 도구를 어떻게 쓰는지 살펴보겠습니다.

에이전트의 두뇌는 #strong[ReAct 패턴] 으로 동작합니다. Reason(추론) + Act(행동). 이름이 거창해 보이지만 원리는 단순해요. 생각하고 행동하는 걸 반복합니다.
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH06/diagram/06_react-flow.png", alt: [그림 6-2: 질문이 들어오면 QueryRouter가 분류하고, ReAct 에이전트가 필요한 도구를 순서대로 호출한다.], max-width: 0.7)

=== 1.7 이제 복합 질문에 답할 수 있다
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH06/terminal/06_chat-hybrid-demo.png", alt: [그림 6-3: 한 번의 질문으로 안내데스크가 담당자들을 알아서 찾아 정보를 조합해준다.], max-width: 0.45)
...
#callout-box([], [기본 환경(Python 3.12, Ollama, Docker)이 없다면 #strong[부록(환경 설정)] 을 먼저 참고하세요.])
...
#callout-box([], [#strong[이전 챕터 Docker 종료]: CH02의 Docker가 실행 중이라면 `cd ex02 && docker compose down` 으로 먼저 종료하세요. 같은 포트를 사용합니다.])
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH06/diagram/06_exercise-flow.png", alt: [실습 순서], max-width: 0.85)
...
에이전트가 도구를 무작정 고르는 것보다 질문의 의미를 먼저 파악하고 방향을 잡아주면 성능이 훨씬 좋아집니다. 이를 위해 3단계로 질문을 분류하는 `QueryRouter` 를 작성합니다.

아래 코드를 `ex06/src/router.py`에 단계별로 나누어 작성합니다.
...
`mcp_tools.py` 에서 데이터를 다루는 인프라 함수는 `db_helper.py` 에 분리해 두었습니다.
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH06/terminal/06_chat-initial.png", alt: [그림 6-2: 로컬호스트로 구동된 에이전트 채팅 인터페이스 화면], max-width: 0.65)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH06/terminal/06_admin-employees.png", alt: [그림 6-3: 관리자 대시보드 --- 직원 관리 화면. 사번은 자동 생성된다.], max-width: 0.65)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH06/terminal/06_chat-crud-result.png", alt: [그림 6-4: 방금 등록한 신입사원의 정보를 에이전트가 DB에서 찾아 답한다.], max-width: 0.65)

방금 대시보드에서 등록한 데이터를 에이전트가 찾아냈습니다. 무슨 일이 벌어진 건지 시퀀스 다이어그램으로 뜯어보겠습니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH06/diagram/06_sequence-crud.png", alt: [시퀀스 다이어그램 --- 정형 데이터 조회 흐름], max-width: 0.7)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH06/terminal/06_hybrid-response.png", alt: [그림 6-7: 연차 잔여(DB)와 신청 규정(문서)을 동시에 가져온다.], max-width: 0.6)
...
= Ch.7: "실제로 써보니" --- 운영을 위한 안정화 (ex07)

#callout-box([], [이번 버전: ex06 → ex07 \
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH07/gemini/07_chapter-opening.png", alt: [운영 안정화 --- 캐시와 모니터링], max-width: 0.4)

=== 1.1 "왜 이렇게 느려?"

CH06에서 통합 에이전트를 완성했습니다. 동료들도 만족했고요. DB 조회도 되고 문서 검색도 되고 출처까지 붙여서 답변합니다.

그런데 일주일쯤 지나자 불만이 들어오기 시작합니다.

#strong[동료 A]: "병가 증빙 필요한지 아까도 물어봤는데, 또 물어보니까 또 20초 걸리네요."

#strong[동료 B]: "우리 팀에서 이거 하루에 30번은 쓰는데, 비용이 얼마나 나가는 건지 모르겠어요."

같은 질문을 반복해도 매번 LLM을 호출합니다. 로컬 모델 기준 10\~20초씩 걸리고, API를 쓰면 돈도 듭니다. 누가 얼마나 쓰는지 추적할 방법도 없었습니다.

문제는 또 있습니다. LLM이 가끔 에러를 냅니다. 네트워크 타임아웃이나 파싱 실패 같은 것인데, ex06에서는 에러가 나면 그대로 멈춰버렸습니다.

=== 1.2 사서에게 메모장을 하나 더 줬다

CH05에서 사서에게 대화용 메모장(WindowMemory)을 줬습니다. 최근 5턴을 기억하는 메모장이었죠.

이번에는 다른 종류의 메모장을 추가합니다.

#strong[답변 메모장 (ResponseCache)]

누군가 같은 질문을 또 했습니다. 사서가 이미 한 번 찾아본 자료라면 서가까지 다시 갈 필요가 없겠죠. 메모장에 적어둔 답을 바로 읽어주면 됩니다.

다만 메모장에는 유통기한이 있습니다. 1시간이 지나면 메모를 지워요. 사내 규정이 바뀌었을 수도 있으니까요. 이걸 #strong[TTL(Time To Live)] 이라고 부릅니다.
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH07/gemini/07_cache-concept.png", alt: [그림 7-1: 같은 질문이 또 오면 LLM을 다시 부르지 않는다. 같은 텍스트를 또 임베딩하지 않는다.], max-width: 0.65)
...
실무에서는 #strong[Langfuse] 나 LangSmith 같은 전문 모니터링 도구가 이 역할을 합니다. 호출별 토큰 수와 비용, 지연 시간을 자동으로 수집해서 대시보드로 보여주죠. 하지만 도구를 붙이기 전에 "무엇을 측정해야 하는지"부터 이해해야 합니다.

그래서 이 책에서는 #strong[TokenTracker] 라는 간단한 클래스를 직접 만들어 봅니다. 매 호출마다 한 줄씩 기록이 쌓이고 나중에 "이번 달 토큰 비용이 얼마야?"라고 물으면 바로 답할 수 있어요. 도서관 이용 통계처럼 운영을 개선하려면 먼저 측정부터 해야 합니다.

=== 1.4 재시도도 배웠다

도서관 사서가 서가에서 자료를 꺼내려는데 하필 그 선반이 점검 중입니다. ex06의 사서는 "지금 못 찾겠어요"라고 말하고 돌아섰습니다. 이용자는 같은 질문을 처음부터 다시 해야 했죠.

LLM도 마찬가지입니다. 네트워크 오류나 타임아웃, 파싱 실패 등으로 가끔 응답을 못 합니다. ex06에서는 에러가 나면 그대로 멈췄습니다.

이제 사서에게 규칙을 하나 알려줬습니다. "한 번 실패하면 #strong[3번까지 다시 시도해.] 시도 사이에는 2초씩 쉬고." 점검 중이던 선반도 잠깐 뒤에 다시 가면 열려 있을 수 있으니까요.

이렇게 캐시와 업무 일지, 재시도까지 운영에 필요한 안정 장치를 모두 갖추게 됐습니다.
...
새 기능을 추가하는 게 아닙니다. 기존 에이전트를 #strong[안정적으로 운영] 할 수 있게 감싸는 것이죠.

도구도 한 가지 바뀝니다. ex06에서 `mcp_tools.py` 하나에 몰아넣었던 도구 4개를 `tools/` 디렉토리 아래 파일 하나씩으로 분리했습니다. 도구가 늘어나도 관리가 수월해집니다.
...
#callout-box([], [#strong[이전 챕터 Docker 종료]: CH06의 Docker가 실행 중이라면 `cd ex06 && docker compose down` 으로 먼저 종료하세요.])
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH07/diagram/07_exercise-flow.png", alt: [실습 순서], max-width: 0.85)
...
#callout-box([], [#strong[참고 파일]: `llm_factory.py` 는 LLM 인스턴스 생성(Ollama/OpenAI 분기), `agent_helpers.py` 는 RAG 체인 구성과 라우팅 매핑을 담당합니다. 둘 다 CH05\~06에서 이미 학습한 패턴과 동일하므로 코드 설명은 생략합니다.])
...
`_store` 딕셔너리에 `(값, 만료시각)` 튜플을 저장합니다. `get()` 할 때 현재 시각이 만료시각을 넘었으면 삭제하고요. 키는 `query::context` 문자열의 SHA-256 해시입니다. 1,000개가 꽉 차면 만료 시각이 가장 임박한 항목부터 지웁니다. `stats()` 에서 보이는 적중률(hit\_rate)이 높을수록 LLM 호출을 아끼고 있다는 뜻이에요.
...
#callout-box([], [#strong[ResponseCache vs EmbeddingCache 비교]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH07/terminal/07_log-first-query.png", alt: [그림 7-2: 첫 질문 처리 흐름. 에이전트가 도구를 호출하고, 토큰을 기록하고, 캐시에 저장한다.], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH07/terminal/07_log-cache-hit.png", alt: [그림 7-3: 같은 질문을 반복하면 캐시에서 즉시 반환한다. LLM을 호출하지 않는다.], max-width: 0.65)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH07/terminal/07_stats-dashboard.png", alt: [그림 7-4: 상태 확인 페이지에서 캐시 적중률 83.33%와 토큰 사용량을 확인한다.], max-width: 0.6)
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH08/gemini/08_chapter-opening.png", alt: [검색 품질 튜닝 --- 청킹, 리랭킹, 하이브리드], max-width: 0.4)

=== 1.1 "병가 규정을 물어봤는데 출장 규정이 나왔다"

CH07에서 캐시와 모니터링까지 달았습니다. 운영은 안정됐어요. 같은 질문엔 캐시로 빠르게 답하고 토큰도 추적합니다.

그런데 동료가 이상한 걸 하나 발견했습니다.

#strong[동료 C]: "병가 규정이 어떻게 되는지 물어봤는데요, 출장 규정을 가져와서 답변하더라고요."

확인해보니 정말 그랬습니다. "병가 신청 절차가 어떻게 되나요?"라고 물었는데 AI 비서가 출장 규정 문서를 근거로 답변을 만들어낸 겁니다. 출장비 정산 기한이라든가 숙박비 한도라든가, 병가와는 아무 관계도 없는 이야기를 그럴듯하게 늘어놓고 있었습니다.

문제는 LLM이 아닙니다. LLM은 받은 문서를 가지고 성실하게 답변했을 뿐이에요. #strong[잘못된 문서를 가져다 준 건 검색 단계]입니다.
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH08/gemini/08_search-evolution.png", alt: [그림 8-1: 사서가 성장하는 과정. 가위로 자르던 사서가, 문단을 보고, 의미를 읽고, 두 가지 방법을 섞어 찾게 된다.], max-width: 0.7)
...
이번 장은 지금까지와 조금 다릅니다. 새로운 기능을 "만드는" 게 아니라 기존 검색 파이프라인을 "실험하고 개선하는" 장이에요.
...
비유로 돌아가면, 사서의 서가 정리법과 꺼내오는 규칙(실험 1)을 바꾸고 꺼낸 문서를 재확인(실험 2)하고 두 가지 검색법을 동시에 쓰는 방법(실험 3)을 배우는 겁니다.
...
#callout-box([], [이번 챕터의 코드는 모두 #strong[독립 실험 모듈]입니다. ex07의 에이전트와 직접 통합하지 않고 각각 독립적으로 실행해서 결과를 비교합니다. 실험 결과를 확인한 뒤 "어떤 설정이 우리 데이터에 맞는지"를 결정하고 이후 챕터에서 에이전트에 반영해요.])
...
#callout-box([], [#strong[이전 챕터 Docker 종료]: CH07의 Docker가 실행 중이라면 `cd ex07 && docker compose down` 으로 먼저 종료하세요.])
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH08/diagram/08_tuning-steps.png", alt: [실습 순서: 청킹+Retriever → 리랭킹 → 하이브리드 검색], max-width: 0.7)
...
"병가를 물어봤는데 출장 규정이 나오는" 문제의 시작점이 바로 여기입니다. 문서를 어떻게 자르느냐가 검색 품질의 첫 번째 관문이에요.

이 실험 파일은 세 가지 실험을 `--step` 옵션으로 하나씩 진행합니다.

==== 실험 1-1: 청크 크기 비교 (300 / 500 / 1000자)

가장 먼저 확인할 건 "몇 자로 자를 것인가"입니다. 같은 문서를 300자, 500자, 1000자로 잘라보고 결과를 비교해봅시다.
...
이번 실험이 핵심입니다. 지금까지의 Fixed-size 청킹은 내용과 상관없이 글자 수로 잘랐습니다. "연차 규정"과 "출장 규정"이 연이어 나오면 하나의 청크에 두 주제가 섞이는 문제를 막을 수가 없어요.

이 문제를 줄이는 방법이 두 가지 있습니다.
...
#emph[\[이미지: Recursive Character Chunker 원리\]]
...
`SemanticChunker` 는 문장마다 임베딩 벡터를 계산한 뒤 인접 문장 간 코사인 유사도를 측정합니다. 유사도가 급격히 떨어지는 지점이 곧 주제가 바뀌는 지점이고, 거기서 잘라냅니다.

여기서 `breakpoint_threshold_amount=70` 이 중요합니다. 이 값이 바로 #strong[Percentile], 쉽게 말해 "검문소의 높이"에요.

#emph[\[이미지: Semantic Chunker 원리\]]
...
#callout-box([], [#strong[한 걸음 더 --- 짧은 문서에서는? (step 1-4)]
...
검색이 10개를 가져왔습니다. 이 중에 진짜 관련 있는 건 몇 개일까요? 초기 벡터 검색은 "대략 비슷한 문서"를 빠르게 가져오는 데는 좋지만 미세한 관련성 차이를 구분하기엔 약합니다. 리랭커가 이 차이를 잡아내요.

`ex08/tuning/step2_reranker/reranker.py` 의 핵심은 Cross-Encoder가 (질문, 문서) 쌍을 직접 읽고 채점하는 부분입니다.
...
#emph[\[이미지: 리랭킹 전후 비교\]] #emph[그림 8-3: 리랭킹 전에는 3위였던 문서가 리랭킹 후 1위로 올라온다. Cross-Encoder가 질문-문서 관련성을 더 정확히 판단한다.]

=== 2.7 실습 3: 하이브리드 검색 (tuning/step3\_hybrid\_search/)

마지막 실험입니다. 벡터 검색만으로는 부족한 경우가 있어요. "BM25"라는 키워드 기반 검색을 섞으면 어떨까요?

`ex08/tuning/step3_hybrid_search/retrievers.py` 에는 검색기가 세 개 들어 있습니다.
...
#emph[\[이미지: 하이브리드 검색 결과\]] #emph[그림 8-4: alpha=0.5일 때 BM25 점수와 Vector 점수가 합산되어 최종 순위가 결정된다.]
...
Fixed-size 청킹은 글자 수만 봅니다. 500자가 되면 자르죠. "연차 규정"이 480자째에서 시작해도 500자에서 잘려버립니다. 다음 청크는 연차 규정의 뒷부분과 출장 규정의 앞부분이 하나로 합쳐져요. 이러면 "연차"를 검색했을 때 출장 규정이 딸려 나옵니다.
...
핵심은 "내 문서에서 하나의 주제가 평균 몇 자인가?"입니다. 취업규칙처럼 한 조항이 500자 이상 이어진다면 시맨틱 청킹이 주제 경계를 정확히 잡아내서 검색 품질을 끌어올립니다.

글자 수와 무관하게 시맨틱 청킹이 특히 유리한 경우도 있습니다.
...
#callout-box([], [`--step 1-3` 으로 긴 문서(10개 규정, 약 4,500자)에서 시맨틱 청킹의 유사도가 역전하는 것을 직접 확인할 수 있습니다. `--step 1-4` 는 짧은 문서에서 percentile을 아무리 조정해도 Recursive가 이기는 것을 보여줍니다.])
...
= Ch.9: "질문을 제대로 이해 못한다" --- 질의+결과 고급 튜닝 (ex09)

#callout-box([], [이번 버전: ex08 → ex09 \
...
#emph[\[이미지: 질문 해석 튜닝 --- 질문을 바꿔야 답변이 달라진다\]]

=== 1.1 "휴가"라고 물었는데 못 찾는다
...
CH08에서 해결한 건 #strong["어떤 문서를 가져오느냐"] 였습니다. 검색 엔진 자체의 성능을 올렸습니다. 그런데 지금 문제는 다릅니다. #strong[검색 엔진이 잘 동작해도, 질문 자체를 이해 못하면 소용없습니다.]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH09/gemini/09_librarian-growth.png", alt: [그림 9-1: CH08이 서가 정리(검색 엔진 튜닝)였다면, CH09는 사서의 언어 능력(질의 변환)과 요약 능력(결과 가공)이다.], max-width: 0.65)

=== 1.3 CH08과 CH09의 차이

혼동하기 쉬우니 정리합니다.

#emph[\[이미지: CH08 vs CH09 비교\]] #emph[그림 9-2: CH08은 서가 정리(검색 엔진), CH09는 검색 전 질의 변환 + 검색 후 결과 가공. 둘은 택일이 아니라 조합하는 관계다.]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH09/diagram/09_ch08-vs-ch09.png", alt: [CH08 vs CH09 비교: 검색 전(질의 전략) → 검색 엔진 → 검색 후(결과 전략)], max-width: 0.7)
...
#callout-box([], [CH08과 마찬가지로 `--step` 옵션으로 서브실험을 하나씩 실행합니다. 파라미터를 바꿔가며 결과를 비교해보세요.])

=== 2.3 실습 1: 고급 Retriever (tuning/step1\_advanced\_retriever/)

CH08에서 리랭커와 하이브리드 검색으로 "가져오는 문서의 품질"을 높였다면, 여기서는 "검색과 반환 방식 자체"를 바꿉니다. 세 가지 Retriever를 서브실험으로 나누어 비교합니다.

실험에 사용할 데이터는 `data.py` 에 정의되어 있습니다. 부모 문서 3개(`PARENT_DOCUMENTS` --- 휴가 규정, 재택근무, 정보보안), 자식 청크 12개(`CHILD_CHUNKS` --- 부모 문서에서 핵심 내용을 한 줄로 요약), 토픽 키워드(`TOPIC_KEYWORDS` --- SelfQuery 필터에 사용)가 들어 있습니다.
...
Parent Document Retriever는 이 문제를 해결합니다. #strong[검색은 작은 청크로, 반환은 원본 문서로.]

부모 문서는 제15조\~16조 전체 텍스트이고, 자식 청크는 핵심 내용을 한 줄로 요약한 것입니다. 검색은 짧은 자식 청크의 임베딩으로, 반환은 긴 부모 문서로 합니다.
...
#emph[\[이미지: ParentDocument Retriever 실험\]] #emph[그림 9-3: 짧은 자식 청크(유사도 0.73)로 정밀하게 검색하고, 긴 부모 문서 전체(423자)를 반환한다.]
...
"연차 관련 규정"이라고 물으면 사람은 "아, 휴가 관련 문서를 찾으면 되겠구나"라고 자연스럽게 범위를 좁힙니다. SelfQuery Retriever는 이 과정을 자동화합니다. 질문에서 메타데이터 필터를 추출하여 #strong[검색 범위를 먼저 좁히고] 임베딩 검색을 수행합니다.

`data.py` 에 정의된 토픽 키워드 사전입니다.
...
#emph[\[이미지: SelfQuery Retriever 실험\]] #emph[그림 9-4: "연차 신청 절차와 팀장 승인 방법"에서 `topic=휴가` 필터를 자동 추출, 12개 → 4개 청크로 범위를 좁힌 뒤 검색한다.]
...
#emph[\[이미지: Contextual Compression 실험\]] #emph[그림 9-5: 423자 원본에서 질문 관련 3문장만 추출하여 40% 압축. 토큰 절약과 답변 집중도를 동시에 확보한다.]

#callout-box([], [Parent Document와 Compression을 #strong[조합]하면 더 효과적입니다. 부모 문서 전체를 가져온 뒤, 관련 문장만 압축하여 LLM에 전달하면 풍부한 컨텍스트와 토큰 절약을 동시에 얻을 수 있습니다.])

=== 2.4 실습 2: 질문을 바꿔서 검색한다 (tuning/step2\_query\_rewrite/)

검색 엔진을 건드리지 않고, 질문 자체를 바꿔서 검색 품질을 올리는 접근입니다. 세 가지 기법을 서브실험으로 나누어 비교합니다.

실험에 사용할 데이터는 `data.py` 에 정의되어 있습니다. 약어 사전 12개(`ABBREVIATION_MAP` --- WFH→재택근무, OT→초과근무 등), 동의어 사전 6개(`SYNONYM_MAP`), HyDE 템플릿 6개(`HYDE_TEMPLATES`), 검색 대상 문서 10개(`SEARCH_DOCUMENTS`)가 들어 있습니다.
...
#emph[\[이미지: 약어/동의어 확장 실험\]] #emph[그림 9-6: WFH→재택근무, 4대보험→국민연금/건강보험/고용보험/산재보험으로 자동 확장. LLM 없이 즉시 적용 가능하다.]
...
#callout-box([], [이 사전은 조직마다 다릅니다. 도입 후 첫 한 달간 사용자들이 자주 쓰는 약어를 모아서 사전에 추가하면 됩니다.])
...
핵심은 직접 검색과 HyDE 검색의 #strong[임베딩 유사도 비교]입니다.
...
#emph[\[이미지: HyDE 실험\]] #emph[그림 9-7: "연차 신청 절차" 질문에서 직접 검색(0.56) vs HyDE(0.91). 가상 문서가 실제 문서와 어투가 비슷해서 유사도가 크게 올라간다.]

#callout-box([], [#strong[HyDE가 항상 좋은 건 아닙니다.] 가상 문서가 잘못되면 오히려 관련 없는 문서를 가져올 수 있습니다. HyDE는 "답변이 구체적인 사실 문서로 존재하는" 환경(사내 규정, 법률 문서 등)에서 가장 효과적입니다.])
...
#emph[\[이미지: Multi-Query 실험\]] #emph[그림 9-8: 원본 1개 쿼리 → 4개 변형으로 검색 범위 확대. 병합 후 중복 제거로 고유 문서를 확보한다.]
...
#emph[\[이미지: CH08과 CH09 파이프라인 조합\]] #emph[그림 9-9: 질의 전략(CH09, 파란색) → 검색 전략(CH08, 주황색) → 결과 가공(CH09, 초록색). 파이프라인에서 LLM 호출이 필요한 단계는 빨간 별표로 표시.]
...
#callout-box([], [시맨틱 청킹(CH08)과 부모-자식 매핑(CH09 ParentDoc)은 #strong[인덱싱 시점의 사전 작업]입니다. 문서를 처음 저장할 때 시맨틱 청킹으로 자식 청크를 만들고, 동시에 부모-자식 매핑을 설정합니다. 위 파이프라인은 검색 시점의 흐름입니다.])

"WFH 신청 방법"이 실제로 이 파이프라인을 타는 순서입니다.

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH09/diagram/09_sequence-pipeline.png", alt: [전체 파이프라인 시퀀스: 질의 준비 → 검색 실행 → 결과 가공 → 최종 답변], max-width: 0.7)
...
= Ch.10: "PDF 이미지까지 잡아라" --- 고급 문서 처리와 평가 (ex10)

#callout-box([], [이번 버전: ex09 → ex10 \
...
#emph[\[이미지: 정보보안서약서 원본\]] #emph[그림 10-0: 팀장이 건넨 정보보안서약서. 결재란, 도장, 하이라이트가 있는 전형적인 사내 문서.]

스캔본이었습니다. 종이 문서를 스캐너로 찍어서 PDF로 만든 것입니다. 전체가 하나의 커다란 이미지입니다. 결재란에 서명이 있고, 빨간 도장도 찍혀 있고, 금지 사항에 하이라이트까지 되어 있습니다.

pypdf는 PDF 안의 텍스트 레이어를 읽는 도구입니다. CH04에서 언급했던 pdfplumber(표 추출 전용 라이브러리)도 마찬가지입니다. 둘 다 PDF 파일 구조에서 텍스트를 꺼내는 라이브러리입니다. 그런데 스캔본에는 텍스트 레이어가 없습니다. 사진을 아무리 뒤져봐야 글자 데이터는 없습니다.
...
문제는 시간입니다. 눈으로 한 페이지를 읽는 데 10초씩 걸립니다. 사내 문서가 수백 페이지면 한참 기다려야 합니다. 확대경은 같은 페이지를 1\~2초면 읽습니다. 문서의 구조에 맞춰 전략적으로 선택할 수 있어야 합니다.

#emph[그런데 확대경이 글자를 읽긴 읽었는데…]
...
#emph[\[이미지: OCR vs Vision LLM\]] #emph[그림 10-1: 광학 문자 인식(OCR)은 이미지 속 글자를 읽고, 비전 LLM은 이미지의 의미를 이해한다.]
...
사서가 문서를 펼쳤을 때 스스로 판단하게 하면 됩니다. "이 페이지는 글자가 잘 뽑히니까 OCR로 충분하고, 이 페이지는 이미지뿐이니까 눈으로 봐야겠다." 사서에게 #strong[판단력]까지 주는 것입니다.

=== 1.4 그런데, 진짜 좋아진 건 맞아?

CH08에서 청킹을 바꿔보고, 리랭킹을 적용하고, 하이브리드 검색을 도입했습니다. CH09에서 질의 재작성(Query Rewrite)과 근거 시스템까지 추가했습니다.

그때마다 "오, 좋아졌다"고 느꼈습니다. 그런데 #strong[느낌]입니다.
...
#emph[\[이미지: 평가 프레임워크\]] #emph[그림 10-2: RAG 엔진의 성적표. 질문마다 검색 정확도와 환각률을 수치로 측정한다.]

=== 1.5 성적표를 받았으니, 진짜 서비스를 만들자
...
#callout-box([], [OCR 엔진 #strong[EasyOCR] 은 `requirements.txt`에 포함되어 있어 별도 시스템 패키지 설치가 필요 없습니다. 비전 LLM은 컴퓨터 사양이 부족하면 `.env`에서 `VISION_PROVIDER=openai`로 전환할 수 있습니다.])
...
#callout-box([], [실습 1\~3은 `tuning/`에서 실험하고, 실습 4는 `src/`에 #strong[새 파일을 직접 만들어] 작성합니다. 기존 파일(`agent_config.py`, `tools/` 등)은 CH05\~09에서 만든 코드로 수정하지 않습니다.])

=== 2.4 실습 순서

#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH10/diagram/10_practice-steps.png", alt: [실습 순서: OCR vs 비전 LLM → 하이브리드 처리 → 평가 프레임워크 → 완성 웹 UI], max-width: 0.7)
...
#emph[\[이미지: 실험 1-1: OCR 파싱 결과\]] #emph[그림 10-2: OCR 파싱 결과. 스캔본에서 텍스트를 추출했지만 구조(표, 항목 번호)가 뭉개진다.]
...
#callout-box([], [로컬 모델의 속도는 하드웨어에 따라 크게 달라집니다. 같은 Qwen2.5-VL 7B라도 RAM 16GB 환경에서 25초, 8GB 환경에서는 수 분이 걸릴 수 있습니다. 또한 Ollama에 다른 모델이 VRAM에 올라가 있으면 메모리 경합으로 속도가 급격히 느려지니, 비전 실험 전에 `ollama ps` 로 확인하세요.])
...
#callout-box([], [#strong[내 컴퓨터 사양이 부족하다면?] Qwen2.5-VL 7B 모델은 RAM 10GB 이상을 권장합니다. 다른 모델이 VRAM에 올라가 있으면 속도가 급격히 느려지니, 비전 실험 전에 `ollama ps`로 확인하세요. 모델 로딩이 너무 느리거나 메모리 부족 오류가 나면, `.env` 파일에서 `VISION_PROVIDER=openai`로 바꿔 GPT-4o-mini 비전 API를 사용할 수 있습니다.])
...
#emph[\[이미지: 실험 1-2: 비전 LLM 파싱 + 비교표\]] #emph[그림 10-3: 같은 스캔 PDF를 OCR과 비전 LLM으로 파싱한 결과. OCR은 텍스트만, 비전 LLM은 구조화된 마크다운을 추출했다.]
...
#callout-box([], [평가용 벡터DB는 청크 크기를 200자로 설정했습니다. 문서 6개가 약 36개 청크로 나뉘어, k=3으로 검색하면 정답을 못 찾는 질문이 생깁니다. 실제 서비스의 검색 품질 차이를 체험하기 위한 의도적인 설정입니다.])
...
#emph[\[이미지: 실험 3-1: Precision\@k 측정\]]
...
#callout-box([], [이 실험은 Ollama LLM(llama3.1:8b)으로 37개 질문에 대한 답변을 생성합니다. 모델에 따라 1\~3분 정도 소요됩니다.])

#emph[\[이미지: 실험 3-3: 환각률 측정\]]
...
#auto-image("/Users/nomadlab/Desktop/김주혁/workspace/coding-study/집필에이전트 v2/projects/사내AI비서_v2/assets/CH10/diagram/10_search-pipeline.png", alt: [검색 파이프라인 전체 흐름: 사용자 질문 → 약어 확장 → HyDE → 하이브리드 검색 → 리랭킹 → 결과], max-width: 0.7)
...
#callout-box([], [#strong[데이터가 흐르는 경로]
...
#callout-box([], [#strong[예시 질문] - "취업규칙에서 연차 관련 조항이 뭐야?" - "정보보안서약서에 어떤 내용이 있어?" - "신규서비스 런칭 전략 알려줘"])

#emph[\[이미지: 웹 UI에서 질문을 던지면 답변 아래에 원본 PDF 페이지 이미지가 근거로 표시된다\]]
...
#strong[psycopg2-binary 설치 실패 (macOS Apple Silicon)]
...
```bash
# EasyOCR — requirements.txt에 포함, 별도 설치 불필요
# pip install easyocr  (이미 requirements.txt에 포함)
...
#table(
    ...
    [`easyocr`], [1.7.x], [OCR (CH10) --- pip만으로 설치],
  )
...
- #strong[Graph RAG] --- 단순 청크 검색이 아니라 지식 그래프로 문서 관계를 표현하는 방식
- #strong[멀티모달 RAG] --- 텍스트뿐 아니라 이미지, 표, 차트를 함께 다루는 RAG
- #strong[Agentic RAG] --- 에이전트가 검색 전략을 스스로 결정하는 방식
