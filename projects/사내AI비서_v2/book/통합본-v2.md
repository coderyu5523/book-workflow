...
**1일차.** 일단 ChatGPT에 물어봤습니다. "우리 회사 연차 규정이 어떻게 되나요?" AI는 자신 있게 답했습니다. 연차는 15일이고 3일 전까지 신청하면 된다고요. 우리 회사 규정을 열어보니 연차는 입사일 기준으로 달랐고 신청 기한도 일주일 전이었습니다. AI가 그럴듯하게 거짓말을 한 겁니다. 그날 하나를 깨달았습니다. **AI는 우리 회사 문서를 읽은 적이 없습니다.** 그래서 직접 넣어주기로 했습니다. 문서를 통째로 프롬프트에 복사해봤습니다. 작은 문서 하나는 됐습니다. 규정집 200페이지를 복사하니 토큰 한도를 넘겼습니다. 그때 RAG라는 걸 알게 됐습니다. "오픈북 시험"이라고 생각하면 됩니다. AI가 사내 문서 전체를 외울 필요 없이 질문할 때마다 관련 문서만 찾아서 보여주면 됩니다.
...
**3일차.** 사내 시스템은 됐습니다. 이제 문서를 넣을 차례입니다. 공유 드라이브를 열었는데 한숨이 나왔습니다. PDF와 DOCX, XLSX, 심지어 HWP까지. 최신 문서와 3년 전 폐기된 문서가 한 폴더에 섞여 있었습니다. (이걸 전부 넣으면?) 쓰레기를 넣으면 쓰레기가 나옵니다. AI도 마찬가지입니다. 도서관을 떠올렸습니다. 사서가 새 책을 받으면 분류하고 라벨을 붙이고 서가에 꽂습니다. 같은 원리입니다. 넣을 문서를 골라내고 형식을 통일하고 메타데이터를 붙이는 것. "신입사원에게 줘도 되는 문서인가?"를 필터링 기준으로 삼았습니다.
