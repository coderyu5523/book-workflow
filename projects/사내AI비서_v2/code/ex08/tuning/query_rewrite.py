...
def generate_hypothetical_document(query, llm=None):
    """HyDE 가상 문서를 생성합니다."""
    if llm is not None:
...
def generate_multi_queries_rule_based(query):
...
def generate_multi_queries(query, llm=None, num_queries=3):
    """다중 쿼리를 생성합니다."""
    if llm is not None:
...
def run_query_rewrite_experiment():
    ...
    console.rule("[bold blue]ex08 Query Rewrite 실험[/bold blue]")
