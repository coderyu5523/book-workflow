...
LangChain은 이 조립을 **파이프(|) 연산자**로 합니다. LCEL(LangChain Expression Language)이라고 부르죠.
...
사람 사서라면 당연히 기억합니다. 하지만 LLM은 기본적으로 **기억력이 없습니다**. 매 요청이 독립적이라 이전에 뭘 물어봤는지 모릅니다.

그래서 **대화 히스토리**를 직접 관리해야 합니다. 이전 대화를 메모해뒀다가 새 질문이 올 때마다 같이 넘겨주는 거예요. 다만 모든 대화를 다 기억할 수는 없으니 **최근 5턴만 유지**하는 슬라이딩 윈도우 방식을 씁니다.
...
-->
![슬라이딩 윈도우 메모장](../assets/CH05/diagram/05_sliding-window.png)
...
![RAG Q&A 결과](../assets/CH05/terminal/05_rag-qa-result.png)
