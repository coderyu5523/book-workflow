...
그런데 문서를 그냥 통째로 넣으면 될까요? CH01에서 이미 경험했습니다. 더미 문서 3개는 괜찮았습니다. 하지만 실제 사내 문서는 다릅니다. 취업규칙만 해도 수십 페이지입니다. 이걸 통째로 넣으면 "연차 몇 일이야?"라고 물었을 때, 보안규정이랑 매출 현황까지 딸려옵니다.
...
PDF 파일을 열어보겠습니다. 사람 눈에는 글자가 보이지만, 컴퓨터 입장에서는 그냥 바이너리 데이터입니다. "취업규칙 제1조"라는 텍스트를 꺼내려면 **파서(Parser)** 가 필요합니다.
...
실제 우리 프로젝트의 `data/docs/` 폴더에는 6개 문서가 있습니다.
...
각 조각에는 **메타데이터**도 함께 붙습니다. 출처 파일명, 페이지 번호, 분류 — CH03에서 설계한 라벨이 여기서 쓰입니다. 나중에 AI 비서가 "이 답변의 출처는 취업규칙 3페이지입니다"라고 말할 수 있게 됩니다.
...
이것이 벡터 검색의 핵심입니다. "연차 사용 규정"을 검색하면, 숫자가 비슷한 "휴가 관련 정책" 청크를 찾아올 수 있습니다. 키워드가 정확히 일치하지 않아도.
...
3. **사내 문서에 적합하다** — 사내 정보를 외부 API에 보내지 않아도 됩니다. 보안 관점에서도 안전합니다.
...
**ChromaDB**가 우리의 냉장고입니다. CH01에서 이미 써봤지만, 그때는 LangChain이 알아서 해줬습니다. 이번에는 직접 넣습니다.

ChromaDB에 저장하는 건 이 네 가지입니다.
...
저장할 때 **upsert**를 사용합니다. 같은 ID가 이미 있으면 덮어쓰기, 없으면 새로 추가합니다. 덕분에 파이프라인을 여러 번 실행해도 데이터가 중복되지 않습니다. CH03에서 설계한 **전체 재인덱싱** 전략과 맞닿는 부분입니다.

---

### 파이프라인을 돌려보겠습니다

손질(파싱) → 다지기(청킹) → 양념(임베딩) → 냉장고 정리(벡터 DB 저장). 네 단계를 알았으니, 이제 직접 돌려볼 차례입니다. `python src/main.py` 한 줄이면 이 과정이 한 번에 일어납니다.
...
6개 문서가 34개 청크로 변환되어 벡터 DB에 들어갔습니다. 냉장고에 재료를 넣었으니, 이제 꺼내볼 차례입니다. "연차 사용 규정"이라고 물어보면, 34개 청크 중에서 가장 가까운 걸 찾아올까요?

---

### 검색해보겠습니다 — "연차 사용 규정"

프로젝트에 포함된 CLI 검색 도구(`cli_search.py`)로 직접 확인해 보겠습니다.
...
1, 2위는 취업규칙에서 연차 관련 내용을 정확히 찾아왔습니다. 3위는 보안규정의 "휴가 중 보안장비 관리"인데 — "휴가"라는 비슷한 의미 때문에 딸려왔습니다. 키워드 "연차"가 없는데도 의미상 연관된 문서를 찾아내는 게 벡터 검색의 힘입니다.
