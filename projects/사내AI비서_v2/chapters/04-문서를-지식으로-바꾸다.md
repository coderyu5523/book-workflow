...
CH03에서 사내 문서를 정리했습니다. 분류도 했고, 메타데이터 라벨도 붙였습니다. 이제 이 문서들을 벡터 DB에 넣어서 AI 비서가 검색할 수 있게 만들 차례입니다.

그런데 문서를 그냥 통째로 넣으면 될까요? CH01에서 이미 경험했습니다. 더미 문서 3개는 괜찮았습니다. 하지만 실제 사내 문서는 다릅니다. 취업규칙만 해도 수십 페이지. 이걸 통째로 넣으면 "연차 몇 일이야?"라고 물었을 때, 보안규정이랑 매출 현황까지 딸려옵니다.

문서를 **검색 가능한 지식**으로 바꾸려면, 몇 단계를 거쳐야 합니다.
...
마트에서 사온 재료를 봉지째로 냉장고에 던져 넣으면 어떻게 될까요? 나중에 찾을 수가 없습니다. 양파가 어디 있는지, 고기는 아직 쓸 수 있는지. 뒤져봐야 압니다.
...
![취업규칙 PDF 원본 확인](../assets/CH04/04_example_hr_rules.png)

위 예제로 들어가 있는 취업규칙 PDF 파일을 확인해봅니다. 사람 눈에는 글자가 보이지만, 컴퓨터 입장에서는 그냥 바이너리 데이터입니다. "취업규칙 제1조"라는 텍스트를 꺼내려면 **파서(Parser)** 가 필요합니다.

문제는 형식마다 파서가 다르다는 것입니다. PDF는 PDF 파서가, DOCX는 DOCX 파서가, XLSX는 XLSX 파서가 필요합니다. CH03에서 허용한 형식을 기억하시나요?
...
텍스트를 꺼냈습니다. 그런데 취업규칙 전문이 한 덩어리로 들어가면 안 됩니다. CH01에서 경험한 그 문제입니다 — 문서가 너무 크면 관련 없는 내용까지 딸려옵니다.

요리에서 재료를 다지는 것처럼, 텍스트를 **적당한 크기의 조각(chunk)** 으로 잘라야 합니다.

CH03에서 설계한 대로, **고정 크기 500자 + 100자 오버랩** 으로 갑니다.

왜 오버랩이 필요할까요? "신입사원은 첫 3년간 연차가 없다" 다음에 "대신 리프레시 데이를 제공한다"가 이어지는데, 정확히 500자에서 잘리면 "연차가 없다"만 남고 뒤의 대안 정보가 사라집니다. 오버랩 100자가 이 문제를 줄여줍니다.
...
각 조각에는 **메타데이터**도 함께 붙습니다. 출처 파일명, 페이지 번호, 분류 — CH03에서 설계한 라벨이 여기서 쓰입니다. 나중에 AI 비서가 "이 답변의 출처는 취업규칙 3페이지입니다"라고 말할 수 있게 되는 것입니다.
...
"연차 사용 규정"이라는 텍스트를 컴퓨터가 이해할 수 있을까요? 컴퓨터는 글자를 모릅니다. 숫자만 압니다.

**임베딩(Embedding)** 은 텍스트의 **의미**를 숫자 벡터(768개의 숫자 리스트)로 바꾸는 과정입니다. 중요한 건, 단순히 글자를 숫자로 바꾸는 게 아니라 **의미가 비슷한 텍스트는 비슷한 숫자**가 된다는 것입니다.
...
이것이 벡터 검색의 핵심입니다. "연차 사용 규정"을 검색하면, 숫자가 비슷한 "휴가 관련 정책" 청크를 찾아올 수 있습니다. 키워드가 정확히 일치하지 않아도, 의미가 가까우면 찾아냅니다.

그런데 "숫자가 비슷하다"는 걸 어떻게 판단할까요? 768개나 되는 숫자를 하나하나 비교할 수는 없습니다.

여기서 **코사인 유사도(Cosine Similarity)** 라는 방법을 씁니다. 벡터를 좌표 위의 **화살표**라고 생각하면 됩니다. 두 화살표가 같은 방향을 가리키면 의미가 비슷한 것이고, 반대 방향이면 의미가 다른 것입니다. 코사인 유사도는 이 **두 화살표 사이의 각도**를 측정합니다.
...
Style: concept-diagram
-->
![코사인 유사도 방향별 값](../assets/CH04/04_cosine-similarity.png)
...
임베딩 모델은 여러 가지가 있습니다. OpenAI의 text-embedding-ada-002도 있고, 다국어 모델도 있습니다. 우리는 **ko-sroberta-multitask**를 선택했습니다.

이유는 간단합니다.

1. **한국어에 특화됐다** — 한국어 문장 유사도 태스크로 파인튜닝된 모델입니다. "연차"와 "휴가"가 의미상 가깝다는 걸 잘 잡아냅니다.
2. **로컬에서 돌릴 수 있다** — OpenAI 임베딩은 API 호출마다 비용이 듭니다. ko-sroberta는 한 번 다운로드하면 로컬에서 무료로 쓸 수 있습니다.
3. **사내 문서에 적합하다** — 사내 정보를 외부 API에 보내지 않아도 됩니다. 보안 관점에서 안전합니다.
...
**ChromaDB** 가 우리의 냉장고입니다. CH01에서 이미 써봤지만, 그때는 `Chroma.from_documents()` 한 줄로 끝냈습니다. 이번에는 직접 넣습니다.

ChromaDB에 저장하는 항목은 이 네 가지입니다.
...
저장할 때 **upsert**를 사용합니다. 같은 ID가 이미 있으면 덮어쓰기, 없으면 새로 추가합니다. 파이프라인을 여러 번 실행해도 데이터가 중복되지 않습니다. CH03에서 설계한 **전체 재인덱싱** 전략과 맞닿는 부분입니다.
...
```mermaid
flowchart LR
    A["main.py --step 1<br>파싱 + 마크다운 저장"] --> B["data/markdown/<br>결과 확인"]
    B --> C["main.py<br>전체 파이프라인"]
    C --> D["cli_search.py<br>검색 테스트"]
```
...
`--step 1` 은 손질(파싱)과 마크다운 변환을 실행합니다. 파싱이 끝나면 `data/markdown/` 폴더에 마크다운 파일이 생깁니다. 이 파일을 열어보는 것이 핵심입니다. **파싱이 잘 됐는지 눈으로 확인하는 과정** — 이 단계를 건너뛰면 안 됩니다.
...
파일 정보(파일명, 파싱 형식, 글자 수)가 상단에 요약되었고, `## 페이지 1` 처럼 페이지 단위로 텍스트가 잘 추출된 것을 볼 수 있습니다.
...
마크다운 표 형식을 확인할 수 있습니다. 표의 생김새가 일반 표보다 삐뚤빼뚤하고 다소 끊어진 것처럼 보일 수 있습니다. 하지만 **벡터 DB(임베딩 모델) 입장에서는 오히려 이 방식이 엑셀 원본보다 훨씬 이해하기 좋습니다.** 헤더 행과 구분선(`---`)이 존재하고 행/열의 문맥이 텍스트 흐름(파이프 `|` 기호 등)으로 유지되기 때문에, "경영지원본부 예산"을 검색하면 해당 표의 데이터가 아주 정확하게 검색됩니다.
> 파싱은 RAG의 첫 단추입니다. 여기서 텍스트가 깨지면 뒤에서 아무리 고쳐도 소용이 없습니다. RAG에서는 쓰레기가 들어가면 쓰레기가 나온다는 **Garbage In, Garbage Out** 격언이 있을 정도로 이 작업은 중요합니다. `data/markdown/` 파일을 반드시 열어보고, 원본 문서와 비교해 보세요.

> 이 책에서는 pypdf, python-docx, openpyxl 라이브러리를 사용하므로 파서 코드를 직접 따라 치지는 않습니다. 하지만 실무에서는 파싱이 까다로운 경우가 훨씬 많습니다. 2단 열 레이아웃, 투명 표, 표 안의 표, 헤더가 페이지를 넘어가며 끊기는 표, 한 페이지에 표가 여러 개인 경우 등 다양한 상황이 존재합니다. 이런 복잡한 레이아웃을 처리하는 전용 라이브러리(pdfplumber, camelot, unstructured 등)도 있고, 이미지를 인식하는 Vision LLM을 활용하는 방법도 있습니다(CH10에서 다룹니다). 여러분의 사내 문서로 직접 파싱해 보고, 결과를 눈으로 확인하는 습관을 들이세요.
---
...
`--chunk-size` 옵션으로 청크 크기를 바꿀 수 있습니다. 아무 옵션 없이 실행하면 기본값(500자, 오버랩 100자)으로 동작합니다. 내부적으로는 두 단계를 순서대로 호출합니다.

1. **Step 1: 손질(파싱) + 마크다운 변환** — `data/docs/` 의 6개 문서에서 텍스트를 추출하고, 마크다운으로 변환하여 `data/markdown/` 에 저장합니다
2. **Step 2: 다지기 + 양념 + 저장** — 마크다운 텍스트를 청크로 자르고, 임베딩하고, ChromaDB에 넣습니다
...
### 실습 3: 벡터 검색 CLI 도구 (cli_search.py)
...
거리가 0이면 유사도 100%(완전 일치), 거리가 2이면 유사도 0%(완전 반대)입니다. "휴가 규정"을 검색했을 때 유사도 71%가 나왔다면, 코사인 거리로는 약 0.58이라는 뜻입니다.

![CLI 검색 결과](../assets/CH04/04_cli-search.png)
*그림 4-6: "휴가 규정"을 검색하자 취업규칙에서 글로벌 노마드, 스마트 휴가 승인 등의 관련 내용을 찾아냈다.*

1~3위 모두 취업규칙에서 휴가(글로벌 노마드, 스마트 휴가, 워케이션) 관련 내용을 찾아왔습니다. 모델이 문맥을 잘 파악하여 정답을 찾아낸 것을 볼 수 있습니다. 다만, 유사도가 71%대로 아주 높지는 않고, `--top-k 5`처럼 검색수를 늘리면 전혀 관계없는 문서가 억지로 딸려올 수도 있습니다. 벡터 DB는 키워드 매칭이 아니라 '그나마 의미가 가까운 순서'대로 k개를 무조건 채워서 가져오기 때문입니다. 검색 품질을 높이는 심화 기법은 CH08에서 집중적으로 다룹니다.
...
핵심은 `extract_text()` 함수입니다. 확장자 → 파서 함수 매핑 딕셔너리를 사용합니다.
...
`extractor_map` 딕셔너리가 확장자별로 다른 함수를 선택합니다. if-elif-else 분기 대신 딕셔너리 매핑을 사용하면, 새 형식을 추가할 때 한 줄만 추가하면 됩니다.
...
`page.extract_text()` 가 `None` 을 반환하는 경우가 있습니다. **이미지로 된 PDF** — 스캔 문서나 캡처 화면 — 는 텍스트 레이어가 없어서, pypdf가 아무것도 꺼내지 못합니다. `or ""` 방어 코드 없이는 `None.strip()` 에서 바로 에러가 납니다. 실제로 예제 파일 중 'HR_정보보안서약서.pdf'가 이미지 기반 PDF입니다. 이 파일은 텍스트가 하나도 추출되지 않아 파싱에서 텍스트 청크가 0개인걸 **그림 4-4** 에서 확인할 수 있습니다.

이 문제는 CH10에서 Vision LLM을 사용해 해결합니다. 지금은 "텍스트 기반 PDF만 파싱된다"는 한계를 인지하고 넘어갑니다.
...
추출된 텍스트를 500자 단위로 자릅니다. **이 함수가 CH04의 핵심입니다.** 청킹이 RAG 검색 품질을 좌우하기 때문입니다.
...
`step = chunk_size - overlap` 한 줄이 핵심입니다. 500자 청크에서 100자 오버랩이면, 다음 청크는 400자 뒤에서 시작합니다. 이전 청크의 마지막 100자가 다음 청크의 처음 100자와 겹칩니다.
...
청크 ID는 `문서ID_text_p페이지_c순번` 형식입니다. 이 ID 덕분에 같은 파이프라인을 여러 번 실행해도 데이터가 중복되지 않습니다 (upsert).
...
1. **메타데이터 정제** — ChromaDB는 메타데이터에 `None` 이나 `bool` 을 허용하지 않습니다. `None` → 빈 문자열, `bool` → 문자열로 변환해야 합니다. 이걸 빠뜨리면 저장 시 에러가 납니다.
2. **`normalize_embeddings=True`** — 벡터를 단위 벡터로 정규화합니다. 코사인 유사도 계산에 필요합니다.
...
`PersistentClient` 가 핵심입니다. 메모리가 아닌 디스크(`data/chroma_db/`)에 저장하므로, 프로그램을 종료해도 데이터가 남아있습니다. `hnsw:space: "cosine"` 으로 코사인 유사도를 사용합니다.

---

### 더 알아보기

**마크다운 변환** — 파이프라인은 추출한 텍스트를 마크다운 형식으로 변환하여 `data/markdown/`에 저장한 뒤 임베딩합니다. 임베딩 모델도 마크다운 구조(제목 `#`, 표 `|`, 리스트 `-`)를 학습했기 때문에, 구조가 잘 보존된 텍스트일수록 의미를 더 정확하게 벡터로 변환합니다. `extract_pdf.py`, `extract_docx.py`, `extract_xlsx.py`를 단독으로 실행하면 형식별로 개별 변환할 수도 있습니다.

**배치 크기 튜닝** — `BATCH_SIZE = 64`가 기본값입니다. GPU 메모리가 부족하면 줄이고, 충분하면 늘릴 수 있습니다. ko-sroberta는 CPU에서도 동작하지만 GPU가 있으면 임베딩 속도가 크게 빨라집니다.
...
- **임베딩은 "의미를 숫자로 바꾸는 것"입니다.** 키워드가 달라도 의미가 비슷하면 가까운 벡터가 됩니다.
