...
-->
![VectorDB 구축 파이프라인](../assets/CH04/diagram/04_vectordb-pipeline.png)
...
![취업규칙 PDF 원본 확인](../assets/CH04/diagram/04_example_hr_rules.png)
...
**임베딩(Embedding)** 은 텍스트의 **의미** 를 숫자 벡터(768개의 숫자 리스트)로 바꾸는 과정입니다. 여기서 핵심은 단순히 글자를 숫자로 바꾸는 게 아니라 **의미가 비슷한 텍스트는 비슷한 숫자** 가 된다는 점입니다.
...
여기서 **코사인 유사도(Cosine Similarity)** 라는 방법을 씁니다. 벡터를 좌표 위의 **화살표** 라고 생각해보세요. 두 화살표가 같은 방향을 가리키면 의미가 비슷한 거고, 반대 방향이면 의미가 다른 겁니다. 코사인 유사도는 이 **두 화살표 사이의 각도** 를 측정합니다.
...
Style: concept-diagram
-->
![코사인 유사도 방향별 값](../assets/CH04/diagram/04_cosine-similarity.png)
...
임베딩 모델은 여러 가지가 있습니다. OpenAI의 text-embedding-ada-002도 있고 다국어 모델도 있어요. 우리는 **ko-sroberta-multitask** 를 선택했습니다.
...
**ChromaDB** 가 우리의 냉장고입니다. CH01에서 이미 써봤지만 그때는 `Chroma.from_documents()` 한 줄로 끝냈죠. 이번에는 직접 넣습니다.
