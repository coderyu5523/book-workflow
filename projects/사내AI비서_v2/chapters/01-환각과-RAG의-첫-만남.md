...
<!-- [CAPTURE NEEDED: ch03_step1_hallucination
  path: assets/CH01/ch01_step1_hallucination.png
  desc: step1_fail.py 실행 결과 — LLM이 존재하지 않는 커넥트 규정을 그럴듯하게 답하는 화면
] -->
![step1 실행 결과](../assets/CH01/ch01_step1_hallucination.png)
...
<!-- [CAPTURE NEEDED: ch03_step2_context
  path: assets/CH01/ch01_step2_context.png
  desc: step2_context.py 실행 결과 — 규정 내용을 직접 주입했을 때 정확한 답변이 나오는 화면
] -->
![step2 실행 결과](../assets/CH01/ch01_step2_context.png)
...
<!-- [CAPTURE NEEDED: ch03_step3_rag
  path: assets/CH01/ch01_step3_rag.png
  desc: step3_rag.py 실행 결과 — 검색된 문서 출처와 함께 정확한 답변이 나오는 화면
] -->
![step3 실행 결과](../assets/CH01/ch01_step3_rag.png)
...
`step3_rag_no_chunking.py`는 모든 문서를 하나의 덩어리로 합쳐서 저장한다. `step3_rag.py`와 비교하면 무슨 차이가 있을까? 직접 실행해서 검색 결과를 비교해보자. 왜 문서를 조각으로 나눠야 하는지 체감할 수 있다. 청킹 전략의 상세 비교는 CH08(검색 품질 튜닝)에서 다룬다.
