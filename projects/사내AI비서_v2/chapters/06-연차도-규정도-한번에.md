...
- **v0.2**에서 직원/연차/매출 데이터를 PostgreSQL DB에 저장하는 API를 만들었습니다.
- **v0.4~v0.5**에서 사내 문서를 벡터DB에 넣고 검색하는 RAG 엔진을 만들었습니다.
...
회사에 처음 입사한 날을 생각해보겠습니다.

낯선 건물에 들어서자마자 1층 안내데스크로 갔습니다.

**방문자**: "인사 관련 서류는 어디서 받아요?"

안내데스크 직원이 잠깐 생각하더니 답했습니다.

**안내데스크**: "인사팀은 3층이에요. 엘리베이터 내리셔서 오른쪽입니다."

며칠 후, 조금 복잡한 질문이 생겼습니다.
...
에이전트 안에는 **QueryRouter**가 있습니다. 모든 질문은 이 라우터를 먼저 거칩니다.

라우터는 3단계로 질문을 분류합니다.

**1단계 — 키워드 매칭 (빠르고 확실)**

"연차", "매출", "잔여" 같은 단어가 있으면 DB 조회 질문으로 봅니다.
"절차", "규정", "방법" 같은 단어가 있으면 문서 검색 질문으로 봅니다.
둘 다 있으면 복합 질문입니다.

명확한 키워드가 없으면 2단계로 넘어갑니다.

**2단계 — DB 컬럼명 매칭 (기술적 표현)**

"remaining_days", "emp_no" 처럼 DB 컬럼명이 질문에 직접 들어있으면 DB 조회로 분류합니다.
개발자가 직접 API를 테스트할 때 이런 표현을 쓰는 경우가 있습니다.

그래도 모호하면 마지막 수단을 씁니다.
...
LLM이 `{"route": "hybrid", "reason": "연차는 DB, 규정은 문서"}` 형태로 답합니다.

---

대부분의 질문은 1단계에서 끝납니다. 2단계와 3단계는 폴백입니다. 확실한 것부터 확인하고, 모호할 때만 더 비싼 방법을 쓰는 구조입니다.
...
데코레이터를 붙이면 LangChain이 도구의 이름, 설명, 파라미터를 자동으로 읽습니다. 에이전트는 이 정보를 보고 "이 질문에는 어떤 도구가 맞지?"를 판단합니다.

독스트링이 중요한 이유가 여기 있습니다. 함수 설명이 불명확하면 에이전트가 엉뚱한 도구를 선택합니다. 안내데스크 직원에게 각 팀이 무슨 일을 하는지 제대로 알려줘야 제대로 연결해주는 것처럼.
...
다음 챕터(v0.7)에서는 이 에이전트를 실제로 쓰다 보면 생기는 문제를 해결합니다. 같은 질문에 매번 DB를 조회하는 비용, 응답이 느려지는 문제입니다.
...
curl -X POST http://localhost:8000/api/chat \
  ...
  -d '{"query": "A 사원 연차 몇 일 남았어? 그리고 연차 신청 규정은?", "use_agent": true}'
```
