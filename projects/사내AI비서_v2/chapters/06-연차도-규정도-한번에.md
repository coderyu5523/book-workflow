...
이번 챕터에서 만들 **통합 에이전트** 가 바로 이 안내데스크입니다.
...
-->
![QueryRouter 3단계](../assets/CH06/diagram/06_query-router.png)
...
에이전트의 두뇌는 **ReAct 패턴** 으로 동작합니다. Reason(추론) + Act(행동). 이름이 거창해 보이지만 원리는 단순해요. 생각하고 행동하는 걸 반복합니다.
...
분류기 클래스를 선언하고 외부에서 호출할 `classify_query` 메서드를 먼저 작성합니다.
규칙 기반 -> 스키마 기반 -> LLM 기반(폴백) 3단계 판별 구조가 어떻게 연결되는지 눈여겨보세요.
...
인터페이스에서 호출한 `_step1_rule_based`, `_step2_schema_based`, `_step3_llm_based` 를 실제로 구현하는 코드입니다.
가장 저렴하고 빠른 키워드 매칭을 먼저 돌리고 그래도 안 될 때만 LLM을 호출합니다. 속도와 비용을 동시에 잡는 구조입니다.
