...
사내 시스템(v0.2)은 만들었습니다. 직원, 연차, 매출 데이터는 API로 조회할 수 있습니다. 이제 AI 비서의 나머지 절반 — **문서 검색**을 본격적으로 준비할 차례입니다.

CH01에서 더미 문서 3개로 RAG를 돌려봤습니다. 잘 됐습니다. 이제 실제 사내 문서를 넣어보겠습니다.

공유 드라이브를 열었습니다.

취업규칙.pdf, 보안지침_v3_최종_진짜최종.docx, 2024년_복지정책.xlsx, 회의록_0301.hwp, 프로젝트_보고서.pptx, README.md…

한숨이 나왔습니다. 형식이 제각각입니다. PDF도 있고, 워드도 있고, 엑셀도 있고, 심지어 한글 파일도 있습니다. 어떤 건 최신인데 어떤 건 2년 전 문서입니다. 이걸 통째로 다 벡터 DB에 밀어 넣으면 될까요?
...
1. **먼저 분류합니다** — 이 책이 어느 분야인지, 대출 가능한지, 최신판인지 확인합니다.
2. **라벨을 붙입니다** — 청구기호(위치), 저자, 출판년도, 키워드를 기록합니다.
3. **서가에 꽂습니다** — 분류에 맞는 위치에 넣습니다.

라벨 없이 책을 마구잡이로 꽂아놓으면? 나중에 찾을 수 없습니다. "경영학 개론이 어딨지?" 하면서 서가 전체를 뒤져야 합니다.

사내 문서도 마찬가지입니다. 벡터 DB에 넣기 전에 **분류하고, 라벨을 붙이고, 정리하는 단계**가 필요합니다. 이 단계를 건너뛰면 나중에 검색 품질이 엉망이 됩니다.
...
간단한 기준이 있습니다. "이 문서를 신입사원에게 줘도 되나?" 된다면 넣고, 아니라면 뺍니다.
...
외국 도서관을 생각해보겠습니다. 영어 책, 일본어 책, 프랑스어 책이 섞여 들어옵니다. 서가에 꽂기 전에 먼저 **한국어로 번역**해야 찾을 수 있습니다. 마찬가지로, PDF/DOCX/XLSX를 먼저 **하나의 텍스트 형태**로 바꿔야 합니다.
...
도서관에서 책에 붙이는 정보 — 청구기호, 저자, 분야, 출판년도. 이것이 문서의 세계에서는 **메타데이터**입니다.

메타데이터가 왜 중요할까요? AI 비서에게 "보안 관련 규정 알려줘"라고 물었을 때, 메타데이터에 `file_name: SEC_보안규정`이 있으면 보안 문서를 바로 식별할 수 있습니다. 메타데이터가 없으면 모든 문서를 다 뒤져야 합니다.

좋은 소식은, 메타데이터를 직접 하나하나 적을 필요가 없다는 점입니다. **파서가 파일명과 경로에서 자동으로 추출**합니다.
...
그래서 **파일명에 정보를 담는 것**이 중요합니다. `HR_취업규칙_v1.0.pdf`처럼 분류(HR)와 버전(v1.0)을 파일명에 넣으면, 파서가 알아서 메타데이터로 만들어줍니다.
...
지금 단계에서는 설계만 해두겠습니다. 실제 구현은 CH04(VectorDB 구축)에서 합니다.
...
재인덱싱을 안 하면 어떻게 될까요? AI 비서가 폐기된 규정을 근거로 답변합니다. "연차 15일입니다"라고 답하는데, 실제로는 규정이 바뀌어서 20일인 상황입니다. 환각보다 더 위험합니다 — 출처까지 달려있으니까 신뢰하게 됩니다.
...
정리하면, 우리 사내 AI 비서에 넣을 문서는 이런 구조로 관리합니다.
...
별도의 메타데이터 파일을 만들 필요 없습니다. **폴더 구조와 파일명이 곧 메타데이터**입니다. CH04에서 파서가 이 정보를 자동으로 추출해서 벡터 DB에 저장합니다.
