...
좋습니다. 그런데 팀장이 새로운 요구를 던졌습니다.

**팀장**: "문서 검색만 되면 안 되지."<br>
**팀장**: "'팀원 연차 몇 개 남았어?', '이번 달 개발팀 매출 얼마야?' 이런 것도 답해줘야지."
...
AI가 "팀원 연차 잔여일을 알려줘"라고 부탁할 대상이 필요한 것입니다.
...
손님(프론트엔드)이 식당에 들어와서 주문을 합니다.
"된장찌개 하나요."
이 주문을 받는 사람이 **웨이터(API)** 입니다.
웨이터는 주문을 받아서 **주방(데이터베이스)** 에 전달합니다.
주방에서 요리가 나오면 웨이터가 손님에게 가져다줍니다.

여기서 중요한 건, 손님이 직접 주방에 들어가지 않는다는 겁니다.
반드시 웨이터를 통해야 합니다.
주방의 레시피도 모르고, 냉장고에 뭐가 있는지도 모릅니다.
"된장찌개 주세요"라고 말하면, 웨이터가 알아서 주방과 소통합니다.
...
식당에 메뉴판이 있듯이, API에도 할 수 있는 일의 목록이 있습니다. 사내 시스템에서 데이터를 다루는 기본 동작은 딱 네 가지입니다.
...
직원 정보든, 연차 잔여량이든, 매출 기록이든 — 결국 등록하고, 조회하고, 수정하고, 삭제하는 겁니다.
...
이 세 테이블의 데이터를 API로 관리하는 시스템. 그게 이번 챕터에서 확인할 것입니다.
