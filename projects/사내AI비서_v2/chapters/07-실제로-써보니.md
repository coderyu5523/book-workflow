...
다만 메모장에는 유통기한이 있습니다. 1시간이 지나면 메모를 지워요. 사내 규정이 바뀌었을 수도 있으니까요. 이걸 **TTL(Time To Live)** 이라고 부릅니다.
...
실무에서는 **Langfuse** 나 LangSmith 같은 전문 모니터링 도구가 이 역할을 합니다. 호출별 토큰 수와 비용, 지연 시간을 자동으로 수집해서 대시보드로 보여주죠. 하지만 도구를 붙이기 전에 "무엇을 측정해야 하는지"부터 이해해야 합니다.

그래서 이 책에서는 **TokenTracker** 라는 간단한 클래스를 직접 만들어 봅니다. 매 호출마다 한 줄씩 기록이 쌓이고 나중에 "이번 달 토큰 비용이 얼마야?"라고 물으면 바로 답할 수 있어요. 도서관 이용 통계처럼 운영을 개선하려면 먼저 측정부터 해야 합니다.
...
새 기능을 추가하는 게 아닙니다. 기존 에이전트를 **안정적으로 운영** 할 수 있게 감싸는 것이죠.
...
`_store` 딕셔너리에 `(값, 만료시각)` 튜플을 저장합니다. `get()` 할 때 현재 시각이 만료시각을 넘었으면 삭제하고요. 키는 `query::context` 문자열의 SHA-256 해시입니다. 1,000개가 꽉 차면 만료 시각이 가장 임박한 항목부터 지웁니다. `stats()` 에서 보이는 적중률(hit_rate)이 높을수록 LLM 호출을 아끼고 있다는 뜻이에요.
