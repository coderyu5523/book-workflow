...
Title (top center, bold): "글자 인식(OCR) vs 이미지 이해(비전 LLM) — 확대경 vs 눈"
Thin horizontal rule below title.

[Left box — rounded rectangle, thin black border, light gray fill]
  Header bar (dark gray background, white text): "확대경 — OCR"
  [Inner box 1 — dashed border] minimalist line-art document icon with chart/boxes inside
  [Inner box 2 — solid border] magnifying glass icon → scattered text fragments:
    "대표이사", "인사팀", "재무팀", "개발팀" (no structure, floating apart)
  Footer label: "글자만 읽음 — 구조는 모름"
  [Result box — hairline border] "대표이사경영지원본부기술개발본부영업본부인사팀재무팀..."

[Center — vertical dotted line divider, "vs" in circle at midpoint]

[Right box — rounded rectangle, thin black border, light gray fill]
  Header bar (dark gray background, white text): "눈 — 비전 LLM"
  [Inner box 1 — dashed border] same document icon
  [Inner box 2 — solid border] eye icon → structured tree output:
    "## 조직도"
    "대표이사"
    "  ├── 경영지원본부 (인사팀, 재무팀)"
    "  ├── 기술개발본부 (개발1팀, 개발2팀)"
    "  └── 영업본부 (국내, 해외)"
  Footer label: "그림을 이해함 — 구조까지 파악"
...
사서가 문서를 펼쳤을 때 스스로 판단하게 하면 됩니다. "이 페이지는 글자가 잘 뽑히니까 OCR로 충분하고, 이 페이지는 이미지뿐이니까 눈으로 봐야겠다." 사서에게 **판단력**까지 주는 것입니다.
...
```mermaid
flowchart LR
    A["실습 1<br>OCR vs 비전 LLM"] --> B["실습 2<br>하이브리드 처리"]
    B --> C["실습 3<br>평가 프레임워크"]
    C --> D["실습 4<br>완성 웹 UI"]
...
### 실습 1: 문서 파싱 전략 비교 (tuning/step1_document_parser/)
...
### 실습 2: 하이브리드 이미지 처리 (tuning/step2_hybrid_parser/)
...
### 실습 3: RAG 평가 프레임워크 (tuning/step3_eval_framework/)
...
```mermaid
flowchart LR
    A["사용자 질문"] --> B["약어 확장<br/>(CH09)"]
    B --> C["HyDE<br/>(CH09)"]
    C --> D["하이브리드 검색<br/>벡터 + BM25<br/>(CH08)"]
    D --> E["리랭킹<br/>Cross-Encoder<br/>(CH08)"]
    E --> F["결과 + image_path<br/>(CH10)"]
```
