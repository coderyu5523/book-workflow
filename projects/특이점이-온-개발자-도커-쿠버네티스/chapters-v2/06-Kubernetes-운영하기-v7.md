...
챕터 5까지 온 오픈이는 네트워크의 윤곽을 머릿속에 그릴 수 있었습니다. Service로 Pod의 고정 주소를 만들었고, Ingress로 외부 브라우저에서 URL로 접근하는 경로도 이어놓았습니다. 그런데 실제 서비스에 쓰려면 아직 빠진 조각이 있었습니다.

- DB 비밀번호나 API 키 같은 **민감한 값**을 이미지 안에 직접 넣어두면 안 된다는 것
...
챕터 3에서 Compose로 띄운 그 구성을 이번엔 K8s 버전으로 옮기는 여정입니다. 이걸 끝내면 책 전체를 통틀어 진짜 도착점에 서게 됩니다.
...
`conn_info=localhost:80`과 `conn_url=config.test`가 환경 변수로 그대로 들어와 있었습니다. **이미지를 건드리지 않고 설정 값만 바깥에서 주입**하는 그림이 눈앞에 확인됐습니다.
...
이미지 안에는 비밀번호가 없고, 실행 시점에 Secret에서 꺼내 주입됩니다. 오늘 세 가지 과제 중 "설정과 민감 정보를 이미지 바깥으로" 한 줄이 먼저 지워졌습니다.
...
이번엔 데이터 쪽 이야기입니다. Pod는 기본적으로 **휘발성**입니다. Pod 안에서 만든 파일은 그 Pod의 수명과 함께하다가, Pod가 죽으면 같이 사라집니다. 개발용 로그라면 괜찮겠지만, DB 데이터나 업로드 파일처럼 **사라지면 안 되는 값**을 Pod 안에 두는 건 위험합니다.
...
처음 들으면 이름이 비슷해서 헷갈립니다. 한 장 그림으로 보면 관계가 분명해집니다.
...
ConfigMap·Secret으로 설정을 바깥으로 뺐고, PV/PVC로 데이터 영속성도 잡았습니다. 네트워크는 챕터 5에서 다 정리된 상태입니다. 이제 챕터 3에서 Compose로 띄웠던 풀스택 구성을 Kubernetes 버전으로 옮길 모든 조각이 모였습니다.
...
진행 순서는 단순합니다. 먼저 YAML을 쭉 훑어 각 리소스가 무슨 역할인지 정리하고, 그다음 `kubectl apply -f k8s/ --recursive` 한 줄로 **네 서비스를 한 번에** 올립니다. Compose 시절의 `docker compose up`처럼, K8s에서도 폴더 하나를 통째로 던져 주면 쿠버네티스가 알아서 서로를 찾아 맞물립니다. "이름만 맞춰두면 나머지는 알아서 맞물린다"는 선언형의 감각이 한 번의 명령으로 드러납니다.
...
지금까지 오픈이가 만든 리소스는 전부 `default`에 들어가 있었습니다. 혼자 실습일 땐 괜찮지만, 팀 여러 서비스가 한 `default`에 섞이면 이름 충돌이 시작됩니다. **Namespace**는 회사 건물의 **층**처럼 같은 클러스터 안을 논리적으로 나눠주는 공간입니다.
...
자동 복구와 무중단 배포까지 뒤에 깔려 있는 상태입니다. Minikube 한 노드 안에서 오픈이의 풀스택 구성이 **K8s 위에서 살아 움직이는** 그림이 완성됐습니다. 오늘 대목표의 도착 지점입니다.
...
챕터 3의 `docker compose up` 한 줄이 한 번에 띄워준 네 개 컨테이너가, Kubernetes에서도 같은 모습으로 살아났습니다. 차이는 분명합니다. Compose는 "지금 이 순간 네 개를 함께 켜는 명령", K8s는 "네 개가 이런 상태여야 한다는 선언". Pod 하나가 죽어도 다시 살아납니다. 트래픽이 몰리면 복제가 붙고, 배포를 갈아 끼워도 접속이 끊기지 않습니다.

## 6.4 문제가 생겼을 때 — 기본 디버깅

### 6.4.1 지우고 다시 만들기 전에 먼저 읽기

배포에 한 번 성공한 뒤에도 하루쯤 지나 같은 명령을 쳤을 때 일이 꼬이는 경우가 있었습니다. 예를 들어 Pod 상태가 `Pending`에 머물러 `Running`으로 넘어가지 않는 상황. 이럴 때 가장 피해야 하는 반응이 "지우고 다시 만들기"입니다. Kubernetes는 대부분 **이유를 자기 로그에 이미 적어 놓기** 때문에, 그 로그를 먼저 읽으면 원인이 분명해집니다.
...
*'get → describe → logs → endpoints. 이 네 줄만 손에 익히면 된다.'*
...
| **Ingress 접속 불가** | Ingress Controller 미설치 또는 URL 만료 | `kubectl get pod -n ingress-nginx`로 컨트롤러 확인, `minikube service <서비스명> --url` 재발급 |

**지우지 말고, 먼저 읽는다.** 이 한 줄이 이번 챕터에서 챙긴 습관입니다.
...
"모든 걸 알고 시작"할 수는 없습니다. 오픈이가 그랬듯, 문제를 만나며 성장하는 겁니다. 챕터 1에서 선배가 던졌던 **"환경이 달라서 그래. Docker 한번 알아봐."** 한 마디가 여기까지 온 출발점이었다는 것, 그리고 새로운 에러를 만났을 때 `describe`를 먼저 치는 손끝이 남아 있다는 것. 그 두 가지면 이 책이 바라던 자리에 도착한 셈입니다.
