...
챕터 4의 끝에서 오픈이는 한 가지 벽 앞에 멈춰 섰습니다. Deployment로 Pod 개수는 맞춰졌지만, 되살아난 Pod의 IP가 매번 달라졌습니다. 프론트엔드 Pod가 백엔드 Pod를 부르려면 주소가 있어야 하는데, 그 주소가 계속 바뀌면 어떻게 연결을 이어가라는 걸까요. 자동 복구까지는 해결됐지만, 브라우저에서 안정적으로 접속할 수 있는 길은 아직 남아 있습니다.

오늘 목표는 한 줄로 정리됩니다.

**Pod IP가 바뀌어도 끊기지 않는 주소를 만들고, 외부 브라우저에서 URL로 접근하는 전체 경로를 완성한다.**
...
세 요소가 이어지는 흐름을 따라가 봅니다. Service가 고정 주소(가상 IP)를 선언합니다. Endpoint Controller는 Pod IP 변화를 감시해 주소록을 갱신하고, kube-proxy가 그 주소록을 각 노드의 iptables 규칙으로 집행하죠. 오픈이가 풀어야 했던 IP 변동 문제는 이 세 요소가 손을 맞춰 해결합니다.
...
맞는 말이었습니다. `minikube service`는 터미널 하나를 점유하는 임시 경로였고, NodePort는 `노드IP:3xxxx` 식이라 사용자가 포트 번호를 외우고 입력해야 했습니다. 실제 서비스는 도메인 기반 URL로 접속되고, 서비스마다 포트를 따로 외우는 게 아니라 같은 도메인 안에서 경로로 나누어 씁니다. Service 혼자서는 이걸 감당하지 못합니다.
...
각 단계마다 "확인하는 것"이 딱 하나씩입니다. Ingress는 URL만, Service는 Label만, kube-proxy는 IP/Port만, Pod는 마지막에 비즈니스 로직을 태웁니다. 역할이 나뉘어 있으니 네트워크 계층은 전달만 하고, 비즈니스 로직은 Pod 안에서 봅니다. 이 원칙이 표 다섯 줄에 담겨 있습니다.

*'각자 한 가지만 본다. 그래서 고장 나도 어느 층에서 막혔는지 찾기 쉽다.'*

오늘 대목표였던 "외부 브라우저에서 URL로 접근하는 전체 경로"가 이 표 하나에 담겨 있습니다. 다음 챕터 종합실습에서는 이 경로가 실제로 동작하는 모습을 확인해 봅니다.
...
오늘 대목표였던 "Pod IP 변동에도 끊기지 않는 주소"와 "브라우저에서 URL로 접근하는 경로"가 이 챕터에서 완성됐습니다. 다음 챕터에서는 이 경로 위에 **설정·비밀번호·데이터 영속성**을 추가하고, 챕터 3에서 만든 풀스택 구성을 Kubernetes 위에 실제로 배포해 봅니다.
