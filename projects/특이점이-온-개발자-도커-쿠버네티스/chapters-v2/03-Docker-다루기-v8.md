...
며칠 뒤 팀 회의에서 새 프로젝트 이야기가 나왔습니다. 사내용 웹 서비스를 만드는 프로젝트였습니다. 프론트엔드와 백엔드, 그리고 데이터를 담을 DB까지 필요한 구성이었습니다. 팀장이 오픈이를 보며 말했습니다.
...
패키지 명을 한 글자만 잘못 쳐도 처음부터 다시였습니다. 설정을 살짝 고치고 싶어도 전 과정을 반복해야 했죠. 프로젝트를 본격적으로 시작하기 전에, 이 수동 작업부터 자동화해야겠다는 생각이 들었습니다.
...
3. **docker run**: docker run: 생성된 이미지를 기반으로 실제 컨테이너를 실행합니다.

컨테이너를 삭제하더라도 이미지는 그대로 남아 있습니다. 덕분에 똑같은 환경이 필요할 때마다 언제든 다시 띄울 수 있습니다. 어제 수동으로 commit 명령어를 쳐서 만들었던 이미지의 빈자리를, 이제는 Dockerfile이 자동으로 채워주게 됩니다.
...
어제만 해도 컨테이너 접속, 업데이트, 설치, 종료, 커밋까지 다섯 단계를 일일이 수동으로 처리해야 했습니다. 하지만 이제는 **Dockerfile 하나와 명령어 한 줄로** 모든 과정이 끝났습니다.
...
NGINX가 요청을 나눠준다는 건 알겠는데, 설정 파일을 어떻게 써야 하는지가 문제였습니다. `nginx.conf`를 열어보니 처음 보는 지시어들이 나왔습니다.

*'upstream이 뭐지, location은 또 뭐지?'*

구조를 보니 세 가지 덩어리로 나뉘어 있었습니다. 배달 앱 주문 흐름을 떠올리면 딱 맞아떨어집니다.
...
경로로 서버를 가르는 방법은 됐습니다. 그런데 같은 앱을 두 대, 세 대로 복제하면 NGINX 설정은 어떻게 바꿔야 할까.

*'upstream을 두 개 만드나? 아니면 하나 안에 서버를 여러 개 넣을 수 있나?'*

답은 후자였습니다. upstream 블록 안에 server 줄을 늘리기만 하면 NGINX가 요청을 번갈아 뿌려줍니다. 놀이공원 매표소 창구 두 곳이 손님을 번갈아 받는 방식입니다. **라운드 로빈**이라고 부릅니다.
...
로드밸런싱 테스트를 하다가 오픈이는 로그를 보고 멈췄습니다. 새로고침할 때마다 같은 이미지 파일 요청이 백엔드 로그에 찍히고 있었습니다.

*'로고 이미지를 100명이 동시에 열면 백엔드가 100번 응답해야 하는 건가? 다 똑같은 파일인데?'*

자주 꺼내 입는 옷을 옷걸이에 미리 걸어두듯, 자주 요청되는 파일을 **NGINX 앞단에 잠시 보관**해두면 됩니다. 이 저장 과정을 **캐싱**이라고 부릅니다.
...
Redis 컨테이너와 API 컨테이너가 서로를 **어떻게 부를 것인가**. 오픈이는 코드에 `host='redis'`라고 써봤습니다. 컨테이너 이름으로 부르면 될 것 같았습니다.

*'이름으로 부르려면 뭔가 조건이 있지 않았나?'*

3.2에서 오픈이는 컨테이너를 `docker run`으로 각각 실행했더니 기본 bridge에 들어갔고, 이름으로 부르는 게 불가능해서 `host.docker.internal`로 호스트를 경유해야 했습니다. 챕터 2에서 예고했던 "사용자 정의 네트워크"가 여기서 실제로 쓰입니다. 사용자 정의 네트워크는 이 우회를 제거합니다. 같은 네트워크에 들어간 컨테이너들은 Docker 내장 DNS가 이름을 IP로 바꿔줍니다. **컨테이너 이름을 그대로 도메인처럼 쓸 수 있습니다**.
...
세션 문제는 Redis로 풀렸습니다. 그런데 다음 날 출근해서 동료가 물었습니다.

**동료**: "세션은 됐는데, 채팅 기록은 어디다 저장해?"

Redis는 메모리 기반이라 컨테이너를 재시작하면 데이터가 사라집니다. 세션처럼 잠깐 들고 있는 데이터는 괜찮지만, 회원 정보나 채팅 기록처럼 **영구히 보관되어야 하는 데이터**는 다른 곳에 둬야 했습니다. 프로젝트에도 사용자 데이터를 담을 DB가 필요했습니다. MySQL 역시 컨테이너로 띄울 수 있습니다.
...
> **참고: Dockerfile에 비밀번호를 직접 적는 방식**
> 이번 실습에서는 학습 편의를 위해 환경 변수에 비밀번호를 직접 넣었습니다. 실무에서는 비밀번호가 이미지에 남아 유출 위험이 있으므로, Docker의 `--env-file` 옵션이나 Kubernetes의 Secret을 사용합니다. Secret은 챕터 6에서 다룹니다.
...
**동료**: "오, 그러면 프론트랑 백엔드랑 DB가 다 따로 뜨는 거지? 한 번에 띄울 수는 없어?"
