...
팀장의 말이 며칠째 귀에 남았습니다. Compose로 풀스택을 한 줄에 띄울 줄 알게 됐다고 내심 뿌듯해하던 참이었는데, 저 짧은 질문 앞에서 답이 막혔습니다. 오픈이는 저녁을 먹고 돌아와 노트북을 다시 열었습니다.

*'정말 누가 살려주지?'*

빈 화면에 커서만 깜빡이는 동안, 풀리지 않은 자리들이 하나씩 떠올랐습니다.
...
**선언형 관리의 감각을 잡고, Pod와 Deployment로 자동 복구·스케일링·무중단 배포가 가능한 상태까지 간다.**

한 번의 저녁으로 전부 끝나는 주제는 아닙니다. 오늘 할 일은 "선언만 하면 시스템이 그 상태를 맞춘다"는 K8s의 핵심 감각을 잡고, Pod와 Deployment까지 직접 띄워 보는 데까지입니다. Service와 Ingress, 설정 관리는 이후 챕터에서 이어집니다.

## 4.1 왜 Kubernetes가 필요한가

### 4.1.1 Compose는 "지금 이대로", Kubernetes는 "이 상태를 유지해라"

두 도구의 성격 차이를 먼저 한 줄로 잡아둡니다.

- **Docker Compose**: *"이 순간 이 세 컨테이너를 같이 띄워라."* 한 번의 실행 지시입니다.
- **Kubernetes**: *"이 서비스 3개가 항상 떠 있게 유지해라."* 지속되는 **약속**이죠.

Compose가 **명령형**이라면 K8s는 **선언형**입니다. 원하는 최종 상태만 써두면 시스템이 **현재 상태를 계속 비교해 가며** 그 상태를 맞춥니다. 컨테이너가 하나 죽으면 누가 알려주지 않아도 새로 띄우고, 숫자를 바꾸면 그 숫자에 맞춰 복제합니다. 어제까지 오픈이가 `docker run`과 `docker compose up`으로 "지금 이 순간" 상태를 만들던 방식과는 결이 다릅니다.
...
이 방식이 실제로 돌아가는 모양은 **프랜차이즈 본사**와 비슷합니다. 본사는 가맹점을 하나하나 운영하지 않습니다. "수도권에 매장 50개 유지" 같은 **지침**만 내려보냅니다. 한 매장이 문을 닫으면 근처에 새 매장이 열리고, 장마철에 주문이 늘면 배달 인력이 늘어납니다. 본사는 **원하는 상태**만 선언하고, 현장은 그 상태를 맞춥니다.
...
구조까지 훑었으니, 이 본사를 어디에 세워 실습할지가 다음 질문입니다. 실제 클러스터는 서버 여러 대가 필요한데, 오픈이에게는 노트북 한 대뿐이었습니다.
...
Minikube가 떴으니 이제 **kubectl**을 쓸 차례입니다. K8s 안의 리소스를 다루는 명령줄 도구입니다. 오늘 풀려는 과제로 가는 첫 조각이 **Pod**입니다.
...
Pod는 떴는데, 오늘의 목적 중 "자동 복구"와는 아직 거리가 있습니다. 이 Pod가 죽으면 어떻게 될지가 다음 질문입니다.
...
`hello-pod1`은 그냥 사라졌습니다. 목록에서 지워졌고, 아무도 다시 살려주지 않았습니다.
...
필요한 건 "매장이 항상 n개 있어야 한다"고 선언해 둔 지침서입니다. 그 지침서 역할을 하는 리소스가 **Deployment**입니다. 오늘 대목표의 중심축이 여기에 있습니다.
...
Pod를 1개가 아니라 여러 개 유지하고 싶다면 `replicas`를 올리면 됩니다. 이 역할은 **ReplicaSet**이 맡습니다. Deployment가 자동으로 생성해서 관리하는 리소스입니다. 오늘 풀려는 과제 중 또 한 축인 **스케일링**이 이 지점에서 해결됩니다.
...
남은 마지막 축이 **무중단 배포**입니다. 기존 Pod를 한꺼번에 내리고 새 Pod를 올리면 그 사이에 서비스가 멈춥니다. **롤링 업데이트**는 새 Pod를 먼저 띄운 뒤 기존 Pod를 순차적으로 교체하는 방식입니다.
...
오늘 대목표였던 자동 복구·스케일링·무중단 배포가 여기까지로 Pod와 Deployment 선언만으로 확인됐습니다.
...
Pod별로 다른 IP가 찍혔습니다. 첫 번째 Pod를 지우고 다시 조회해 보니, 자리를 채우는 새 Pod의 IP가 이전 번호와 달라져 있었습니다. Pod 개수는 그대로 유지되지만, **주방 전화번호가 계속 바뀌는** 상황입니다.
...
IP라면 백엔드 Pod가 한 번 죽고 다시 태어날 때마다 프론트의 설정을 고쳐야 한다는 이야기가 됩니다. 이 상태로는 실제 서비스에 쓸 수 없습니다.

그때 동료에게서 메시지가 왔습니다.

**동료**: "로그인 지금 되긴 되는데 왜 자꾸 끊기지?"

자동 복구·스케일링·무중단 배포는 맞춰놨지만, **브라우저에서 안정적으로 접속할 수 있는 상태**는 아직 아닙니다. 가맹점 주방이 바뀌어도 손님이 늘 거는 **대표 전화번호**가 있어야 합니다. 그게 다음 챕터의 주제인 **Service**의 역할입니다.
...
오늘 대목표였던 자동 복구·스케일링·무중단 배포까지는 맞춰놨습니다. 하지만 되살아난 Pod의 IP는 매번 바뀝니다. 다음 챕터에서 **Service**와 **Ingress**로 "브라우저에서 안정적으로 접속 가능한 상태"를 만드는 일이 이어집니다.
