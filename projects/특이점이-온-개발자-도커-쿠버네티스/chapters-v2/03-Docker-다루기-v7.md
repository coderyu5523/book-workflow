...
챕터 2를 마친 오픈이는 다음 날 저녁 다시 노트북 앞에 앉았습니다. 어제는 Docker의 개념과 기본 사용법을 익혔고, commit으로 이미지를 저장해 Hub에 올리는 흐름까지 경험했죠. 그런데 `docker commit`은 한 번이면 족했습니다. 두 번째부터 손이 무거워졌죠. 게다가 서비스는 앱 하나로 굴러가지 않습니다. 프론트엔드와 백엔드, DB가 각자 떠서 서로 맞물려야 했습니다.

오늘의 목표를 이렇게 잡았습니다.

**Dockerfile로 이미지를 자동 빌드하고, 웹 서버·세션 저장소·DB를 차례로 컨테이너로 세워본 뒤, 마지막에는 웹 서버·백엔드·DB를 묶은 풀스택 구성을 명령 한 줄로 띄우는 상태로 완성한다.**

수동 작업의 피로를 Dockerfile로 지우고, 컨테이너 하나가 아니라 **서비스** 전체를 다뤄봅니다.

## 3.1 Dockerfile — 환경을 자동으로 만들기

### 3.1.1 매번 수동으로 깔기는 지친다

어제 오픈이는 Tomcat 컨테이너에 들어가 `apt update`를 치고 vim을 깔고 index.html을 만든 뒤 `docker commit`으로 이미지를 남겼습니다. 한 번은 재미있었죠. 두 번째는 손이 무거웠습니다.

*'이걸 매일 하라고?'*

패키지명을 한 글자만 잘못 치면 처음부터 다시였습니다. 설정을 살짝 고치면 전 과정을 처음부터 반복해야 했습니다. 수동 작업을 **레시피 한 장**으로 자동화하는 것. 오늘 첫 과제였습니다.

![](../assets/CH03/chap02-provisioning.png)

*그림 3-1 수동 세팅과 Dockerfile 자동화 비교*

요리 레시피 카드를 떠올리면 됩니다. 재료와 순서만 적어두면 누가 만들어도 같은 맛이 납니다. Docker에서 이 역할을 하는 파일이 **Dockerfile**입니다.
...
여기에 **WORKDIR**와 **COPY**를 얹으면 호스트의 파일을 컨테이너 안으로 끌어오는 흐름까지 잡힙니다. WORKDIR는 이후 명령이 실행될 기본 폴더를 지정하고, COPY는 호스트의 파일을 컨테이너 내부로 옮겨 놓습니다.
...
이미지를 재현 가능하게 만드는 조각은 여기까지 갖춰졌습니다. 그다음은 그 앱이 사용자 요청을 어떻게 받을지였습니다.

## 3.2 NGINX — 요청을 앞에서 받아 나눠주기

### 3.2.1 왜 서버 앞에 NGINX를 둘까

앱이 한 대만 떠 있다면 사용자가 그 앱 주소로 바로 접속하면 됩니다. 그런데 사용자가 늘어 같은 앱을 두세 대로 복제해서 띄워야 할 때가 옵니다. 사용자는 `my-service.com` 하나만 알고 있습니다. 복제된 세 대의 IP를 각자 알려줄 수는 없습니다. 앞쪽에 창구 하나를 두고, 그 창구가 뒤의 서버들로 요청을 나눠주는 구조가 필요합니다.
...
NGINX는 웹 서버이자 요청을 중계하는 **리버스 프록시**입니다. 사용자의 요청을 NGINX가 중간에서 받아 뒤쪽 서버로 넘기고, 응답도 NGINX가 받아 사용자에게 돌려줍니다. 실제 서버 주소를 바깥에 노출하지 않아 보안에도 좋습니다. 요청을 여러 서버로 나눠주는 **로드밸런싱**도 자연스럽게 따라옵니다. 오늘 풀스택 구성에서 앱 앞에 꼭 서 있어야 할 요소입니다.
...
NGINX 설정 파일(`nginx.conf`)에서 자주 마주치는 핵심 지시어는 세 개입니다. 배달 앱 주문 흐름에 대응해 보면 금방 읽힙니다.
...
브라우저에서 `localhost:80/app1`로 들어가면 app1 서버가, `localhost:80/app2`로 들어가면 app2 서버가 응답했습니다.
...
이번엔 같은 서비스를 **여러 대로 복제**해서 NGINX가 요청을 번갈아 뿌려주는 구조입니다. 놀이공원 매표소 창구 두 곳이 손님을 1번 → 2번 순으로 번갈아 받는 방식과 같습니다. 이름이 **라운드 로빈**입니다. 앱도 사용자가 늘어나면 복제해야 하니 꼭 익혀둘 포인트입니다.
...
브라우저로 `localhost:80/app1`을 새로고침하면 응답 HTML 자체는 똑같습니다. 두 컨테이너가 같은 이미지에서 올라와 같은 `index.html`을 돌려주기 때문입니다. 대신 각 컨테이너의 access 로그(`docker logs <컨테이너>`)를 보면 새로고침마다 8000번 컨테이너와 8001번 컨테이너로 요청이 번갈아 들어간 것이 확인됩니다.
...
같은 파일이 초당 수백 번 요청되는 상황이라면, 이미지 한 장을 백엔드가 매번 응답하는 건 낭비입니다. 자주 입는 옷을 박스 깊숙이 넣지 않고 옷걸이에 걸어두듯, 자주 요청되는 파일은 **NGINX 앞단에 잠시 보관**해두면 됩니다. 이 저장 과정을 **캐싱**이라고 부릅니다.
...
세 예제를 다 돌리고 나면 `nginx.conf`의 구조는 늘 비슷합니다. 뼈대는 같고, 그 위에 **"upstream 두 세트 / server 두 줄 / 캐시 옵션"** 중 어떤 옷을 입혔는지만 다릅니다. 오픈이가 만들 구성에 들어갈 NGINX도 이 중에서 필요한 옵션만 골라 쓰면 됩니다.

## 3.3 Redis — 서버가 여러 대일 때 세션은 어디에

### 3.3.1 서버 여러 대면 생기는 세션 문제

NGINX로 백엔드 복제가 가능해졌는데, 복제 자체가 새 문제를 만드는 경우가 있습니다. 사용자가 로그인하고 장바구니에 상품을 담은 뒤 결제 페이지로 넘어갔을 때 로그인이 풀리는 증상입니다. 내부에 로그인 기능이 있는 앱이라면 이 문제를 피해 갈 수 없습니다.
...
해결책은 단순합니다. **세션을 서버 메모리 대신, 서버들 바깥의 공용 저장소에 둔다**.
...
3.2에서 오픈이는 컨테이너를 `docker run`으로 각각 실행했더니 기본 bridge에 들어갔고, 이름으로 부르는 게 불가능해서 `host.docker.internal`로 호스트를 경유해야 했습니다. 사용자 정의 네트워크는 이 우회를 제거합니다. 같은 네트워크에 들어간 컨테이너들은 Docker 내장 DNS가 이름을 IP로 바꿔주므로 **컨테이너 이름을 그대로 도메인처럼 씁니다**.
...
세 컨테이너가 같은 `myNetwork` 안에 있으니 api1·api2가 `host='redis'`만 써도 Redis 컨테이너에 닿습니다. 서버가 두 대로 복제돼도 세션이 끊기지 않는 구조가 만들어졌습니다.

*'데이터를 서버 밖으로 빼두기만 해도 이 문제가 통째로 사라지네.'*

## 3.4 MySQL — 영구 데이터는 DB 서버에

Redis는 메모리 기반이라 재시작하면 데이터가 사라집니다. 회원 정보, 게시글, 주문 내역처럼 **영구히 보관되어야 하는 데이터**는 관계형 데이터베이스가 따로 필요합니다. 오늘 구성에서도 DB가 한 자리 차지하게 되는데, MySQL 역시 컨테이너로 띄울 수 있습니다.
...
앱이 붙을 DB까지 컨테이너로 준비할 수 있습니다. 이제 이 조각들을 **한 덩어리**로 묶어 띄우는 일만 남았습니다.

## 3.5 Docker Compose — 여러 컨테이너를 한 번에

### 3.5.1 docker run을 매번 치는 게 지친다

여기까지 오픈이가 머릿속에 그려둔 구성은 여러 덩어리였습니다. 네트워크 만들고, Redis 띄우고, API 두 대 띄우고, MySQL 띄우고, NGINX 띄우기. 실습 중에는 이 조각들을 하나씩 쳐보는 것까지 됐는데, 매일 이 긴 명령 묶음을 손으로 치는 건 오늘의 목표와 정반대였습니다.
...
해결책은 **여러 컨테이너를 하나의 파일에 적어두고 명령어 하나로 실행**하는 것입니다. 그 도구가 **Docker Compose**입니다. 오케스트라의 악보처럼, 각 악기(컨테이너)의 역할을 한 악보에 적어두면 지휘 한 번으로 모든 연주자가 동시에 시작합니다. 오늘 대목표인 "한 줄 명령으로 전체 구성 띄우기"가 이 도구로 가능해집니다.

![](../assets/CH03/chap02-49.png)

*그림 3-32 Docker Compose 방식 — 한 번에 생성 및 연결*

Compose가 풀어주는 것은 세 가지입니다.
...
옵션이 많아 보이지만 실습에서 쓰는 건 이 중 일부입니다.
...
결과는 EX01과 동일한데, 손에서 타이핑할 `docker build` + `docker run` 여섯 명령이 `docker compose up` 한 줄로 줄었습니다.
...
챕터 도입에서 잡은 대목표가 이 절에서 완성됩니다. **프론트엔드(NGINX) + 백엔드(Spring Boot) + DB(MySQL)** 세 서비스를 Compose 한 파일에 담고, `docker compose up` 한 줄로 전체를 띄우는 구성입니다.
...
*'앞 실습에서 하나씩 쌓은 부품이 그대로 제자리에 들어간다.'*
...
`server backend:8080`이 이번 구성의 연결 고리입니다. Compose에 `backend`라는 이름의 서비스가 있으니, 같은 네트워크 안의 frontend 컨테이너는 `backend`라는 이름으로 바로 그 서비스에 닿습니다.
...
세 서비스가 같은 `ex07-network`에 묶여 있으니, frontend의 NGINX는 `backend`라는 이름으로, backend는 `db`라는 이름으로 서로를 부릅니다. backend의 `SPRING_DATASOURCE_URL`에 들어간 `jdbc:mysql://db:3306/...`이 그 호출 지점입니다.
...
백엔드 컨테이너는 git clone과 Gradle 빌드를 먼저 돌리기 때문에 처음에는 몇 분 걸립니다. 준비가 끝난 뒤 브라우저에서 `localhost:80`에 접속했더니 사용자 목록이 조회됐습니다.
...
어제까지 순서대로 치던 명령 묶음이 한 번에 정리됐습니다. 한 줄 명령 뒤에서 일어난 일을 되짚어 봤습니다.
...
대목표였던 "풀스택 구성을 한 줄 명령에 띄우기"가 실습으로 확인됐습니다. 환경 불일치로 막혔던 상황은 이 구조 안에서는 반복되지 않습니다. 이미지에 필요한 모든 것이 들어 있고, Compose가 세 서비스를 하나의 네트워크로 묶어 서비스 이름만으로 서로를 부르게 해주기 때문입니다.

## 3.7 Compose까지 와도 남은 것

오늘의 목표는 달성됐습니다. 그런데 운영 관점으로 눈을 돌리면 아직 풀리지 않은 문제가 보였습니다.
