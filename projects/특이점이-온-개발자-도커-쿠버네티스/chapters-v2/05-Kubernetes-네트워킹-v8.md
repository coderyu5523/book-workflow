...
Service YAML 샘플을 찾아보니 포트 관련 항목이 세 줄 나왔습니다. `nodePort`, `port`, `targetPort`. 셋 다 포트인데 왜 나눠놓은 건지 처음엔 구분이 안 됐습니다.

*'이름이 너무 비슷한데, 어디에 어떤 숫자를 써야 하지?'*

찾아보니 셋 다 같은 요청 흐름을 각자 다른 입장에서 부르는 번호였습니다. **누구 입장에서 붙인 번호인지**로 구분하면 헷갈리지 않습니다.
