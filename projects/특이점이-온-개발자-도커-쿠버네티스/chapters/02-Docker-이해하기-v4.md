...
- Docker의 작동 원리와 핵심 구성 요소를 이해한다.
- Docker를 설치하고 컨테이너를 실행·조작한다.
- 컨테이너를 이미지로 저장하고 Dockerhub에 공유한다.
- 마운트를 사용하여 컨테이너 삭제 후에도 데이터를 보존한다.

챕터 1에서 컨테이너가 왜 필요한지 살펴보았습니다. 이제 Docker가 컨테이너를 어떻게 만드는지 원리를 이해하고, 직접 설치해서 사용해 보겠습니다.
...
*그림 2-9: nginx 이미지 다운로드*

### 2.3.3 docker run : 컨테이너 실행
...
> 이미 리눅스 명령어에 익숙하다면 2.5 컨테이너 : 생명주기와 옵션으로 건너뛰어도 됩니다.
...
*그림 2-13: Ubuntu 컨테이너 실행*



### 2.4.2 탐색 명령어 : pwd, cd, ls, clear
...
### 2.4.3 파일/폴더 관리 : mkdir, touch, rm, cp, mv
...
### 2.5.1 exit vs detach : 빠져나올 때 살아있는 경우 vs 죽은 경우
...
*그림 2-28: ubuntu -dit 실행*

이번에는 ubuntu 컨테이너가 백그라운드에서 정상적으로 실행됩니다.

### 2.5.3 CMD : 컨테이너 시작 명령어 덮어쓰기
...
*그림 2-29: CMD로 프로세스 유지*

### 2.5.4 attach : 실행 중인 컨테이너에 접근
...
*그림 2-44: 호스트 PC의 폴더와 컨테이너 내부 폴더가 직접 연결*
...
*그림 2-49: Docker 엔진이 관리하는 내부 저장 공간에 데이터 저장*
