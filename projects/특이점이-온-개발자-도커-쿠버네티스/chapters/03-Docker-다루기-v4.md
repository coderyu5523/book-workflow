...
챕터 2에서 Ubuntu 컨테이너를 실행할 때마다 vim 패키지를 직접 설치해야 했습니다. 컨테이너를 새로 만들 때마다 같은 작업을 반복하는 건 비효율적입니다. 필요한 환경을 미리 준비해두고 컨테이너 실행 시 자동으로 적용되면 이런 반복을 줄일 수 있습니다. 이를 프로비저닝이라 합니다.
...
이전 섹션에서는 Dockerfile로 컨테이너 환경을 자동화하는 방법을 배웠습니다. 서비스를 운영하다 보면 사용자가 늘어나고 백엔드 서버 한 대로는 트래픽을 감당하기 어려운 상황이 옵니다. 챕터 2에서 nginx 이미지를 실행해 봤는데, 사실 NGINX는 단순히 HTML을 보여주는 것 외에도 강력한 기능을 갖추고 있습니다. 이번에는 NGINX의 주요 기능을 살펴보겠습니다.
...
*그림 3-19: 첫 번째 요청 (MISS) - 캐시가 비어있어 백엔드 서버에 요청 후 응답을 캐시에 저장*

![캐시 HIT](images/cache-hit.png)
*그림 3-20: 두 번째 요청 (HIT) - 캐시에 저장된 응답을 바로 반환, 백엔드 서버 접근 없음*
...
*그림 3-23: X-Cache-Status: MISS 확인*
...
*그림 3-25: 세션 불일치 - 서버 1에 저장된 세션이 서버 2에는 없어 요청이 실패*
...
*그림 3-26: Redis로 해결 - 세션을 공유 저장소에 보관하여 어떤 서버에서든 조회 가능*
...
*그림 3-35: 기존 방식 — Dockerfile 개별 빌드 및 실행*
...
*그림 3-36: Docker Compose 방식 — 한 번에 생성 및 연결*
...
*그림 3-38: Docker Desktop에서 Compose 컨테이너 확인*

![실행 결과](images/chap02-54.png)
*그림 3-39: Docker Desktop에서 Compose 로그 확인*
...
*그림 3-41: docker compose up 실행 결과*
