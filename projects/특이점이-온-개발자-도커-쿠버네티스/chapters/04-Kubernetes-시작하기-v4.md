...
> 4.2부터 4.7까지 작성하는 **YAML(yml)** 파일은 https://github.com/metacoding-10-linux-docker/docker/tree/master/yaml 에서 확인할 수 있습니다.
...
*그림 4-14: Deployment가 ReplicaSet을 생성하고, ReplicaSet이 Pod 수를 유지한다*
...
*그림 4-15: Selector가 app: web인 Pod만 매칭하고, app: db인 Pod는 매칭하지 않음*
...
*그림 4-16: Deployment와 Pod 생성 확인*

아래 명령어로 현재 생성된 Pod를 모두 제거합니다. 4.2에서 만든 hello-pod1, hello-pod2도 포함됩니다.
...
*그림 4-18: Pod 3이 종료되면 ReplicaSet이 설정 개수를 맞추기 위해 Pod 4를 자동 생성*
...
*그림 4-19: replicas 설정으로 Pod 4개 생성*
...
*그림 4-23: Service는 고정 주소를 제공한다. Pod IP가 바뀌어도 Service 주소는 그대로다*
...
*그림 4-29: 외부 요청은 Ingress를 통해 Service로 전달된다*
...
*그림 4-30: ConfigMap과 Secret은 Pod에 설정과 민감 정보를 주입한다*
...
4.5.1에서 작성한 deploy-ex03.yml의 `envFrom` 항목에 `secretRef`를 추가합니다. 추가된 부분만 표시합니다.
...
*그림 4-33: Pod 환경 변수에 Secret 값 확인*
...
*그림 4-34: ConfigMap 변경 후 Pod 재시작으로 반영되는 흐름*
...
*그림 4-35: ConfigMap 수정 후 Pod 재시작*
...
*그림 4-37: PV는 실제 저장 공간, PVC는 요청서. PVC를 통해 Pod에 저장소를 연결한다*
...
챕터 3에서는 프론트 서버 + 백엔드 서버 + DB 서버를 Docker Compose로 실행했습니다. 이번에는 Redis 서버를 추가해 미니큐브 환경에서 실행해보겠습니다.
...
*그림 4-43: ex08 풀스택 Kubernetes 아키텍처*

챕터 3에서는 브라우저에서 `localhost`로 프론트엔드에 직접 접속했습니다. 쿠버네티스에서는 외부 요청이 클러스터 내부로 바로 들어갈 수 없어 `Ingress`가 앞단에 놓입니다. 브라우저 요청은 `Ingress`가 받아 `Frontend Service`로 넘기고, 이후 프론트엔드 → 백엔드 → DB/Redis 순으로 흐릅니다.
...
*그림 4-44: 같은 Cluster 안에서 Namespace로 리소스를 분리한다*
...
*그림 4-45: Ingress Controller 활성화*
...
*그림 4-46: Ingress Controller 실행 확인*
...
*그림 4-50: Deployment, Pod, Service 조회*
...
Docker와 Kubernetes를 향한 여정이 마무리되었습니다. 챕터 2에서 컨테이너 하나를 띄우는 것으로 시작해, 챕터 3에서 Docker Compose로 여러 컨테이너를 한 번에 관리했습니다. 챕터 4에서는 쿠버네티스로 자동화된 운영 환경까지 구성했습니다. Ingress로 외부 요청을 도메인 기반으로 연결하는 것까지 마쳤으니, 실무 환경과의 거리가 한층 좁혀졌습니다. 실무에는 Helm, CI/CD 파이프라인 같은 주제가 더 기다리고 있습니다. 여기까지 온 이상, 그 어떤 기술도 낯설지 않을 것입니다.
