...
- `localhost/stores` → `stores-service`  ("매장 안내 …")
...
```bash
kubectl apply -f yaml/order-deploy.yml
kubectl apply -f yaml/order-service.yml
kubectl apply -f yaml/stores-deploy.yml
kubectl apply -f yaml/stores-service.yml
```
