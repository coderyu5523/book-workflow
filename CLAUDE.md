...
8. **전체 존댓말**: 이야기 파트 내러티브와 설명문 모두 존댓말(-습니다/-입니다/-세요)로 쓴다.
   - 내러티브: `"입사 3일 차였다."` → `"입사 3일 차였습니다."` / `"팀장이 자리로 왔다."` → `"팀장이 자리로 왔습니다."`
   - 설명 문장: `~이다`, `~한다` → `~입니다`, `~합니다`
   - 독자 직접 지시: `참고하자` → `참고하세요`, `생각해보자` → `생각해 보겠습니다`
   - 섹션 제목: `이것만은 기억하자` → `이것만은 기억하세요`
   - **유지**: 캐릭터 대화 따옴표 안 내용, `*(내면 독백)*` 이탤릭 안 내용 — 구어체 그대로 유지
9. **대화 화자 구분**: 캐릭터 대화는 **볼드 역할 + 콜론** 형식으로 표시한다. 내면 독백은 이탤릭체 `*(...)* `로 표시한다.
   - 캐릭터 대화: `**팀장**: "AI로 사내 문서 검색 시스템 만들어봐."` / `**나**: "언제까지요?"`
   - LLM 대화(블록쿼트): `> **나**: 질문` / `> **LLM**: 답변`
   - 내면 독백: `*(ChatGPT도 뭐든 대답하잖아. LLM에게 직접 물어보면 되는 거 아니야?)*`
   - 내러티브(배경 서술): 볼드/이탤릭 없이 일반 텍스트
10. **등장인물 역할 표기**: 스토리 속 직원은 실명 대신 역할로만 표기한다. 코드/쿼리 예시 데이터도 동일하게 적용.
    - 말하는 인물: `**팀장**`, `**나**`, `**동료**`, `**사서**`, `**안내데스크**` 등 역할명 사용
    - 조회 대상 직원: 이름 없이 `A 사원`, `B 팀원`, `C 대리` 등 알파벳+직급 형식
    - 예시 쿼리: `"진아 연차 몇 개?"` → `"A 사원 연차 몇 개?"` / `"김대리 연차"` → `"팀원 연차"`
11. **챕터 닫기**: 챕터의 **맨 마지막**(기술 파트 뒤)에 "이것만은 기억하세요" + 다음 버전 예고.
12. **기술은 저자가 결정**: AI가 제안, 저자가 최종 결정.
13. **실패 먼저**: 정답부터 알려주지 않는다. 실수 → 에러 → 해결 순서.
14. **이미지는 플레이스홀더**: 집필 시 `[GEMINI PROMPT]`(개념도) / `[CAPTURE NEEDED]`(실습 캡처) 삽입. 실제 이미지는 집필 완료 후 생성.
15. **스크린샷과 코드 블록 중복 금지**: 스크린샷이 확보되면 동일 내용의 출력 코드 블록 제거.
