...
8. **문체 구분**: 독자에게 말하는 설명문은 **존댓말(-입니다/-합니다/-세요)**로 쓴다. 이야기 파트의 내러티브(과거형 서술), 캐릭터 대화, 내면 독백(괄호 안)은 그대로 유지한다.
   ...
   - **유지**: `"입사 3일 차였다."` 같은 과거형 내러티브, `*(오, 이거면 되는 거 아니야?)*` 같은 이탤릭 독백
9. **대화 화자 구분**: 캐릭터 대화는 **볼드 이름 + 콜론** 형식으로 표시한다. 내면 독백은 이탤릭체 `*(...)* `로 표시한다.
...
10. **챕터 닫기**: 챕터의 **맨 마지막**(기술 파트 뒤)에 "이것만은 기억하세요" + 다음 버전 예고.
11. **기술은 저자가 결정**: AI가 제안, 저자가 최종 결정.
12. **실패 먼저**: 정답부터 알려주지 않는다. 실수 → 에러 → 해결 순서.
13. **이미지는 플레이스홀더**: 집필 시 `[GEMINI PROMPT]`(개념도) / `[CAPTURE NEEDED]`(실습 캡처) 삽입. 실제 이미지는 집필 완료 후 생성.
14. **스크린샷과 코드 블록 중복 금지**: 스크린샷이 확보되면 동일 내용의 출력 코드 블록 제거.
